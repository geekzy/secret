...
<script language="JavaScript" type="text/JavaScript">
	function konfirmasi(id,awal){
       ...
       if (tamp == true)
	   {
	      return location.href ="pakar_solusi.php?id="+id+"&hapus=hapus&awal="+awal;
	   }
	}
...
if($Submit=="Tambah")
{
	...
	mysql_query("INSERT INTO solusi(kode_solusi, nama_solusi)
 				values ('$kode','$nama')");
	header("location:pakar_solusi.php?awal=$awal");
	exit();
	}
}
...
elseif ($Submit=="Edit")
{
	$rubah="update solusi set kode_solusi='$id', nama_solusi='$nama' where kode_solusi='$id'";
     mysql_query($rubah);
	 header("location:pakar_solusi.php?awal=$awal");
	 exit();
}
...
if($hapus=="hapus")
{
   $delete="delete from solusi where kode_solusi='$id'";
   mysql_query($delete);
   header("location:pakar_solusi.php?awal=$awal");
	exit();
}
...
if(!$kirim == edit)
{
	$qry_id = mysql_query("SELECT kode_solusi FROM solusi order by kode_solusi desc");
	$row_id = mysql_fetch_row ($qry_id);
	$id_akhir = explode("T",$row_id[0]);
	...
	echo "<tr><td colspan=2 align='left' class='title'>INPUT DATA solusi</td></tr>";
	echo "<tr><td align='right' valign='top' width='150'>Kode solusi</td><td><input type='text' name='kode' maxlength='4' size='4' value='$kode_gjl'></td></tr>";
	echo "<tr><td align='right' valign='top' width='150'>Nama solusi</td><td><textarea name='nama' cols='50'></textarea></td></tr>";
	...
	{
 	$sql1=mysql_query (" select * from solusi where kode_solusi='$id'");
 	$row1 = mysql_fetch_row ($sql1);
	echo "<table align='center' width='550' class='content1'>";
	echo "<tr><td colspan=2 align='left' class='title'>EDIT DATA solusi</td></tr>";
	echo "<tr><td align='right' valign='top' width='150'>Kode solusi</td><td><input type='text' name='id' maxlength='4' size='4' value='$row1[0]'></td></tr>";
	echo "<tr><td align='right' valign='top' width='150'>Nama solusi</td><TD><textarea name='nama' cols='50'>$row1[1]</textarea></td></tr>";
...
<table width="550" border="1" bordercolor="#EEEEEE" align="center" cellspacing="0" cellpading="0">
	<tr><td colspan="5" class="headertabel">DAFTAR SOLUSI PENYAKIT</td></tr>
  <tr bgcolor="#ABBECA"> 
  ...
  <? 
    ...
    $sql=mysql_query ("select * from solusi order by kode_solusi asc limit $awal,20");
    while ($row = mysql_fetch_row ($sql))
	{
		...
		echo "<td align='center' valign='top'><a href=pakar_solusi.php?id=$row[0]&kirim=Edit&awal=$awal><img src='../images/edit.gif' alt='Edit' border='0'></a>"; 
...
<?
$query2=mysql_query("select * from solusi");
...
{
$awal=(($j-1)*19+$j)-1;
print("<a href='pakar_solusi.php?awal=$awal'><font color=red>[$j</a>]\n");
}

echo "<br><div align='center'><a href='pakar_solusi.php?awal=$awal#top'>
