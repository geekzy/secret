...
<script language="JavaScript" type="text/JavaScript">
	function konfirmasi(penyakit,solusi){
	   var tamp = confirm("Yakin anda akan menghapus?");
       if (tamp == true)
	   {
	      return location.href ="pakar_solusipenyakit.php?penyakit="+penyakit+"&solusi="+solusi+"&hapus=hapus";
	   }
	}
</script>
</head>
...
if($Submit=="Insert")
{
	if (!empty($solusi))
	{
	mysql_query("INSERT INTO solusipenyakit (kd_penyakit, kd_solusi) VALUES ('$penyakit','$solusi')");
	header("location:pakar_solusipenyakit.php?penyakit=$penyakit");
	 exit();
	 }
}
...
if($hapus=="hapus")
{
   $delete="DELETE FROM solusipenyakit WHERE kd_penyakit='$penyakit' AND kd_solusi='$solusi'";
   mysql_query($delete);
   header("location:pakar_solusipenyakit.php?penyakit=$penyakit");
	 exit();
}
...
<?
	if ($penyakit=="")
	{
		echo "<table width='550'><tr><td colspan='4' class='headertabel'>DAFTAR PENYAKIT</td></tr>
				...
				<td align='center' width='60'>Solusi</td></tr>";
		...
		while ($row = mysql_fetch_row ($q_penyakit))
		{
			...
			echo "<td align='center' valign='top'><a href=pakar_solusipenyakit.php?penyakit=$row[0]><img src='../images/isi.gif' alt='solusi' border=0></a>"; 
		}
		echo "</table>";
		echo "<br><div align='center'><a href='pakar_solusipenyakit.php#top'>
	...
	else
	{
		...
		$q_solusi=mysql_query("SELECT * FROM solusi");
		...
		echo "<tr><td align='right' valign='top'>solusi : </td><td><select name='solusi' size='15'>";
		while ($r_solusi=mysql_fetch_row($q_solusi)){
		$string=explode(" ",$r_solusi[1]);
		$penggalan="";
		for($i=0;$i<8;$i++) $penggalan=$penggalan." ".$string[$i];
		if (!$string[$i]=="")
		$penggalan=$penggalan."....";
		echo "<option value=$r_solusi[0]>$r_solusi[0]-$penggalan</option>";
		}
		echo "</select></td></tr>";
		...
		echo "<table ALIGN='CENTER' width='90%'>
				<tr bgcolor='#ABBECA'>
				<td align='center'>Kode Solusi</td>
				<td align='center'>Nama Solusi</td>
				<td align='center'>Delete</td></tr>";
		$q_gpenyakit=mysql_query("	SELECT a.kd_penyakit, a.kd_solusi, b.nama_solusi
									FROM solusipenyakit a, solusi b 
									WHERE b.kode_solusi=a.kd_solusi AND a.kd_penyakit='$penyakit'");
		...
		echo "</table>";
		echo "<br><div align='center'><a href='pakar_solusipenyakit.php?penyakit=$penyakit#top'>
				<img src='../images/up.gif' alt='top' border='0'><br>Top</a></div><br>";
	}
?>
