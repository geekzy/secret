...
*   $sm_language, I think
*
* $Id: class.multi_language.php,v 1.1.1.1 2004/06/27 18:34:42 rudi Exp $
*/

if (defined('CLASS_MULTI_LANGUAGE')) {
	return 0;
} else {
	define('CLASS_MULTI_LANGUAGE', TRUE);
}
...
class multi_language {
	...
	function multi_language($language='english', $lang_dir='language') {
		...
		*/
		...
		$this->charset = $this->get_charset($this->language);
	}
	...
	function gettext_php_load_strings() {
		...
		*/
		...
		$filename = $this->lang_dir.$file_separator.$this->language.'.po';
		...
		if ($file == false) {
			...
			$gettext_php_loaded_language = $this->language;
...
//}
	...
	function get_charset($language='english') {
		...
		switch ($language) {
			case 'english':	
				return $languages['en_US']['CHARSET'];
			case 'indonesian': 
				return $languages['id_ID']['CHARSET'];
			case 'korean':
				return $languages['ko_KR']['CHARSET'];
			default:
				return 'iso-8859-1';
		}
	}
