...
*/
  function match($token)
    {
    //print(" $token ".$this->_lookahead." <br>");
    switch($token)
        {
        case ADD:
            return($token);
        ...
        case CLOSE:
            ...
            break;
        default:
            if(eregi("^[A-Z][0-9]+$",$token) and !ereg("[0-9]",$this->_lookahead))
                {
                return($token);
                }
            elseif(is_numeric($token) and !ereg("[0-9]",$this->_lookahead))
                {
                return($token);
                }
            return(0);
  ...
  function _expression()
    ...
    while ($this->_current_token == ADD or $this->_current_token == SUB)
        ...
        if ($this->_current_token == ADD)
            {
            $this->_advance();
            //$result = $this->_create_tree(pack("C", $this->ptg['ptgAdd']), $result, $this->_term());
            $result = $this->_create_tree('ptgAdd', $result, $this->_term());
            }
        else 
            {
            $this->_advance();
            //$result = $this->_create_tree(pack("C", $this->ptg['ptgSub']), $result, $this->_term());
  ...
  function _term()
    ...
    while ($this->_current_token == MUL || $this->_current_token == DIV)
        ...
        if ($this->_current_token == MUL)
            {
            $this->_advance();
            //$result = $this->_create_tree(pack("C", $this->ptg['ptgMul']), $result, $this->_fact());
            $result = $this->_create_tree('ptgMul', $result, $this->_fact());
            }
        else 
            {
            $this->_advance();
            //$result = $this->_create_tree(pack("C", $this->ptg['ptgDiv']), $result, $this->_fact());
