	...
	*/
	function send($data, $timeout=0) {
		...
		//	"User-Agent: $this->title/$this->version\r\n".
			"User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; MSIE 5.5; Windows 98) Opera 7.0  [en]\r\n".
			"Host: ".$this->host."\r\n";
		// http auth
		$credentials = '';
		if($this->username != '') {
		...
		// referer
		if ($this->referer!='') {
			$this->outgoing_payload .= 'Referer: '.$this->referer."\r\n";
		}
		// cookie
		unset($this->cookie['path']);
		if(is_array($this->cookie) && count($this->cookie)) {
			$this->debug('setting cookie');
			foreach ($this->cookie as $key => $value) {
				if ($cookie_payload) $cookie_payload .= "; ";
				$cookie_payload .= "$key=$value";
			}
			$this->outgoing_payload .= 'Cookie: '.$cookie_payload."\r\n";
		}
		// set content type
		$this->outgoing_payload .= 'Content-Type: text/html; charset='.$this->soap_defencoding."\r\nContent-Length: ".strlen($data)."\r\n";
		// http encoding
		...
		if(isset($headers['Set-Cookie']) && $headers['Set-Cookie'] != ''){
			$piece = explode(';',$headers['Set-Cookie']);
			foreach($piece as $cookie_line){
				$arr = explode('=',$cookie_line);
				if(count($arr) >= 2){
				$this->cookie[trim($arr[0])] = trim($arr[1]);
				}
			}
		}
		$this->referer = $this->host;
