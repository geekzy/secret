...
class cek_kelengkapan_data_edar_local_controller extends depkes2_manager {
	...
	function cek_kelengkapan_data_edar_local_controller() {
		$this->cek_kelengkapan_data_edar_local_label = array (
			...
			'nama_pendaftar_edar_pkrt_local' => 'Nama Pendaftar',
			...
			'nama_produk' => 'Jenis pkrt Yang Diproduksi',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_1_edar_pkrt_local.no_tt as svalue,
				pendaftar_edar_pkrt_local.nama_pabrik as svalue2
			FROM
			tt_1_edar_pkrt_local
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_pkrt_local.kode_subdit)
			LEFT OUTER JOIN cek_kelengkapan_data_edar_localON(cek_kelengkapan_data_edar_local.no_tt=tt_1_edar_pkrt_local.no_tt)
			LEFT OUTER JOIN pendaftar_edar_pkrt_localON(pendaftar_edar_pkrt_local.kode_pendaftar_edar_pkrt_local= tt_1_edar_pkrt_local.kode_pendaftar_edar_pkrt_local)
			ORDER BY
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_kelengkapan_data_edar_localON(cek_kelengkapan_data_edar_local.no_tt=tt_1_edar_pkrt_local.no_tt)
			LEFT OUTER JOIN pendaftar_edar_pkrt_localON(pendaftar_edar_pkrt_local.kode_pendaftar_edar_pkrt_local= tt_1_edar_pkrt_local.kode_pendaftar_edar_pkrt_local)
			WHERE cek_kelengkapan_data_edar_local.no_tt IS NULL
			ORDER BY
				tt_1_edar_pkrt_local.date_insert
		...
		$result = pendaftar_edar_pkrt_local::select($fk_sql);
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_edar_pkrt_local.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_edar_pkrt_localLEFT OUTER JOIN tt_1_edar_pkrt_localON(tt_1_edar_pkrt_local.kode_pendaftar_edar_pkrt_local= pendaftar_edar_pkrt_local.kode_pendaftar_edar_pkrt_local) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_pkrt_local.kode_subdit)  WHERE tt_1_edar_pkrt_local.no_tt =\''+ b +'\'';

		jumpto1('frame_tt_1_edar_pkrt_local.php?sql='+sql+'&a='+b)

		";

	$GLOBALS['out_before_footer'] = '
 	<iframe marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=auto name=\'iframe_entry1\' id=\'iframe_entry1\' style="width:0;height:0" src="frame_tt_1_edar_pkrt_local.php"></iframe>
	...
	// create add cek_kelengkapan_data_edar_localform
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM cek_kelengkapan_data_edar_localWHERE id_cek_1='{$record['id_cek_1']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		global $adodb;
			$sqlx = "SELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_pkrt ORDER BY id_kelengkapan";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_edar_localWHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$optional_arr['id_golongan'] = 'user_defined';
                ...
                $arr['sql'] = 'SELECT id_golongan as val, golongan as txt FROM gol_pkrt';
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Edar PKRT Lokal";
	...
	// create update cek_kelengkapan_data_edar_localform
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			FROM
			pendaftar_edar_pkrt_local
			LEFT OUTER JOIN tt_1_edar_pkrt_localON(tt_1_edar_pkrt_local.kode_pendaftar_edar_pkrt_local= pendaftar_edar_pkrt_local.kode_pendaftar_edar_pkrt_local)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_pkrt_local.kode_subdit)
			...
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Jenis Produksi pkrt','',0,'L');$pdf->Cell(50,7,': '.$nama_produk.'','',0,'L');
			$pdf->Ln(5);
			...
			LEFT OUTER JOIN kelengkapan_pkrt ON(kelengkapan_pkrt.id_kelengkapan = detail_cek_kelengkapan_data_edar_local.id_kelengkapan)
			WHERE
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_localWHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		$adodb->Execute("DELETE FROM cek_kelengkapan_data_edar_localWHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		...
		$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_kelengkapan_data_edar_localORDER BY id_cek_1 DESC LIMIT 1");
		$id_cek_1 = $rsx->fields['id_cek_1'];

		include "class.detail_cek_kelengkapan_data_edar_local.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_kelengkapan_data_edar_local::add($record);
		}
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Edar PKRT Lokal');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
                        $success = $adodb->Execute("delete from cek_kelengkapan_data_edar_localwhere ".$query);
			$success = $adodb->Execute("delete from detail_cek_kelengkapan_data_edar_localwhere ".$query);
		}
                if ($success){
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Edar PKRT Lokal'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_edar_pkrt_local.nama_pabrik,
		tt_1_edar_pkrt_local.no_tt as status,
		tt_1_edar_pkrt_local.no_tt as status_pending,
		tt_1_edar_pkrt_local.no_tt as status_tolak,
		tt_1_edar_pkrt_local.no_tt as status_selesai,
		...
		FROM
		tt_1_edar_pkrt_local
		LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.no_tt = tt_1_edar_pkrt_local.no_tt)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit = tt_1_edar_pkrt_local.kode_subdit)
		...
		$USql = "
			...
			kelengkapan_pkrt.nama_kelengkapan,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_penilai =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_penilai,
			detail_cek_kelengkapan_data_edar_ local.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_kelengkapan_data_edar_local.alasan_kaseks,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_kelengkapan_data_edar_local.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_kelengkapan_data_edar_local.alasan_direktur
		...
		FROM
		surat_keputusan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_localON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_keputusan.id_cek_1)
		WHERE
		cek_kelengkapan_data_edar_local.no_tt =\'$f\'";
		...
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_localON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_penolakan.id_cek_1)
		WHERE
		cek_kelengkapan_data_edar_local.no_tt =\'$f\'";
		...
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_kelengkapan_data_edar_localON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_tambahan_data.kepada_surat)
		WHERE
		cek_kelengkapan_data_edar_local.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_edar_pkrt_localWHERE no_tt=\'$f\'");
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Edar PKRT Lokal'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Izin Edar PKRT Lokal';
