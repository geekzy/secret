...
if (class_exists('cek_kelengkapan_data_edar_local_controller')) {
	// do nothing
} else if (defined('CLASS_cek_kelengkapan_data_edar_local_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_cek_kelengkapan_data_edar_local_CONTROLLER', TRUE);

include_once 'class.cek_kelengkapan_data_edar_local.inc.php';
class cek_kelengkapan_data_edar_local_controller extends depkes2_manager {
	var $cek_kelengkapan_data_edar_local_label;
	var $optional_arr;
	function cek_kelengkapan_data_edar_local_controller() {
		$this->cek_kelengkapan_data_edar_local_label = array (
	...
	function no_tt_form(&$config) {
		...
		include_once 'class.pendaftar_edar_alkes_local.inc.php';
		if($_GET['id_cek_1']){
			if($bag=='kaseksi'){
				$fk_sql =
				"SELECT
					tt_1_edar_alkes_local.no_tt as skey,
					tt_1_edar_alkes_local.urut_no_tt as svalue,
					pendaftar_edar_alkes_local.nama_pabrik as svalue2
				FROM
				tt_1_edar_alkes_local
				LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_alkes_local.kode_subdit)
				LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.no_tt=tt_1_edar_alkes_local.no_tt)
				...
				LEFT OUTER JOIN inbox_izin_alkes_local_subdit ON(inbox_izin_alkes_local_subdit.no_tt = tt_1_edar_alkes_local.no_tt )
				LEFT OUTER JOIN inbox_izin_alkes_local_seksi ON(inbox_izin_alkes_local_seksi.id_inbox_izin_alkes_local_subdit = inbox_izin_alkes_local_subdit.id_inbox_izin_alkes_local_subdit)
				WHERE inbox_izin_alkes_local_seksi.insert_by='$userlog'
				ORDER BY
				...
				LEFT OUTER JOIN pendaftar_edar_alkes_local ON(pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local = tt_1_edar_alkes_local.kode_pendaftar_edar_alkes_local )
				LEFT OUTER JOIN inbox_izin_alkes_local_subdit ON(inbox_izin_alkes_local_subdit.no_tt = tt_1_edar_alkes_local.no_tt )
				RIGHT OUTER JOIN inbox_izin_alkes_local_seksi ON(inbox_izin_alkes_local_seksi.id_inbox_izin_alkes_local_subdit = inbox_izin_alkes_local_subdit.id_inbox_izin_alkes_local_subdit)
				ORDER BY
				tt_1_edar_alkes_local.date_insert
				";
			}
			...
			LEFT OUTER JOIN inbox_izin_alkes_local_subdit ON(inbox_izin_alkes_local_subdit.no_tt = tt_1_edar_alkes_local.no_tt )
			RIGHT OUTER JOIN inbox_izin_alkes_local_seksi ON(inbox_izin_alkes_local_seksi.id_inbox_izin_alkes_local_subdit = inbox_izin_alkes_local_subdit.id_inbox_izin_alkes_local_subdit)
			WHERE ((tt_1_edar_alkes_local.no_tt IS NULL) OR (inbox_izin_alkes_local_seksi.insert_by='$userlog')) AND cek_kelengkapan_data_edar_local.no_tt IS NULL
			ORDER BY
			tt_1_edar_alkes_local.date_insert
			";
		...
		$result = pendaftar_edar_alkes_local::select($fk_sql);
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_edar_alkes_local.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_edar_alkes_local LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.kode_pendaftar_edar_alkes_local = pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_alkes_local.kode_subdit)  WHERE tt_1_edar_alkes_local.no_tt =\''+ b +'\'';
	...
	// create add cek_kelengkapan_data_edar_local form
	function add_cek_kelengkapan_data_edar_local_form() {
		include_once 'class.xform.inc.php';
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		//print_r($_GET);exit();
		$record = $_GET;
		$label_arr = $this->cek_kelengkapan_data_edar_local_label;
		...
		$rs = $adodb->Execute("SELECT * FROM cek_kelengkapan_data_edar_local WHERE id_cek_1='{$record['id_cek_1']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$optional_arr['dapendaftar_edar_alkes_localte_1_rule'] = '';
		...
		global $adodb;
			$sqlx = "SELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_admin_local ORDER BY id_kelengkapan";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_edar_local_admin WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		global $adodb;
			$sqlx2 = "SELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_teknis_local ORDER BY id_kelengkapan";
			$rsx2 = $adodb->Execute($sqlx2);
			$a=1;
			while(! $rsx2->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_edar_local_teknis WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx2->fields['id_kelengkapan']."'";
				...
				$has2 .= "<td><input type=checkbox name=status_penilaix2[".$a."] value=1 class=tex2t ".$check1."></td></td><td><textarea name=alasan_penilaix2[".$a."] class='text'>".$alasan_penilaix2."</textarea></td>
					...
					$has2 .= "<td><input type=checkbox name=status_kaseksix2[".$a."] value=1 class=tex2t ".$check2."></td><td><textarea name=alasan_kaseksix2[".$a."] class='text'>".$alasan_kaseksix2."</textarea>
						...
						<input type=hidden name=alasan_kasubditx2[".$a."] value='".$alasan_kasubditx2."'>
						<input type=hidden name=status_direkturx2[".$a."] value='".$status_direkturx2."'>
						<input type=hidden name=alasan_direktuSELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_teknis_local ORDER BY id_kelengkapanrx2[".$a."] value='".$alasan_direkturx2."'>
					</td>";
					}else{
						if($ses->action == 'kasubdit'){
						if($check1 == "checked"){$has2 .= "<td><input type=hidden name=status_penilaix2[".$a."] value='".$status_penilaix2."'>".$tex2t1.",<br><input type=hidden name=alasan_penilaix2[".$a."] class='text' value='".$alasan_penilaix2."'>".$alasan_penilaix2."</td>";}else{ $has2 .= "<td><input type=hidden name=alasan_penilaix2[".$a."] class='text' value='".$alasan_penilaix2."'><nobr>&nbsp;".$tex2t1.",</nobr><br><nobr>".$alasan_penilaix2."</nobr></td>";}
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek  Kelengkapan Izin Edar Alkes Lokal";
	...
	// create update cek_kelengkapan_data_edar_local form
	function update_cek_kelengkapan_data_edar_local_form() {
		return $this->add_cek_kelengkapan_data_edar_local_form();
	}

	function view_cek_kelengkapan_data_edar_local_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = cek_kelengkapan_data_edar_local::get_field_set();
		...
		$result = cek_kelengkapan_data_edar_local::get($record);
		$value_arr = $result[0];
		$label_arr = $this->cek_kelengkapan_data_edar_local_label;
		global $adodb;
		...
		$pdf->Ln(5);
		$sqlA = "SELECT
		cek_kelengkapan_data_edar_local.id_cek_1,
		cek_kelengkapan_data_edar_local.no_tt,
		cek_kelengkapan_data_edar_local.kemasan,
		cek_kelengkapan_data_edar_local.id_golongan,
		cek_kelengkapan_data_edar_local.ukuran,
		kategori_edar.nama_kategori_edar,
		sub_kategori_edar.nama_sub_kategori_edar,
		cek_kelengkapan_data_edar_local.nama,
		...
		cek_kelengkapan_data_edar_local.id_golongan as golongan,
		CASE WHEN cek_kelengkapan_data_edar_local.status_penilai ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_penilai,
		cek_kelengkapan_data_edar_local.nama_penilai,
		cek_kelengkapan_data_edar_local.date_1,
		CASE WHEN cek_kelengkapan_data_edar_local.status_kaseksi ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_kaseksi,
		cek_kelengkapan_data_edar_local.nama_kaseksi,
		cek_kelengkapan_data_edar_local.date_2,
		CASE WHEN cek_kelengkapan_data_edar_local.status_kasubdit ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_kasubdit,
		cek_kelengkapan_data_edar_local.nama_kasubdit,
		cek_kelengkapan_data_edar_local.date_3,
		CASE WHEN cek_kelengkapan_data_edar_local.status_direktur ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_direktur,
		cek_kelengkapan_data_edar_local.nama_direktur,
		cek_kelengkapan_data_edar_local.date_4,
		cek_kelengkapan_data_edar_local.keterangan,
		cek_kelengkapan_data_edar_local.insert_by,
		cek_kelengkapan_data_edar_local.date_insert
		FROM cek_kelengkapan_data_edar_local
		...
		LEFT OUTER JOIN gol_edar ON(gol_edar.id_golongan = cek_kelengkapan_data_edar_local.id_golongan)
		WHERE
		cek_kelengkapan_data_edar_local.id_cek_1 ='".$value_arr['id_cek_1']."'
		";
		...
		while(! $hasil->EOF){
			...
			FROM
			pendaftar_edar_alkes_local
			LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.kode_pendaftar_edar_alkes_local = pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_alkes_local.kode_subdit)
			WHERE tt_1_edar_alkes_local.no_tt ='".$no_tt."'
			";
			...
			CASE WHEN detail_cek_kelengkapan_data_edar_local_admin.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_kelengkapan_data_edar_local_admin.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_admin.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_kelengkapan_data_edar_local_admin.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_admin.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_kelengkapan_data_edar_local_admin.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_admin.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_kelengkapan_data_edar_local_admin.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_edar_local_admin
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_edar_local_admin.id_kelengkapan)
			WHERE
			detail_cek_kelengkapan_data_edar_local_admin.id_cek_1='".$id_cek_1."'
			";
			...
			SELECT
			kelengkapan_alkes.nama_kelengkapan,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_teknis.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_teknis.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_teknis.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_edar_local_teknis.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_edar_local_teknis
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_edar_local_teknis.id_kelengkapan)
			WHERE
			detail_cek_kelengkapan_data_edar_local_admin.id_cek_1='".$id_cek_1."'
			GROUP BY kelengkapan_alkes.nama_kelengkapan,
			detail_cek_kelengkapan_data_edar_local_teknis.status_penilai,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_penilai,
			detail_cek_kelengkapan_data_edar_local_teknis.status_kaseksi,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_kaseksi,
			detail_cek_kelengkapan_data_edar_local_teknis.status_kasubdit,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_kasubdit,
			detail_cek_kelengkapan_data_edar_local_teknis.status_direktur,
			detail_cek_kelengkapan_data_edar_local_teknis.alasan_direktur
			";
	...
	// handle event add cek_kelengkapan_data_edar_local
	function add_cek_kelengkapan_data_edar_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($read_direktur == 'on'){ $xread_direktur = '1';}else{ $xread_direktur ='0';}
			...
			cek_kelengkapan_data_edar_local::add($record);

		$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_kelengkapan_data_edar_local ORDER BY id_cek_1 DESC LIMIT 1");
		$id_cek_1 = $rsx->fields['id_cek_1'];

		include "class.detail_cek_kelengkapan_data_edar_local_admin.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_kelengkapan_data_edar_local_admin::add($record);
		}
		
		
		

		include "class.detail_cek_kelengkapan_data_edar_local_teknis.inc.php";
		for($a=1;$a<=$jml_kelengkapan2;$a++){
			...
			$alasan_direkturx2 = $_POST['alasan_direkturx2'][$a];
				...
				detail_cek_kelengkapan_data_edar_local_teknis::add($record);
		}
	...
	// handle event update cek_kelengkapan_data_edar_local
	function update_cek_kelengkapan_data_edar_local() {
		//return $this->add_cek_kelengkapan_data_edar_local();
		...
		$_block = new block();
		$_block->set_config('title', ('Update Cek  Kelengkapan Izin Edar Alkes Lokal Status'));
		$_block->set_config('width', 595);
		if ($result = cek_kelengkapan_data_edar_local::get($record)) {
			$record = $result[0];
			...
			eval($this->save_config);
			if (cek_kelengkapan_data_edar_local::update($record)) {
				$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_local_admin WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
				$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_local_teknis WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
				//$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_kelengkapan_data_edar_local ORDER BY id_cek_1 DESC LIMIT 1");
	...
	// handle delete cek_kelengkapan_data_edar_local
	function delete_cek_kelengkapan_data_edar_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
					...
					$optional_arr['date_4'] = TRUE;
					$optional_arr['status_direktur'] = TRUE;
            $success = $adodb->Execute("delete from cek_kelengkapan_data_edar_local where ".$query);
            $success .= $adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_local_admin WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
            $success .= $adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_local_teknis WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		}
                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'cek_kelengkapan_data_edar_local <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek  Kelengkapan Izin Edar Alkes Lokal Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list cek_kelengkapan_data_edar_local
	function list_cek_kelengkapan_data_edar_local($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
			...
			cek_kelengkapan_data_edar_local.id_cek_1,
			cek_kelengkapan_data_edar_local.no_tt,
			cek_kelengkapan_data_edar_local.no_tt as urut_no_tt,
			cek_kelengkapan_data_edar_local.no_tt as alamat,
			cek_kelengkapan_data_edar_local.no_tt as subdit,
			cek_kelengkapan_data_edar_local.no_pemohon,
			cek_kelengkapan_data_edar_local.kemasan,
			cek_kelengkapan_data_edar_local.ukuran,
			cek_kelengkapan_data_edar_local.nama_produk,
			cek_kelengkapan_data_edar_local.id_golongan as golongan,
			CASE WHEN cek_kelengkapan_data_edar_local.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_kelengkapan_data_edar_local.nama_penilai,
			cek_kelengkapan_data_edar_local.date_1,
			CASE WHEN cek_kelengkapan_data_edar_local.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_kelengkapan_data_edar_local.nama_kaseksi,
			cek_kelengkapan_data_edar_local.date_2,
			CASE WHEN cek_kelengkapan_data_edar_local.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_kelengkapan_data_edar_local.nama_kasubdit,
			cek_kelengkapan_data_edar_local.date_3,
			CASE WHEN cek_kelengkapan_data_edar_local.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_kelengkapan_data_edar_local.nama_direktur,
			cek_kelengkapan_data_edar_local.date_4,
			...
			cek_kelengkapan_data_edar_local.insert_by,
			cek_kelengkapan_data_edar_local.date_insert,
			Case WHEN cek_kelengkapan_data_edar_local.read_penilai='0' THEN 'Unread'
			else 'Read' END AS read_penilai,
			cek_kelengkapan_data_edar_local.id_cek_1 as detail
			FROM cek_kelengkapan_data_edar_local
			LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.no_tt = cek_kelengkapan_data_edar_local.no_tt)
			WHERE
			cek_kelengkapan_data_edar_local.nama_penilai ='' OR cek_kelengkapan_data_edar_local.nama_penilai = '$user'
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_kelengkapan_data_edar_local.insert_by,
			cek_kelengkapan_data_edar_local.date_insert,
			CASE WHEN cek_kelengkapan_data_edar_local.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_kelengkapan_data_edar_local.read_kaseksi='0' THEN 'Unread'
			...
			LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.no_tt = cek_kelengkapan_data_edar_local.no_tt)
			WHERE
			cek_kelengkapan_data_edar_local.nama_kaseksi ='' OR cek_kelengkapan_data_edar_local.nama_kaseksi = '$user'
			ORDER BY cek_kelengkapan_data_edar_local.read_kaseksi
			";
		}
		...
		if($bag =='kasubdit'){
			...
			cek_kelengkapan_data_edar_local.insert_by,
			cek_kelengkapan_data_edar_local.date_insert,
			CASE WHEN cek_kelengkapan_data_edar_local.indi_kasubdit='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_kelengkapan_data_edar_local.read_kasubdit='0' THEN 'Unread'
			else 'Read' END AS read_kasubdit,
			cek_kelengkapan_data_edar_local.id_cek_1 as detail
			FROM cek_kelengkapan_data_edar_local
			LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.no_tt = cek_kelengkapan_data_edar_local.no_tt)
			WHERE
			cek_kelengkapan_data_edar_local.nama_kasubdit ='' OR cek_kelengkapan_data_edar_local.nama_kasubdit='$user'
			ORDER BY cek_kelengkapan_data_edar_local.read_kasubdit
			";
		}
		...
		if($bag =='direktur'){
			...
			cek_kelengkapan_data_edar_local.insert_by,
			cek_kelengkapan_data_edar_local.date_insert,
			CASE WHEN cek_kelengkapan_data_edar_local.indi_direktur='0' THEN 'Return'
			...
			LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.no_tt = cek_kelengkapan_data_edar_local.no_tt)
			WHERE
			cek_kelengkapan_data_edar_local.nama_direktur ='' OR cek_kelengkapan_data_edar_local.nama_direktur ='$user'
			ORDER BY cek_kelengkapan_data_edar_local.read_direktur
			";
		}
		...
		LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.kode_pendaftar_edar_alkes_local = pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_alkes_local.kode_subdit)
		WHERE tt_1_edar_alkes_local.no_tt =\'$f\'";
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_kelengkapan_data_edar_local WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_kelengkapan_data_edar_local WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_kelengkapan_data_edar_local WHERE no_tt=\'$f\'");
		...
		LEFT OUTER JOIN tt_1_edar_alkes_local ON(tt_1_edar_alkes_local.kode_pendaftar_edar_alkes_local = pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_alkes_local.kode_subdit)
		WHERE tt_1_edar_alkes_local.no_tt =\'$c\'";
		...
		$USql = "
			SELECT
			kelengkapan_alkes.nama_kelengkapan,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_penilai =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_penilai,
			detail_cek_kelengkapan_data_edar_local.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_kelengkapan_data_edar_local.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_kelengkapan_data_edar_local.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_edar_local.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_kelengkapan_data_edar_local.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_edar_local
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_edar_local.id_kelengkapan)
			WHERE
			id_cek_1=\'%s\' AND detail_cek_kelengkapan_data_edar_local.alasan_kaseksi IS NOT NULL
			";
			global $adodb;
...
//		if ($this->get_permission('fill_this')) {
  		if($bag=='kaseksi'){
			...
			'?action=add' .
			'\', 800, 600, null, null, \'add_cek_kelengkapan_data_edar_local\');'.
			'win.focus()',
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 800, 600, null, null, \'edit_cek_kelengkapan_data_edar_local\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_cek_kelengkapan_data_edar_local\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_cek_kelengkapan_data_edar_local\');'.
			'win.focus()',
			"label"=>__('Print'),
			"type"=>"button",
		...
		$label_arr = $this->cek_kelengkapan_data_edar_local_label;
		$config = array (
			'id'		=> 'cek_kelengkapan_data_edar_local',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek  Kelengkapan Izin Edar Alkes Lokal'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print cek_kelengkapan_data_edar_local
	function print_cek_kelengkapan_data_edar_local() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_cek_kelengkapan_data_edar_local($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$cek_kelengkapan_data_edar_local_controller = new cek_kelengkapan_data_edar_local_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_kelengkapan_data_edar_local_controller->add_cek_kelengkapan_data_edar_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $cek_kelengkapan_data_edar_local_controller->add_cek_kelengkapan_data_edar_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangeno_tt()"';
			$out_content = $cek_kelengkapan_data_edar_local_controller->update_cek_kelengkapan_data_edar_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $cek_kelengkapan_data_edar_local_controller->update_cek_kelengkapan_data_edar_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $cek_kelengkapan_data_edar_local_controller->view_cek_kelengkapan_data_edar_local_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $cek_kelengkapan_data_edar_local_controller->delete_cek_kelengkapan_data_edar_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'local':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_kelengkapan_data_edar_local_controller->local_cek_kelengkapan_data_edar_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postlocal':
			$out_content = $cek_kelengkapan_data_edar_local_controller->local_cek_kelengkapan_data_edar_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $cek_kelengkapan_data_edar_local_controller->print_cek_kelengkapan_data_edar_local();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $cek_kelengkapan_data_edar_local_controller->list_cek_kelengkapan_data_edar_local();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Cek  Kelengkapan Izin Edar Alkes Lokal Administration';
