...
if (class_exists('surat_tmb_data_edar_pkrt_local_controller')) {
	// do nothing
} else if (defined('CLASS_surat_tmb_data_edar_pkrt_local_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_tmb_data_edar_pkrt_local_CONTROLLER', TRUE);

include_once 'class.surat_tmb_data_edar_pkrt_local.inc.php';
class surat_tmb_data_edar_pkrt_local_controller extends depkes2_manager {
	var $surat_tmb_data_edar_pkrt_local_label;
	var $optional_arr;
	function surat_tmb_data_edar_pkrt_local_controller() {
		$this->surat_tmb_data_edar_pkrt_local_label = array (
			'id_surat_tmb_data_edar_pkrt_local' => 'Id Surat Tambahan Data Edar pkrt local',
			'nomor_menteri' => 'NOMOR',
		...
		$this->optional_arr = array (
			'id_surat_tmb_data_edar_pkrt_local' => FALSE,
			'passwd' => FALSE,
			'name' => FALSE,
			'insert_by' => TRUE,
	...
	// create add surat_tmb_data_edar_pkrt_local form
	function add_surat_tmb_data_edar_pkrt_local_form() {
		include_once 'class.xform.inc.php';

//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_tmb_data_edar_pkrt_local_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM surat_tmb_data_edar_pkrt_local WHERE id_surat_tmb_data_edar_pkrt_local='{$record['id_surat_tmb_data_edar_pkrt_local']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_tmb_data_edar_pkrt_local'] = 'protect';
			$mode = 'edit';
		...
		$optional_arr['kepada_surat'] = 'user_defined';
			...
			LEFT OUTER JOIN surat_tmb_data_edar_pkrt_local ON(surat_tmb_data_edar_pkrt_local.kepada_surat = cek_kelengkapan_data_edar_local.id_cek_1)
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_tmb_data_edar_pkrt_local']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Tambahan Data Izin Edar PKRT Lokal";
	...
	// create update surat_tmb_data_edar_pkrt_local form
	function update_surat_tmb_data_edar_pkrt_local_form() {
		return $this->add_surat_tmb_data_edar_pkrt_local_form();
	}

	
	function view_surat_tmb_data_edar_pkrt_local_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_tmb_data_edar_pkrt_local::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_tmb_data_edar_pkrt_local'] = 'protect';

		$record = array (
			'id_surat_tmb_data_edar_pkrt_local' => ${$GLOBALS['get_vars']}['id_surat_tmb_data_edar_pkrt_local']
		);
		$result = surat_tmb_data_edar_pkrt_local::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_tmb_data_edar_pkrt_local_label;
		global $adodb;
		...
		$sqlx = "
		SELECT
		surat_tmb_data_edar_pkrt_local.nomor_surat,
		surat_tmb_data_edar_pkrt_local.lampiran_surat,
		surat_tmb_data_edar_pkrt_local.date_surat,
		pendaftar_edar_pkrt_local.nama_pabrik,
		pendaftar_edar_pkrt_local.alamat_pabrik,
		surat_tmb_data_edar_pkrt_local.alat,
		surat_tmb_data_edar_pkrt_local.isi_1,
		surat_tmb_data_edar_pkrt_local.isi_2,
		surat_tmb_data_edar_pkrt_local.isi_3,
		surat_tmb_data_edar_pkrt_local.isi_4,
		surat_tmb_data_edar_pkrt_local.isi_5,
		surat_tmb_data_edar_pkrt_local.nama,
		surat_tmb_data_edar_pkrt_local.nip,
		surat_tmb_data_edar_pkrt_local.insert_by,
		surat_tmb_data_edar_pkrt_local.date_insert
		FROM surat_tmb_data_edar_pkrt_local
		LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_tmb_data_edar_pkrt_local.kepada_surat)
		LEFT OUTER JOIN tt_1_edar_pkrt_local ON (tt_1_edar_pkrt_local.no_tt = cek_kelengkapan_data_edar_local.no_tt)
		LEFT OUTER JOIN pendaftar_edar_pkrt_local ON( pendaftar_edar_pkrt_local.kode_pendaftar_edar_pkrt_local= tt_1_edar_pkrt_local.kode_pendaftar_edar_pkrt_local)
		WHERE
		surat_tmb_data_edar_pkrt_local.id_surat_tmb_data_edar_pkrt_local ='".$value_arr['id_surat_tmb_data_edar_pkrt_local']."'
		";
	...
	// handle event add surat_tmb_data_edar_pkrt_local
	function add_surat_tmb_data_edar_pkrt_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;

		$rs = $adodb->Execute("SELECT * FROM surat_tmb_data_edar_pkrt_local WHERE id_surat_tmb_data_edar_pkrt_local = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM surat_tmb_data_edar_pkrt_local WHERE id_surat_tmb_data_edar_pkrt_local = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		$status = "Successfull $st '<b>{$record['id_surat_tmb_data_edar_pkrt_local']}</b>'";
		$this->log($status);
		$_block = new block();
		$_block->set_config('title', 'Status Surat Tambahan Data Izin Edar PKRT Lokal');
	...
	// handle event update surat_tmb_data_edar_pkrt_local
	function update_surat_tmb_data_edar_pkrt_local() {
		return $this->add_surat_tmb_data_edar_pkrt_local();
	}

	// handle delete surat_tmb_data_edar_pkrt_local
	function delete_surat_tmb_data_edar_pkrt_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_tmb_data_edar_pkrt_local where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_tmb_data_edar_pkrt_local <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Tambahan Data Izin Edar PKRT Lokal'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_tmb_data_edar_pkrt_local
	function list_surat_tmb_data_edar_pkrt_local($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $adodb, ${$GLOBALS['get_vars']};
		$sql = 'SELECT
		surat_tmb_data_edar_pkrt_local.id_surat_tmb_data_edar_pkrt_local,
		surat_tmb_data_edar_pkrt_local.nomor_surat,
		surat_tmb_data_edar_pkrt_local.lampiran_surat,
		surat_tmb_data_edar_pkrt_local.date_surat,
		pendaftar_edar_pkrt_local.nama_pabrik,
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_tmb_data_edar_pkrt_local' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_tmb_data_edar_pkrt_local\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_tmb_data_edar_pkrt_local\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_tmb_data_edar_pkrt_local\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_tmb_data_edar_pkrt_local\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$label_arr = $this->surat_tmb_data_edar_pkrt_local_label;
		$config = array (
			'id'		=> 'surat_tmb_data_edar_pkrt_local',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Tambahan Data Izin Edar PKRT Lokal'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_tmb_data_edar_pkrt_local
	function print_surat_tmb_data_edar_pkrt_local() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_tmb_data_edar_pkrt_local($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_tmb_data_edar_pkrt_local_controller = new surat_tmb_data_edar_pkrt_local_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->add_surat_tmb_data_edar_pkrt_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->add_surat_tmb_data_edar_pkrt_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->update_surat_tmb_data_edar_pkrt_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->update_surat_tmb_data_edar_pkrt_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_tmb_data_edar_pkrt_local_controller->view_surat_tmb_data_edar_pkrt_local_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->delete_surat_tmb_data_edar_pkrt_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'local':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->local_surat_tmb_data_edar_pkrt_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postlocal':
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->local_surat_tmb_data_edar_pkrt_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->print_surat_tmb_data_edar_pkrt_local();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_tmb_data_edar_pkrt_local_controller->list_surat_tmb_data_edar_pkrt_local();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Tambahan Data Izin Edar PKRT Lokal Administration';
