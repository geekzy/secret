...
if (class_exists('cek_2_alkes_controller')) {
	// do nothing
} else if (defined('CLASS_cek_2_alkes_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_cek_2_alkes_CONTROLLER', TRUE);

include_once 'class.cek_2_alkes.inc.php';
class cek_2_alkes_controller extends depkes2_manager {
	var $cek_2_alkes_label;
	var $optional_arr;
	function cek_2_alkes_controller() {
		$this->cek_2_alkes_label = array (
			'id_cek_2' => 'Id Check Alkes',
			...
			'id_pengelompokan_ubah_ijin_produksi' => 'Item Perubahan',
			'detail_perubahan' => 'Detail Perubahan Kelengkapan',
	...
	function no_tt_form(&$config) {
		...
		if($_GET['id_cek_2']){
			if($bag=='kaseksi'){
				...
				WHERE inbox_ubah_izin_produksi_seksi.insert_by= \'$userlog\'
				ORDER BY
			...
			$fk_sql = ''.
				...
				LEFT OUTER JOIN subdit ON(subdit.id_subdit = tt_2_alkes.kode_subdit)
				LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.no_tt = tt_2_alkes.no_tt)
				WHERE ((tt_2_alkes.no_tt IS NULL) OR (inbox_ubah_izin_produksi_seksi.insert_by= \'$userlog\')) AND cek_2_alkes.no_tt IS NULL
				ORDER BY
					tt_2_alkes.date_insert
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar.alamat_pabrik,surat_keputusan.no_surat_keputusan as nama_propinsi_2 FROM pemohon_ubah_ijin_produksi LEFT OUTER JOIN surat_keputusan ON(surat_keputusan.no_surat_keputusan = pemohon_ubah_ijin_produksi.no_surat_keputusan ) LEFT OUTER JOIN cek_1_alkes ON(cek_1_alkes.id_cek_1 = surat_keputusan.id_cek_1) LEFT OUTER JOIN tt_1_alkes ON(tt_1_alkes.no_tt = cek_1_alkes.no_tt) LEFT OUTER JOIN pendaftar ON(pendaftar.kode_pendaftar = tt_1_alkes.kode_pendaftar) LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.kode_pendaftar = pemohon_ubah_ijin_produksi.kode_pemohon_ubah_ijin_produksi)  WHERE tt_2_alkes.no_tt=\''+ b +'\'';
	...
	'.$GLOBALS['out_before_footer']
		...
		$label_arr['alamat'] = 'Alamat Pabrik';
	...
	// create add cek_2_alkes form
	function add_cek_2_alkes_form() {
		include_once 'class.xform.inc.php';
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		//print_r($_GET);exit();
		$record = $_GET;
		$label_arr = $this->cek_2_alkes_label;
		$optional_arr = $this->optional_arr;

		$field_arr[] = xform::xf('no_tt','N','8');
		$field_arr[] = xform::xf('id_pengelompokan_ubah_ijin_produksi','N','8');
		...
		$field_arr[] = xform::xf('detail_perubahan','C','32');
		...
		$field_arr[] = xform::xf('status_sk','N','8');
		$field_arr[] = xform::xf('read_kaseksi','C','32');
		$field_arr[] = xform::xf('read_kasubdit','C','32');
		$field_arr[] = xform::xf('read_direktur','C','32');


		$rs = $adodb->Execute("SELECT * FROM cek_2_alkes WHERE id_cek_2='{$record['id_cek_2']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_cek_2'] = 'protect';
			$mode = 'edit';
		} else {
			$value_arr = array ();
			$mode = 'add';
		...
		$optional_arr['detail_perubahan_rule'] = '';
		...
		$optional_arr['detail_perubahan'] = 'user_defined';
		$value_arr['detail_perubahan'] = '<textarea name="detail_perubahan" class="text" rows="3" cols="50">'.$value_arr['detail_perubahan'].'</textarea>';
		$status_sk=$value_arr['status_sk'];
		if($status_sk == 1){$check0="checked";$check1="";}else{$check0="";$check1="checked";}
		$optional_arr['status_sk'] = 'user_defined';
		$value_arr['status_sk'] = '<input type="radio" name="status_sk" value="1" '.$check0.'>Perlu <input type="radio" name="status_sk" value="0" '.$check1.'> Tidak Perlu';
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_2']){
				$sqld = "SELECT id_cek_2,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_2_alkes WHERE id_cek_2='".$_GET['id_cek_2']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
				$rsd = $adodb->Execute($sqld);
				$id_cek_2x = $rsd->fields['id_cek_2'];
		...
		$optional_arr['id_pengelompokan_ubah_ijin_produksi'] = 'user_defined';
                $arr = array();
                $arr['name'] = 'id_pengelompokan_ubah_ijin_produksi';
                $arr['selected'] = $value_arr['id_pengelompokan_ubah_ijin_produksi'];
                $arr['sql'] = 'SELECT id_pengelompokan_ubah_ijin_produksi as val, perubahan as txt FROM pengelompokan_ubah_ijin_produksi GROUP BY id_pengelompokan_ubah_ijin_produksi,perubahan';
                $value_arr['id_pengelompokan_ubah_ijin_produksi'] = xform::dbs($arr);


		$label_arr['submit_val'] = "Submit";
		$label_arr['form_extra']  = "<input type=hidden name=action     value='post$mode'>"; // default null
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_cek_2']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek Kelengkapan Perubahan Izin Produksi";
	...
	// create update cek_2_alkes form
	function update_cek_2_alkes_form() {
		return $this->add_cek_2_alkes_form();
	}

	function view_cek_2_alkes_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = cek_2_alkes::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_cek_2'] = 'protect';

		$record = array (
			'id_cek_2' => ${$GLOBALS['get_vars']}['id_cek_2']
		);
		$result = cek_2_alkes::get($record);
		$value_arr = $result[0];
		$label_arr = $this->cek_2_alkes_label;
		global $adodb;
		...
		$pdf->Cell(180,7,'UBAH KELENGKAPAN IZIN ALKES','',0,'C');
		$pdf->Ln(5);
		...
		date_4,
		keterangan,
		CASE WHEN status_sk ='0' THEN 'Tidak Perlu'
				else 'Perlu'
		END AS status_sk,
		insert_by,
		date_insert
		...
		while(! $hasil->EOF){
			...
			$detail_perubahan = $hasil->fields['detail_perubahan'];
			...
			$status_sk = $hasil->fields['status_sk'];
			...
			$pdf->Ln(5);*/
			$pdf->Cell(50,7,'Jenis Produksi Alkes','',0,'L');$pdf->Cell(50,7,': '.$detail_perubahan.'','',0,'L');
			$pdf->Ln(5);
			...
			CASE WHEN detail_cek_2_alkes.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_2_alkes.alasan_penilai,
			CASE WHEN detail_cek_2_alkes.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_2_alkes.alasan_kaseksi,
			CASE WHEN detail_cek_2_alkes.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_2_alkes.alasan_kasubdit,
			CASE WHEN detail_cek_2_alkes.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_2_alkes.alasan_direktur
			FROM
			detail_cek_2_alkes
			LEFT OUTER JOIN kelengkapan_2_alkes ON(kelengkapan_2_alkes.id_kelengkapan = detail_cek_2_alkes.id_kelengkapan)
			WHERE
			id_cek_2='".$id_cek_2."' 
			";
		...
		$pdf->Ln(5);
		$pdf->Cell(50,7,'Keterangan','',0,'L');$pdf->Cell(50,7,': '.$keterangan.' ','',0,'L');
		$pdf->Ln(5);
		$pdf->Cell(50,7,'Perlu SK Baru','',0,'L');$pdf->Cell(50,7,': '.$status_sk.'','',0,'L');
		$pdf->Ln(5);
		$pdf->Cell(50,7,'No SK','',0,'L');$pdf->Cell(50,7,': ','',0,'L');
		$pdf->Ln(5);
	...
	// handle event add cek_2_alkes
	function add_cek_2_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($read_direktur == 'on'){ $xread_direktur = '1';}else{ $xread_direktur ='0';}
			...
			$record = array (
				...
				'no_tt' => $_POST['no_tt'],
				'id_pengelompokan_ubah_ijin_produksi' => $_POST['id_pengelompokan_ubah_ijin_produksi'],
				'detail_perubahan' => $_POST['detail_perubahan'],
		...
		cek_2_alkes::add($record);
		
		//$adodb->Execute("DELETE FROM detail_cek_2_alkes WHERE id_cek_2 = '".$_POST['oldpkvalue']."'");
		$rsx = $adodb->Execute("SELECT id_cek_2 FROM cek_2_alkes ORDER BY id_cek_2 DESC LIMIT 1");
		$id_cek_2 = $rsx->fields['id_cek_2'];

		include "class.detail_cek_2_alkes.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_2_alkes::add($record);
		}

		$status = "Successfull $st '<b>{$record['id_cek_2']}</b>'";
		$this->log($status);
	...
	// handle event update cek_2_alkes
	function update_cek_2_alkes() {
		//return $this->add_cek_2_alkes();
		...
		$record = array (
			'id_cek_2' => ${$GLOBALS['post_vars']}['oldpkvalue']
		);
		$_block = new block();
		$_block->set_config('title', ('Update Cek Kelengkapan Perubahan Izin Produksi Status'));
		$_block->set_config('width', 595);
		if ($result = cek_2_alkes::get($record)) {
			$record = $result[0];
			if (${$GLOBALS['post_vars']}['id_golongan']!='') $record['id_golongan'] = $_POST['id_golongan'];
			if (${$GLOBALS['post_vars']}['no_tt']!='') $record['no_tt'] = $_POST['no_tt'];
			if (${$GLOBALS['post_vars']}['id_pengelompokan_ubah_ijin_produksi']!='') $record['id_pengelompokan_ubah_ijin_produksi'] = $_POST['id_pengelompokan_ubah_ijin_produksi'];
			if (${$GLOBALS['post_vars']}['detail_perubahan']!='') $record['detail_perubahan'] = $_POST['detail_perubahan'];
			...
			if (${$GLOBALS['post_vars']}['status_sk']!='') $record['status_sk'] = $_POST['status_sk'];
			...
			eval($this->save_config);
			if (cek_2_alkes::update($record)) {

		$adodb->Execute("DELETE FROM detail_cek_2_alkes WHERE id_cek_2 = '".$_POST['oldpkvalue']."'");

		//$rsx = $adodb->Execute("SELECT id_cek_2 FROM cek_2_alkes ORDER BY id_cek_2 DESC LIMIT 1");
		$id_cek_2 = $_POST['oldpkvalue'];
	...
	// handle delete cek_2_alkes
	function delete_cek_2_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
					...
					$optional_arr['date_4'] = TRUE;
					$optional_arr['status_direktur'] = TRUE;
                        $success = $adodb->Execute("delete from cek_2_alkes where ".$query);
			$success = $adodb->Execute("delete from detail_cek_2_alkes where ".$query);
		}
                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'cek_2_alkes <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek Kelengkapan Perubahan Izin Produksi Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list cek_2_alkes
	function list_cek_2_alkes($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
			$sql = "SELECT
			cek_2_alkes.id_cek_2,
			cek_2_alkes.no_tt,
			cek_2_alkes.no_tt as urut_no_tt,
			CASE WHEN tt_2_alkes.jenis_izin_produksi='0' THEN 'Izin Produksi Alkes'
			ELSE
			'Izin Produksi PKRT' END as jenis_izin_produksi,
			cek_2_alkes.no_tt as alamat,
			cek_2_alkes.no_tt as subdit,
			pengelompokan_ubah_ijin_produksi.perubahan,
			cek_2_alkes.detail_perubahan,
			cek_2_alkes.id_golongan as golongan,
			CASE WHEN cek_2_alkes.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_2_alkes.nama_penilai,
			cek_2_alkes.date_1,
			CASE WHEN cek_2_alkes.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_2_alkes.nama_kaseksi,
			cek_2_alkes.date_2,
			CASE WHEN cek_2_alkes.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_2_alkes.nama_kasubdit,
			cek_2_alkes.date_3,
			CASE WHEN cek_2_alkes.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_2_alkes.nama_direktur,
			cek_2_alkes.date_4,
			CASE WHEN cek_2_alkes.status_sk = '0' THEN 'Tidak Perlu'
					else 'Perlu'
			END AS status_sk,
			cek_2_alkes.keterangan,
			cek_2_alkes.insert_by,
			cek_2_alkes.date_insert,
			cek_2_alkes.id_cek_2 as detail
			FROM cek_2_alkes
			LEFT OUTER JOIN pengelompokan_ubah_ijin_produksi ON(pengelompokan_ubah_ijin_produksi.id_pengelompokan_ubah_ijin_produksi = cek_2_alkes.id_pengelompokan_ubah_ijin_produksi)
			LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
			WHERE
			nama_penilai ='' OR nama_penilai = '$user'
			GROUP BY
			cek_2_alkes.id_cek_2,
			cek_2_alkes.no_tt,
			...
			pengelompokan_ubah_ijin_produksi.perubahan,
			cek_2_alkes.detail_perubahan,
			cek_2_alkes.id_golongan,
			cek_2_alkes.status_penilai,
			cek_2_alkes.nama_penilai,
			cek_2_alkes.date_1,
			cek_2_alkes.status_kaseksi,
			cek_2_alkes.nama_kaseksi,
			cek_2_alkes.date_2,
			cek_2_alkes.status_kasubdit,
			cek_2_alkes.nama_kasubdit,
			cek_2_alkes.date_3,
			cek_2_alkes.status_direktur,
			cek_2_alkes.nama_direktur,
			cek_2_alkes.date_4,
			cek_2_alkes.status_sk,
			cek_2_alkes.keterangan,
			cek_2_alkes.insert_by,
			cek_2_alkes.date_insert,
			cek_2_alkes.indi_kaseksi,
			cek_2_alkes.read_kaseksi,
			cek_2_alkes.indi_kasubdit,
			cek_2_alkes.read_kasubdit,
			cek_2_alkes.indi_direktur,
			cek_2_alkes.read_direktur
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_2_alkes.insert_by,
			cek_2_alkes.date_insert,
			CASE WHEN cek_2_alkes.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_2_alkes.read_kaseksi='0' THEN 'Unread'
			...
			LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
			WHERE
			nama_kaseksi ='' OR nama_kaseksi = '$user'
  			GROUP BY
			cek_2_alkes.id_cek_2,
			cek_2_alkes.no_tt,
			pengelompokan_ubah_ijin_produksi.perubahan,
		...
		if($bag =='kasubdit'){
			...
			cek_2_alkes.insert_by,
			cek_2_alkes.date_insert,
			CASE WHEN cek_2_alkes.indi_kasubdit='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_2_alkes.read_kasubdit='0' THEN 'Unread'
			...
			LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
			WHERE
			cek_2_alkes.nama_kasubdit ='' OR cek_2_alkes.nama_kasubdit = '$user'
			GROUP BY
			cek_2_alkes.id_cek_2,
			cek_2_alkes.no_tt,
			pengelompokan_ubah_ijin_produksi.perubahan,
		...
		if($bag =='direktur'){
			...
			cek_2_alkes.insert_by,
			cek_2_alkes.date_insert,
			CASE WHEN cek_2_alkes.indi_direktur='0' THEN 'Return'
			else 'OK' END AS indi_direktur,
			Case WHEN cek_2_alkes.read_direktur='0' THEN 'Unread'
			...
			LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
			WHERE
			nama_direktur ='' OR nama_direktur = '$user'
			GROUP BY
			cek_2_alkes.id_cek_2,
			cek_2_alkes.no_tt,
			pengelompokan_ubah_ijin_produksi.perubahan,
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_2_alkes WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_2_alkes WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_2_alkes WHERE no_tt=\'$f\'");
		...
		$USql = "
			SELECT
			kelengkapan_2_alkes.nama_kelengkapan,
			CASE WHEN detail_cek_2_alkes.status_penilai =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_penilai,
			detail_cek_2_alkes.alasan_penilai,
			CASE WHEN detail_cek_2_alkes.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_2_alkes.alasan_kaseksi,
			CASE WHEN detail_cek_2_alkes.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_2_alkes.alasan_kasubdit,
			CASE WHEN detail_cek_2_alkes.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_2_alkes.alasan_direktur
			FROM
			detail_cek_2_alkes
			LEFT OUTER JOIN kelengkapan_2_alkes ON(kelengkapan_2_alkes.id_kelengkapan = detail_cek_2_alkes.id_kelengkapan)
			WHERE
			id_cek_2=\'%s\' AND detail_cek_2_alkes.alasan_kaseksi IS NOT NULL
			";
			global $adodb;
...
//		if ($this->get_permission('fill_this')) {
		if($bag=='kaseksi'){
			...
			'?action=add' .
			'\', 800, 600, null, null, \'add_cek_2_alkes\');'.
			'win.focus()',
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 800, 600, null, null, \'edit_cek_2_alkes\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_cek_2_alkes\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_cek_2_alkes\');'.
			'win.focus()',
			"label"=>__('Print'),
			"type"=>"button",
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek Kelengkapan Perubahan Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print cek_2_alkes
	function print_cek_2_alkes() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_cek_2_alkes($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$cek_2_alkes_controller = new cek_2_alkes_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_2_alkes_controller->add_cek_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $cek_2_alkes_controller->add_cek_2_alkes();
			//$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangeno_tt()"';
			$out_content = $cek_2_alkes_controller->update_cek_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $cek_2_alkes_controller->update_cek_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $cek_2_alkes_controller->view_cek_2_alkes_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $cek_2_alkes_controller->delete_cek_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_2_alkes_controller->import_cek_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $cek_2_alkes_controller->import_cek_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $cek_2_alkes_controller->print_cek_2_alkes();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $cek_2_alkes_controller->list_cek_2_alkes();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Cek Kelengkapan Perubahan Izin Produksi Administration';
