...
if (class_exists('cek_pemutih_alkes_controller')) {
	// do nothing
} else if (defined('CLASS_cek_pemutih_alkes_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_cek_pemutih_alkes_CONTROLLER', TRUE);

include_once 'class.cek_pemutih_alkes.inc.php';
class cek_pemutih_alkes_controller extends depkes2_manager {
	var $cek_pemutih_alkes_label;
	var $optional_arr;
	function cek_pemutih_alkes_controller() {
		$this->cek_pemutih_alkes_label = array (
			'id_cek_pemutih' => 'Id Check Alkes',
			...
			'no_rekomendasi' => 'No Keputusan Lama',
			'date_rekomendasi' => 'Tgl Keputusan Lama',
	...
	function no_tt_form(&$config) {
		...
		if($_GET['id_cek_pemutih']){
			if($bag=='kaseksi'){
				$fk_sql =
				"SELECT
					tt_pemutih_alkes.no_tt as skey,
				...
				LEFT OUTER JOIN inbox_pemutih_produksi_subdit ON(inbox_pemutih_produksi_subdit.no_tt = tt_pemutih_alkes.no_tt )
				RIGHT OUTER JOIN inbox_pemutih_produksi_seksi ON(inbox_pemutih_produksi_seksi.id_inbox_pemutih_produksi_subdit = inbox_SELECT
					tt_pemutih_alkes.no_tt as skey,
					tt_pemutih_alkes.urut_no_tt as svalue,
					pendaftar_pemutih.nama_pabrik as svalue2
				FROM
				...
				LEFT OUTER JOIN pendaftar_pemutih ON(pendaftar_pemutih.kode_pendaftar_pemutih = tt_pemutih_alkes.kode_pendaftar_pemutih )
				LEFT OUTER JOIN inbox_pemutih_produksi_subdit ON(inbox_pemutih_produksi_subdit.no_tt = tt_pemutih_alkes.no_tt )
				RIGHT OUTER JOIN inbox_pemutih_produksi_seksi ON(inbox_pemutih_produksi_seksi.id_inbox_pemutih_produksi_subdit = inbox_produksi_subdit.id_inbox_pemutih_produksi_subdit)pemutih_produksi_subdit.id_inbox_pemutih_produksi_subdit)
				WHERE inbox_pemutih_produksi_seksi.insert_by='$userlog'
				ORDER BY
				...
				LEFT OUTER JOIN pendaftar_pemutih ON(pendaftar_pemutih.kode_pendaftar_pemutih = tt_pemutih_alkes.kode_pendaftar_pemutih )
				LEFT OUTER JOIN inbox_pemutih_produksi_subdit ON(inbox_pemutih_produksi_subdit.no_tt = tt_pemutih_alkes.no_tt )
				RIGHT OUTER JOIN inbox_pemutih_produksi_seksi ON(inbox_pemutih_produksi_seksi.id_inbox_pemutih_produksi_subdit = inbox_pemutih_produksi_subdit.id_inbox_pemutih_produksi_subdit)
				ORDER BY
				tt_pemutih_alkes.date_insert
				";
			}
			...
			LEFT OUTER JOIN inbox_pemutih_produksi_subdit ON(inbox_pemutih_produksi_subdit.no_tt = tt_pemutih_alkes.no_tt )
			RIGHT OUTER JOIN inbox_pemutih_produksi_seksi ON(inbox_pemutih_produksi_seksi.id_inbox_pemutih_produksi_subdit = inbox_pemutih_produksi_subdit.id_inbox_pemutih_produksi_subdit)
			WHERE ((tt_pemutih_alkes.no_tt IS NULL) OR (inbox_pemutih_produksi_seksi.insert_by='$userlog')) AND cek_pemutih_alkes.no_tt IS NULL
			ORDER BY
			tt_pemutih_alkes.date_insert
			";
		}
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_pemutih.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_pemutih LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.kode_pendaftar_pemutih = pendaftar_pemutih.kode_pendaftar_pemutih) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_pemutih_alkes.kode_subdit)  WHERE tt_pemutih_alkes.no_tt =\''+ b +'\'';
	...
	// create add cek_pemutih_alkes form
	function add_cek_pemutih_alkes_form() {
		include_once 'class.xform.inc.php';
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		//print_r($_GET);exit();
		$record = $_GET;
		$label_arr = $this->cek_pemutih_alkes_label;
		...
		$field_arr[] = xform::xf('kelengkapan_pemutih','C','32');
		...
		$rs = $adodb->Execute("SELECT * FROM cek_pemutih_alkes WHERE id_cek_pemutih='{$record['id_cek_pemutih']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$optional_arr['kelengkapan_pemutih_rule'] = '';
		...
		$has ='';
		$has .='<table>';
		if($ses->action == 'penilai'){$has .= '<tr><td>No</td><td>Nama kelengkapan_pemutih</td><td>Status Penilai</td><td>Komentar Penilai</td></tr>';}
		if($ses->action == 'kaseksi'){$has .= '<tr><td>No</td><td>Nama kelengkapan_pemutih</td><td>Komentar Penilai</td><td>Status Kaseksi</td><td>Komentar Kaseksi</td></tr>';}
		if($ses->action == 'kasubdit'){$has .= '<tr><td>No</td><td>Nama kelengkapan_pemutih</td><td>Komentar Penilai</td><td>Komentar Kaseksi</td><td>Status Kasubdit</td><td>Komentar Kasubdit</td></tr>';}
		if($ses->action == 'direktur'){$has .= '<tr><td>No</td><td>Nama kelengkapan_pemutih</td><td>Komentar Penilai</td><td>Komentar Kaseksi</td><td>Komentar Kasubdit</td><td>Status Direktur</td><td>Komentar Direktur</td></tr>';}
		if($ses->action == 'admin'){$has .= '<tr><td>No</td><td>Nama kelengkapan_pemutih</td><td>Komentar Penilai</td><td>Komentar Kaseksi</td><td>Komentar Kasubdit</td><td>Komentar Direktur</td></tr>';}
		global $adodb;
			$sqlx = "SELECT id_kelengkapan_pemutih,nama_kelengkapan_pemutih FROM kelengkapan_pemutih_alkes ORDER BY id_kelengkapan_pemutih";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){
				if($_GET['id_cek_pemutih']){
				$sqld = "SELECT id_cek_pemutih,id_kelengkapan_pemutih,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_pemutih_alkes WHERE id_cek_pemutih='".$_GET['id_cek_pemutih']."' AND id_kelengkapan_pemutih='".$rsx->fields['id_kelengkapan_pemutih']."'";
				$rsd = $adodb->Execute($sqld);
				$id_cek_pemutihx = $rsd->fields['id_cek_pemutih'];
				$id_kelengkapan_pemutihx = $rsd->fields['id_kelengkapan_pemutih'];
				...
				$alasan_direkturx = $rsd->fields['alasan_direktur'];
					if($id_kelengkapan_pemutihx == $rsx->fields['id_kelengkapan_pemutih']){
				...
				$id_kelengkapan_pemutih = $rsx->fields['id_kelengkapan_pemutih'];
				$nama_kelengkapan_pemutih = $rsx->fields['nama_kelengkapan_pemutih'];
				$has .= "<tr><td>".$a."<input type=hidden name=id_kelengkapan_pemutih[".$a."] value='".$id_kelengkapan_pemutih."'</td><td>".$nama_kelengkapan_pemutih."</td>";

				if($ses->action == 'penilai'){

				$has .= "<td><input type=checkbox name=status_penilaix[".$a."] value=1 class=text ".$check1."></td></td><td><textarea name=alasan_penilaix[".$a."] class='text'>".$alasan_penilaix."</textarea>
		...
		$optional_arr['kelengkapan_pemutih']='user_defined';
		$label_arr['kelengkapan_pemutih']='kelengkapan_pemutih';
		$value_arr['kelengkapan_pemutih'] = $has;
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek Kelengkapan Pemutihan Izin Produksi";
	...
	// create update cek_pemutih_alkes form
	function update_cek_pemutih_alkes_form() {
		return $this->add_cek_pemutih_alkes_form();
	}

	function view_cek_pemutih_alkes_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = cek_pemutih_alkes::get_field_set();
		...
		$result = cek_pemutih_alkes::get($record);
		$value_arr = $result[0];
		$label_arr = $this->cek_pemutih_alkes_label;
		global $adodb;
		...
		$pdf->Cell(180,7,'KELENGKAPAN PEMUTIHAN IZIN ALKES','',0,'C');
		$pdf->Ln(5);
		...
		insert_by,
		date_insert
		FROM cek_pemutih_alkes
		WHERE
		id_cek_pemutih ='".$_GET['id_cek_pemutih']."'
		";
//print_r($sqlA);
		...
		while(! $hasil->EOF){
			...
			FROM
			pendaftar_pemutih
			LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.kode_pendaftar_pemutih = pendaftar_pemutih.kode_pendaftar_pemutih)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_pemutih_alkes.kode_subdit)
			WHERE tt_pemutih_alkes.no_tt ='".$no_tt."'
			";
			...
			$pdf->Ln(5);
			$pdf->Cell(50,7,'No Keputusan Lama','',0,'L');$pdf->Cell(50,7,': '.$no_rekomendasi.'','',0,'L');
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Tanggal Keputusan Lama','',0,'L');$pdf->Cell(50,7,': '.$date_rekomendasi.'','',0,'L');
			$pdf->Ln(5);
			...
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Jenis Produksi Alkes','',0,'L');$pdf->Cell(50,7,': '.$nama_produk.'','',0,'L');
			$pdf->Ln(5);
			...
			$pdf->Cell(35,7,'Kel. Pemutih','LRB',0,'C');
			...
			$sqlD = "
			SELECT
			kelengkapan_pemutih_alkes.nama_kelengkapan_pemutih,
			CASE WHEN detail_cek_pemutih_alkes.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_pemutih_alkes.alasan_penilai,
			CASE WHEN detail_cek_pemutih_alkes.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_pemutih_alkes.alasan_kaseksi,
			CASE WHEN detail_cek_pemutih_alkes.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_pemutih_alkes.alasan_kasubdit,
			CASE WHEN detail_cek_pemutih_alkes.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_pemutih_alkes.alasan_direktur
			FROM
			detail_cek_pemutih_alkes
			LEFT OUTER JOIN kelengkapan_pemutih_alkes ON(kelengkapan_pemutih_alkes.id_kelengkapan_pemutih = detail_cek_pemutih_alkes.id_kelengkapan_pemutih)
			WHERE
			...
			while(! $hasilD->EOF){
				...
				$pdf->Cell(35,3,'','TRL',0,'L');
				$pdf->Ln();
				$nama_kelengkapan_pemutih = $hasilD->fields['nama_kelengkapan_pemutih'];
				...
				$pdf->SetFont('Arial','',8);
					...
					$jml_nama_kelengkapan_pemutih = strlen($nama_kelengkapan_pemutih);
					...
					$max = max($jml_nama_kelengkapan_pemutih,$jml_alasan_penilaix,$jml_alasan_kaseksix,$jml_alasan_kasubditx,$jml_alasan_direkturx);
					...
					for($a=0;$a<=$max;$a=$a+1){
						...
						if(($g == 0)&&($a != 0)){
							...
							$nama_kelengkapan_pemutihY .= $nama_kelengkapan_pemutih[$a];
							...
							$pdf->Cell(35,3,''.$nama_kelengkapan_pemutihY.'','LR',0,'L');
	...
	// handle event add cek_pemutih_alkes
	function add_cek_pemutih_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		//foreach ($record as $k => $v) $record[$k] = trim($v);
		//print_r($_POST);exit();
		$jml_kelengkapan_pemutih = count($_POST['id_kelengkapan_pemutih']);
		...
		if($read_direktur == 'on'){ $xread_direktur = '1';}else{ $xread_direktur ='0';}
			...
			$record = array (
				...
				'status_penila1856i' => $_POST['status_penilai'],
		...
		cek_pemutih_alkes::add($record);

		$rsx = $adodb->Execute("SELECT id_cek_pemutih FROM cek_pemutih_alkes ORDER BY id_cek_pemutih DESC LIMIT 1");
		$id_cek_pemutih = $rsx->fields['id_cek_pemutih'];

		include "class.detail_cek_pemutih_alkes.inc.php";
		for($a=1;$a<=$jml_kelengkapan_pemutih;$a++){
			$id_kelengkapan_pemutihx = $_POST['id_kelengkapan_pemutih'][$a];
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				$record = array (
					'id_cek_pemutih' => $id_cek_pemutih,
					'id_kelengkapan_pemutih' => $id_kelengkapan_pemutihx,
			...
			detail_cek_pemutih_alkes::add($record);
		}
		...
		$_block = new block();
		$_block->set_config('title', 'Kelengkapan Pemutihan Izin Produksi Status');
		$_block->set_config('width', "90%");
		$_block->parse(array("*".$status));
		return $_block->get_str();
	}

	// handle event update cek_pemutih_alkes
	function update_cek_pemutih_alkes() {
		//return $this->add_cek_pemutih_alkes();
		...
		$record = array (
			'id_cek_pemutih' => ${$GLOBALS['post_vars']}['oldpkvalue']
		);
		$_block = new block();
		$_block->set_config('title', ('Update Cek Kelengkapan Pemutihan Izin Produksi Status'));
		$_block->set_config('width', 595);
		if ($result = cek_pemutih_alkes::get($record)) {
			$record = $result[0];
			...
			eval($this->save_config);
			if (cek_pemutih_alkes::update($record)) {

		//$adodb->Execute("DELETE FROM detail_cek_pemutih_alkes WHERE id_cek_pemutih = '".$_POST['oldpkvalue']."'");
		//$rsx = $adodb->Execute("SELECT id_cek_pemutih FROM cek_pemutih_alkes ORDER BY id_cek_pemutih DESC LIMIT 1");
		$id_cek_pemutih = $_POST['oldpkvalue'];
	...
	// handle delete cek_pemutih_alkes
	function delete_cek_pemutih_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
					...
					$optional_arr['date_4'] = TRUE;
					$optional_arr['status_direktur'] = TRUE;
                        $success = $adodb->Execute("delete from cek_pemutih_alkes where ".$query);
			$success = $adodb->Execute("delete from detail_cek_pemutih_alkes where ".$query);
		}
                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'cek_pemutih_alkes <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek Kelengkapan Pemutihan Izin Produksi Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list cek_pemutih_alkes
	function list_cek_pemutih_alkes($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
		//$sqlk = "SELECT id_Cek_pemutih FROM cek_pemutih_alkes WHERE nama_penilai
			$sql = "SELECT
			cek_pemutih_alkes.id_cek_pemutih,
			cek_pemutih_alkes.no_tt,
			cek_pemutih_alkes.no_tt as urut_no_tt,
			CASE WHEN tt_pemutih_alkes.jenis_izin_produksi='0' THEN 'Izin Produksi Alkes'
			ELSE
			'Izin Produksi PKRT' END as jenis_izin_produksi,
			cek_pemutih_alkes.no_tt as alamat,
			cek_pemutih_alkes.no_tt as subdit,
			...
			cek_pemutih_alkes.id_golongan as golongan,
			CASE WHEN cek_pemutih_alkes.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_pemutih_alkes.nama_penilai,
			cek_pemutih_alkes.date_1,
			CASE WHEN cek_pemutih_alkes.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_pemutih_alkes.nama_kaseksi,
			cek_pemutih_alkes.date_2,
			CASE WHEN cek_pemutih_alkes.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_pemutih_alkes.nama_kasubdit,
			cek_pemutih_alkes.date_3,
			CASE WHEN cek_pemutih_alkes.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_pemutih_alkes.nama_direktur,
			cek_pemutih_alkes.date_4,
			cek_pemutih_alkes.keterangan,
			cek_pemutih_alkes.insert_by,
			cek_pemutih_alkes.date_insert,
			cek_pemutih_alkes.id_cek_pemutih as detail
			FROM cek_pemutih_alkes
			LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.no_tt = cek_pemutih_alkes.no_tt)
			WHERE
			cek_pemutih_alkes.nama_penilai ='' OR cek_pemutih_alkes.nama_penilai = '$user'
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_pemutih_alkes.insert_by,
			cek_pemutih_alkes.date_insert,
			CASE WHEN cek_pemutih_alkes.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_pemutih_alkes.read_kaseksi='0' THEN 'Unread'
			else 'Read' END AS read_kaseksi,
			cek_pemutih_alkes.id_cek_pemutih as detail
			FROM cek_pemutih_alkes
			LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.no_tt = cek_pemutih_alkes.no_tt)
			WHERE
			cek_pemutih_alkes.nama_kaseksi ='' OR cek_pemutih_alkes.nama_kaseksi = '$user'
			ORDER BY cek_pemutih_alkes.read_kaseksi
			";
		}
		...
		if($bag =='kasubdit'){
			...
			cek_pemutih_alkes.insert_by,
			cek_pemutih_alkes.date_insert,
			CASE WHEN cek_pemutih_alkes.indi_kasubdit ='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_pemutih_alkes.read_kasubdit='0' THEN 'Unread'
			else 'Read' END AS read_kasubdit,
			cek_pemutih_alkes.id_cek_pemutih as detail
			FROM cek_pemutih_alkes
			LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.no_tt = cek_pemutih_alkes.no_tt)
			WHERE
			cek_pemutih_alkes.nama_kasubdit ='' OR cek_pemutih_alkes.nama_kasubdit = '$user'
			ORDER BY cek_pemutih_alkes.read_kasubdit
			";
		}
		...
		if($bag =='direktur'){
			...
			cek_pemutih_alkes.insert_by,
			cek_pemutih_alkes.date_insert,
			CASE WHEN cek_pemutih_alkes.indi_direktur ='0' THEN 'Return'
			else 'OK' END AS indi_direktur,
			Case WHEN cek_pemutih_alkes.read_direktur='0' THEN 'Unread'
			else 'Read' END AS read_direktur,
			cek_pemutih_alkes.id_cek_pemutih as detail
			FROM cek_pemutih_alkes
			LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.no_tt = cek_pemutih_alkes.no_tt)
			WHERE
			cek_pemutih_alkes.nama_direktur ='' OR cek_pemutih_alkes.nama_direktur = '$user'
			ORDER BY cek_pemutih_alkes.read_direktur
			";
		}
		...
		LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.kode_pendaftar_pemutih = pendaftar_pemutih.kode_pendaftar_pemutih)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_pemutih_alkes.kode_subdit)
		WHERE tt_pemutih_alkes.no_tt =\'$f\'";
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_pemutih_alkes WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_pemutih_alkes WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_pemutih_alkes WHERE no_tt=\'$f\'");
		...
		LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.kode_pendaftar_pemutih = pendaftar_pemutih.kode_pendaftar_pemutih)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_pemutih_alkes.kode_subdit)
		WHERE tt_pemutih_alkes.no_tt =\'$b\'";
		...
		LEFT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.kode_pendaftar_pemutih = pendaftar_pemutih.kode_pendaftar_pemutih)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_pemutih_alkes.kode_subdit)
		WHERE tt_pemutih_alkes.no_tt =\'$c\'";
...
//		if ($this->get_permission('fill_this')) {
		if($bag=='kaseksi'){
			...
			'?action=add' .
			'\', 800, 600, null, null, \'add_cek_pemutih_alkes\');'.
			'win.focus()',
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 800, 600, null, null, \'edit_cek_pemutih_alkes\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_cek_pemutih_alkes\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_cek_pemutih_alkes\');'.
			'win.focus()',
			"label"=>__('Print'),
			"type"=>"button",
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek Kelengkapan Pemutihan Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print cek_pemutih_alkes
	function print_cek_pemutih_alkes() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_cek_pemutih_alkes($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$cek_pemutih_alkes_controller = new cek_pemutih_alkes_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_pemutih_alkes_controller->add_cek_pemutih_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $cek_pemutih_alkes_controller->add_cek_pemutih_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangeno_tt()"';
			$out_content = $cek_pemutih_alkes_controller->update_cek_pemutih_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $cek_pemutih_alkes_controller->update_cek_pemutih_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $cek_pemutih_alkes_controller->view_cek_pemutih_alkes_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $cek_pemutih_alkes_controller->delete_cek_pemutih_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_pemutih_alkes_controller->import_cek_pemutih_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $cek_pemutih_alkes_controller->import_cek_pemutih_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $cek_pemutih_alkes_controller->print_cek_pemutih_alkes();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $cek_pemutih_alkes_controller->list_cek_pemutih_alkes();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Cek Kelengkapan Pemutihan Izin Produksi Administration';
