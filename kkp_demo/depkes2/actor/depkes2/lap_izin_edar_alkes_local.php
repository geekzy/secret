...
class cek_kelengkapan_data_edar_local_controller extends depkes2_manager {
	...
	function cek_kelengkapan_data_edar_local_controller() {
		$this->cek_kelengkapan_data_edar_local_label = array (
			...
			'nama_pendaftar_edar_alkes_local' => 'Nama Pendaftar',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_1_edar_alkes_local.no_tt as svalue,
  ...
  $jas ="
		...
		jumpto1('frame_tt_1_edar_alkes_local.php?sql='+sql+'&a='+b)

		";

	$GLOBALS['out_before_footer'] = '
 	<iframe marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=auto name=\'iframe_entry1\' id=\'iframe_entry1\' style="width:0;height:0" src="frame_tt_1_edar_alkes_local.php"></iframe>
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_edar_local WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Edar Alkes Lokal";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_local WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		$adodb->Execute("DELETE FROM cek_kelengkapan_data_edar_local WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Edar Alkes Lokal');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_kelengkapan_data_edar_local where ".$query);
		}
                if ($success){
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Edar Alkes Lokal'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_edar_alkes_local.nama_pabrik,
		tt_1_edar_alkes_local.no_tt as status,
		tt_1_edar_alkes_local.no_tt as status_pending,
		tt_1_edar_alkes_local.no_tt as status_tolak,
		tt_1_edar_alkes_local.no_tt as status_selesai,
		...
		FROM
		surat_keputusan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_keputusan.id_cek_1)
		WHERE
		...
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_penolakan.id_cek_1)
		WHERE
		cek_kelengkapan_data_edar_local.no_tt =\'$f\'";
		...
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_kelengkapan_data_edar_local ON(cek_kelengkapan_data_edar_local.id_cek_1 = surat_tambahan_data.kepada_surat)
		WHERE
		cek_kelengkapan_data_edar_local.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_edar_alkes_local WHERE no_tt=\'$f\'");
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Edar Alkes Lokal'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Izin Edar Alkes Lokal';
