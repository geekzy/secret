...
if (class_exists('pendaftar_edar_alkes_local_controller')) {
	// do nothing
} else if (defined('CLASS_pendaftar_edar_alkes_local_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_pendaftar_edar_alkes_local_CONTROLLER', TRUE);

include_once 'class.pendaftar_edar_alkes_local.inc.php';
class pendaftar_edar_alkes_local_controller extends depkes2_manager {
	var $pendaftar_edar_alkes_local_label;
	var $optional_arr;
	function pendaftar_edar_alkes_local_controller() {
		$this->pendaftar_edar_alkes_local_label = array (
			'kode_pendaftar_edar_alkes_local' => 'Kode Pemohon',
			'nama_pendaftar_edar_alkes_local' => 'Nama Pemohon',
			'alamat_pendaftar_edar_alkes_local' => 'Kota Pemohon',
			'notelp_1' => 'No Telp',
			'npwp' => 'NPWP',
		...
		$this->optional_arr = array (
			'kode_pendaftar_edar_alkes_local' => FALSE,
			'nama_pendaftar_edar_alkes_local' => FALSE,
			'insert_by' => TRUE,
	...
	function nama_propinsi_1_form(&$config) {
		...
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->pendaftar_edar_alkes_local_label['nama_propinsi_1']
			)
		);
	...
	function nama_propinsi_2_form(&$config) {
		...
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->pendaftar_edar_alkes_local_label['nama_propinsi_2']
			)
		);
		$result = array_merge($default_value, $result);
	...
	// create add pendaftar_edar_alkes_local form
	function add_pendaftar_edar_alkes_local_form() {
		include_once 'class.xform.inc.php';

//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->pendaftar_edar_alkes_local_label;
		$optional_arr = $this->optional_arr;

		$field_arr[] = xform::xf('kode_pendaftar_edar_alkes_local','C','255');
		$field_arr[] = xform::xf('nama_pendaftar_edar_alkes_local','C','255');
		$field_arr[] = xform::xf('npwp','C','255');
		$field_arr[] = xform::xf('alamat_pendaftar_edar_alkes_local','C','255');
		...
		$rs = $adodb->Execute("SELECT * FROM pendaftar_edar_alkes_local WHERE kode_pendaftar_edar_alkes_local='{$record['kode_pendaftar_edar_alkes_local']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['kode_pendaftar_edar_alkes_local'] = 'protect';
			$mode = 'edit';
		...
		$optional_arr['kode_pendaftar_edar_alkes_local_rule'] = '';
		$optional_arr['nama_pendaftar_edar_alkes_local_rule'] = '';
		$optional_arr['alamat_pendaftar_edar_alkes_local_rule'] = '';
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['kode_pendaftar_edar_alkes_local']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Pemohon Edar Alkes Lokal";
	...
	// create update pendaftar_edar_alkes_local form
	function update_pendaftar_edar_alkes_local_form() {
		return $this->add_pendaftar_edar_alkes_local_form();
	}

	
	function local_pendaftar_edar_alkes_local_form() {
		$this->local_form();
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$label_arr['form_title'] = ('local Pemohon Edar Alkes Lokal'); // default null
	...
	function local_form() {
		...
		if ($_POST[template]) {
			...
			echo '"kode_pendaftar_edar_alkes_local","nama_pendaftar_edar_alkes_local","alamat_pendaftar_edar_alkes_local","notelp_1","nama_propinsi_1","nama_pabrik","alamat_pabrik","notelp_2","nama_propinsi_2","alamat_bengkel","notelp_3","alamatgudang","notelp_4","nama_direktur","namapenanggungjwb"';
			exit;
		}
		if ($_POST[local]) {
			$uploaddir = '/tmp/';
			...
			while ($data = fgetcsv ($fd, 8000, ",")) {
				$num = count ($data);
				$kode_pendaftar_edar_alkes_local = $data[0];
				$nama_pendaftar_edar_alkes_local = $data[1];
				$alamat_pendaftar_edar_alkes_local = $data[2];
				$notelp_1 = $data[3];
				...
				if ($kode_pendaftar_edar_alkes_local=='') continue;

				global $ses;
				global $adodb;

				$record[kode_pendaftar_edar_alkes_local] = $kode_pendaftar_edar_alkes_local;
				$record[nama_pendaftar_edar_alkes_local] = $nama_pendaftar_edar_alkes_local;
				$record[alamat_pendaftar_edar_alkes_local] = $alamat_pendaftar_edar_alkes_local;
				...
				$rs = $adodb->Execute("SELECT * FROM pendaftar_edar_alkes_local WHERE kode_pendaftar_edar_alkes_local = '".$kode_pendaftar_edar_alkes_local."'");
				if ($rs->fields[kode_pendaftar_edar_alkes_local]) {
				...
				$kode_pendaftar_edar_alkes_local = '';
				$nama_pendaftar_edar_alkes_local = '';
				$alamat_pendaftar_edar_alkes_local = '';
				$notelp_1 = '';
				$nama_propinsi_1 = '';
				$nama_pabrik = '';
				$alamat_pabrik = '';
...
<html>
<head>
<title>Lokal Label Izin Produksi</title>
</head>
<body>
<div align=center>
<form method=post>
...
<input type=submit name=local value='local'>
	...
	// handle event add pendaftar_edar_alkes_local
	function add_pendaftar_edar_alkes_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM pendaftar_edar_alkes_local WHERE kode_pendaftar_edar_alkes_local = '{$record['kode_pendaftar_edar_alkes_local']}'");
 		 if ($rs && ! $rs->EOF) {
			...
			$record['insert_by'] = $ses->loginkode_pendaftar_edar_alkes_local;
			$record['date_insert'] = time();
			$rs = $adodb->Execute("SELECT * FROM pendaftar_edar_alkes_local WHERE kode_pendaftar_edar_alkes_local = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		
		$status = "Successfull $st '<b>{$record['kode_pendaftar_edar_alkes_local']}</b>'";
		$this->log($status);
		
		$_block = new block();
		$_block->set_config('title', 'Pemohon Edar Alkes Lokal Status');
		$_block->set_config('Pemohon', "90%");
		$_block->parse(array("*".$status));
		return $_block->get_str();
	}
	
	// handle event update pendaftar_edar_alkes_local
	function update_pendaftar_edar_alkes_local() {
		return $this->add_pendaftar_edar_alkes_local();
	}

	// handle delete pendaftar_edar_alkes_local
	function delete_pendaftar_edar_alkes_local() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from pendaftar_edar_alkes_local where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'pendaftar_edar_alkes_local <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Pemohon Edar Alkes Lokal Status'));
                $_block->set_config('wkode_pendaftar_edar_alkes_localth', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list pendaftar_edar_alkes_local
	function list_pendaftar_edar_alkes_local($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $adodb, ${$GLOBALS['get_vars']};
		$sql = 'SELECT
		pendaftar_edar_alkes_local.kode_pendaftar_edar_alkes_local,
		...
		pendaftar_edar_alkes_local.notelp_1,
		pendaftar_edar_alkes_local.nama_propinsi_1,
		pendaftar_edar_alkes_local.nama_pabrik,
		pendaftar_edar_alkes_local.alamat_pabrik,
		pendaftar_edar_alkes_local.notelp_2,
		pendaftar_edar_alkes_local.nama_propinsi_2,
		pendaftar_edar_alkes_local.alamat_bengkel,
		pendaftar_edar_alkes_local.notelp_3,
		pendaftar_edar_alkes_local.alamatgudang,
		pendaftar_edar_alkes_local.notelp_4,
		pendaftar_edar_alkes_local.nama_direktur,
		pendaftar_edar_alkes_local.namapenanggungjwb,
		pendaftar_edar_alkes_local.userid,
		pendaftar_edar_alkes_local.passwd,
		pendaftar_edar_alkes_local.date_insert,
		pendaftar_edar_alkes_local.insert_by
		FROM pendaftar_edar_alkes_local
		LEFT OUTER JOIN propinsi ON(propinsi.id = pendaftar_edar_alkes_local.nama_propinsi_1)
		';
		...
		$vsel_arr = array (
			'kode_pendaftar_edar_alkes_local' => TRUE,
			'nama_pendaftar_edar_alkes_local' => TRUE,
			'insert_by' => FALSE,
			'date_insert' => FALSE
		);
		$eval_arr = array ();
...
//		if ($this->get_permission('fill_this')) {
			...
			'?action=add' . 
			'\', 600, 400, null, null, \'add_pendaftar_edar_alkes_local\');'.
			'win.focus()',
			"label"=>__("Add"),
			...
			'win=openIT(\''.$GLOBALS['PHP_SELF'] .
			'?action=local' .
			'\', 800, 600, null, null, \'local_pendaftar_edar_alkes_local\');'.
			'win.focus()',
			"title"=>__("local").' Pemohon Edar Alkes Lokal',
			"label"=>__("local"),
			"image"=>$GLOBALS['path_theme'].'/images/new.gif',
			"type"=>"button"));
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_pendaftar_edar_alkes_local\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_pendaftar_edar_alkes_local\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_pendaftar_edar_alkes_local\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$label_arr = $this->pendaftar_edar_alkes_local_label;
		$config = array (
			'kode_pendaftar_edar_alkes_local'		=> 'pendaftar_edar_alkes_local',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_wkode_pendaftar_edar_alkes_localth'	=> 595,
			'form_title'	=> __('List').' Pemohon Edar Alkes Lokal '.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print pendaftar_edar_alkes_local
	function print_pendaftar_edar_alkes_local() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_pendaftar_edar_alkes_local($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$pendaftar_edar_alkes_local_controller = new pendaftar_edar_alkes_local_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $pendaftar_edar_alkes_local_controller->add_pendaftar_edar_alkes_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $pendaftar_edar_alkes_local_controller->add_pendaftar_edar_alkes_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $pendaftar_edar_alkes_local_controller->update_pendaftar_edar_alkes_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $pendaftar_edar_alkes_local_controller->update_pendaftar_edar_alkes_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'del':
			$out_content = $pendaftar_edar_alkes_local_controller->delete_pendaftar_edar_alkes_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'local':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $pendaftar_edar_alkes_local_controller->local_pendaftar_edar_alkes_local_form();
			$out_content .= $back_to_menu;
			break;
		case 'postlocal':
			$out_content = $pendaftar_edar_alkes_local_controller->local_pendaftar_edar_alkes_local();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $pendaftar_edar_alkes_local_controller->print_pendaftar_edar_alkes_local();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $pendaftar_edar_alkes_local_controller->list_pendaftar_edar_alkes_local();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Pemohon Edar Alkes Lokal Administration';
