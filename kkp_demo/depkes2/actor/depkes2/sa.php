...
class sk_controller extends depkes2_manager {
	...
	function sk_controller() {
		$this->sk_label = array (
			'id_sk' => 'Id Surat Addendum',
			'no_sk' => 'Nomor Surat Addendum',
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		 ...
		 * ADDED BY : 566
		 */
		$field_arr[] = xform::xf('addendum_ke','C','255');
		$field_arr[] = xform::xf('mulai_nomer','C','255');
		$field_arr[] = xform::xf('no_lampiran','C','255');
		$field_arr[] = xform::xf('date_lampiran','I','8');
		 ...
		 * ADDED BY : 566
		 */
                $label_arr['addendum_ke'] = 'Addendum Ke';
                $label_arr['mulai_nomer'] = 'Mulai Dari Nomer Ke';
		$label_arr['no_lampiran'] = "No. Lampiran";
		$label_arr['date_lampiran'] = "Tanggal Lampiran";
...
//		$value_arr = array ();
		...
		$this->slip_field($config,'izin_produksi','after','no_sk');
                ...
                $label_arr['form_title'] = "Form ".ucwords($mode)." Surat Addendum ".$GLOBALS[my_title]."";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$record['date_lampiran'] = $this->parsedate(trim($record['date_lampiran']));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
	...
	function view_sk_form() {
		 ...
		 * ADDED BY : 566
		 */
		global $smarty_path, $template_path, $compile_path, $my_kat, $suf1, $suf2;
		 ...
		 * ADDED BY : 566
		 */
		$addendum_ke = $rsb->fields[addendum_ke];
		$mulai_nomer = $rsb->fields[mulai_nomer];
		$no_lampiran = $rsb->fields[no_lampiran];
		$date_lampiran = strftime("%d %B %G",strtotime(date('d F Y',$rsb->fields[date_lampiran])));
	...
	if ($suf2=="penyalur") {
		...
		$show_license    = "";
		$company_license = "";
		$number          = $mulai_nomer - 1;
		for ($i=1;$i<=$default_license_to_show;$i++) {
			if ($license_pattern['license_'.$i]!="") {
				$number++;
				$company_license .= ($company_license=="") ? $license_pattern['license_'.$i] : ", ".$license_pattern['license_'.$i] ;

				$show_license .= $number." <b>".$license_pattern['license_'.$i]."</b>";
		...
		$smarty->assign('kategori',"NOT DEFINED");
		$smarty->assign('kemasan',$kemasan);
		$smarty->assign('kota_pendaftar',$alamat_pendaftar_2);
		$smarty->assign('addendum_ke',$addendum_ke);
		$smarty->assign('no_lampiran',$no_lampiran);
		$smarty->assign('date_lampiran',$date_lampiran);

	if ($suf2=="penyalur") {
		$smarty->assign('lisensi',$show_license);
		$smarty->assign('perusahaan_lisensi',$company_license);
		$smarty->assign('dari_1_sampai',$mulai_nomer - 1);
	}
		...
		$smarty->assign('sub_kategori',"NOT DEFINED");
		$smarty->assign('tg',$tg);
		$smarty->assign('tipe',"NOT DEFINED");
	...
	if ($suf2=="penyalur") {
		...
		switch ($suf1) {
		  case "registrasi" : switch($suf2) {
					case "produksi" : $smarty->display('surat_addendum_registrasi_izin_produksi.tpl');
							  break;

					case "penyalur" : $smarty->display('surat_addendum_registrasi_izin_penyalur.tpl');
							  break;
				      } // end : switch($suf2)
				      break;

		  case "ubah" : switch($suf2) {
				  case "produksi" : // NOT FOUND
						    break;
				} // end : switch($suf2)
				break;
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Addendum '.$GLOBALS[my_title].' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Surat Addendum '.$GLOBALS[my_title].' Administration';
