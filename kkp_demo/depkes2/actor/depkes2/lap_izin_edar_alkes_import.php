...
class cek_kelengkapan_data_edar_import_controller extends depkes2_manager {
	...
	function cek_kelengkapan_data_edar_import_controller() {
		$this->cek_kelengkapan_data_edar_import_label = array (
			...
			'nama_pendaftar_edar_alkes_import' => 'Nama Pendaftar',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_1_edar_alkes_import.no_tt as svalue,
  ...
  $jas ="
		...
		jumpto1('frame_tt_1_edar_alkes_import.php?sql='+sql+'&a='+b)

		";

	$GLOBALS['out_before_footer'] = '
 	<iframe marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=auto name=\'iframe_entry1\' id=\'iframe_entry1\' style="width:0;height:0" src="frame_tt_1_edar_alkes_import.php"></iframe>
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Edar Alkes Import";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Edar Alkes Import');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_kelengkapan_data_edar_import where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Edar Alkes Import'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_edar_alkes_import.nama_pabrik,
		tt_1_edar_alkes_import.no_tt as status,
		tt_1_edar_alkes_import.no_tt as status_pending,
		tt_1_edar_alkes_import.no_tt as status_tolak,
		tt_1_edar_alkes_import.no_tt as status_selesai,
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_edar_alkes_import WHERE no_tt=\'$f\'");
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Edar Alkes Import'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Izin Edar Alkes Import';
