...
class cek_1_penyalur_controller extends depkes2_manager {
	...
	function cek_1_penyalur_controller() {
		$this->cek_1_penyalur_label = array (
			...
			'nama_pendaftar_penyalur' => 'Nama Pendaftar Penyalur',
	...
	function no_tt_form(&$config) {
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.no_tt=tt_1_penyalur.no_tt)
			LEFT OUTER JOIN pendaftar_penyalur ON(pendaftar_penyalur.kode_pendaftar_penyalur = tt_1_penyalur.kode_pendaftar_penyalur )
			WHERE cek_1_penyalur.no_tt IS NULL
			ORDER BY
				tt_1_penyalur.date_insert
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_1_penyalur WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Penyalur ";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			LEFT OUTER JOIN kelengkapan_penyalur ON(kelengkapan_penyalur.id_kelengkapan = detail_cek_1_penyalur.id_kelengkapan)
			WHERE
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM cek_1_penyalur WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Penyalur');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_1_penyalur where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Penyalur'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_penyalur.nama_pabrik,
		tt_1_penyalur.no_tt as status,
		tt_1_penyalur.no_tt as status_pending,
		tt_1_penyalur.no_tt as status_tolak,
		tt_1_penyalur.no_tt as status_selesai,
		...
		$skA = $skA->fields[\'no_surat_keputusan_penyalur\'];
		if($skA !=""){
		$str .= $skA;
		}else{
		$str .= "&nbsp;";
		...
		$USql = "
			...
			detail_cek_1_penyalur.alasan_penilai,
			CASE WHEN detail_cek_1_penyalur.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_1_penyalur.alasan_kaseks,
			CASE WHEN detail_cek_1_penyalur.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_1_penyalur.alasan_kasubdit,
			CASE WHEN detail_cek_1_penyalur.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_1_penyalur.alasan_direktur
		...
		$Cql = "
		SELECT
		surat_tambahan_penyalur_data.kepada_surat,
		cek_1_penyalur.no_tt
		FROM
		surat_tambahan_penyalur_data
		LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.id_cek_1 = surat_tambahan_penyalur_data.kepada_surat)
		WHERE
		cek_1_penyalur.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_penyalur WHERE no_tt=\'$f\'");
		...
		SELECT
		surat_tambahan_penyalur_data.kepada_surat,
		surat_tambahan_penyalur_data.nomor_surat,
		cek_1_penyalur.no_tt
		FROM
		surat_tambahan_penyalur_data
		LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.id_cek_1 = surat_tambahan_penyalur_data.kepada_surat)
		...
		//print $A." -- ".$B." -- ".$C." -- ".$D." -- ";
		if((!$A) && (!$B)){
		 if($D ==""){$surat = "";}else{$surat ="Cek Kelengkapan";}
		 $str .= "&nbsp;".$surat;
		}else{
		 $str .= "&nbsp;";
		}
		...
		SELECT
		surat_penolakan_penyalur.id_cek_1,
		surat_penolakan_penyalur.nomor_surat as nomor_surat_penolakan_penyalur,
		cek_1_penyalur.no_tt
		FROM
		surat_penolakan_penyalur
		LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.id_cek_1 = surat_penolakan_penyalur.id_cek_1)
		...
		cek_1_penyalur.no_tt =\'$f\'";
		$rsB = $adodb->Execute($Bql);
		$nomor_surat_penolakan_penyalur = $rsB->fields[nomor_surat_penolakan_penyalur];
		$B = $rsB->fields[no_tt];
		...
		//print $A." -- ".$B." -- ".$C." -- ".$D." -- ";
		if($B){
		 $str .= "&nbsp;".$nomor_surat_penolakan_penyalur;
		}else{
		 $str .= "&nbsp;";
		}
		';
		...
		cek_1_penyalur.no_tt =\'$f\'";
		$rsA = $adodb->Execute($Aql);
		$no_surat_keputusan_penyalur = $rsA->fields[no_surat_keputusan_penyalur];
		$A = $rsA->fields[no_tt];
		...
		//print $A." -- ".$B." -- ".$C." -- ".$D." -- ";
		if(($A)){
		 $str .= "&nbsp;".$no_surat_keputusan_penyalur;
		}else{
		 $str .= "&nbsp;";
		}
		';
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Penyalur'.' - '.date($GLOBALS['date_format']));
