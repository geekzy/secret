...
if (class_exists('kelengkapan_2_alkes_controller')) {
	// do nothing
} else if (defined('CLASS_kelengkapan_2_alkes_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_kelengkapan_2_alkes_CONTROLLER', TRUE);

include_once 'class.kelengkapan_2_alkes.inc.php';
class kelengkapan_2_alkes_controller extends depkes2_manager {
	var $kelengkapan_2_alkes_label;
	var $optional_arr;
	function kelengkapan_2_alkes_controller() {
		$this->kelengkapan_2_alkes_label = array (
	...
	function add_kelengkapan_2_alkes_form() {
		include_once 'class.xform.inc.php';
	
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->kelengkapan_2_alkes_label;
		...
		$rs = $adodb->Execute("SELECT * FROM kelengkapan_2_alkes WHERE id_kelengkapan='{$record['id_kelengkapan']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Kelengkapan Perubahan Data Izin Produksi";
	...
	function update_kelengkapan_2_alkes_form() {
		return $this->add_kelengkapan_2_alkes_form();
	}

	// handle event add Kelengkapan Perubahan Data Alkes
	function add_kelengkapan_2_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM kelengkapan_2_alkes WHERE id_kelengkapan = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM kelengkapan_2_alkes WHERE id_kelengkapan = NULL");
	...
	function update_kelengkapan_2_alkes() {
		return $this->add_kelengkapan_2_alkes();
	}

	// handle delete Kelengkapan Perubahan Data Alkes
	function delete_kelengkapan_2_alkes() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from kelengkapan_2_alkes where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Kelengkapan Perubahan Data Izin Produksi Status'));
	...
	function list_kelengkapan_2_alkes($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
...
//		if ($this->get_permission('fill_this')) {
			...
			'?action=add' . 
			'\', 600, 400, null, null, \'add_kelengkapan_2_alkes\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_kelengkapan_2_alkes\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_kelengkapan_2_alkes\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_kelengkapan_2_alkes\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Kelengkapan Perubahan Data Izin Produksi'.' - '.date($GLOBALS['date_format']));
	...
	function print_kelengkapan_2_alkes() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_kelengkapan_2_alkes($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$kelengkapan_2_alkes_controller = new kelengkapan_2_alkes_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_2_alkes_controller->add_kelengkapan_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $kelengkapan_2_alkes_controller->add_kelengkapan_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_2_alkes_controller->update_kelengkapan_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $kelengkapan_2_alkes_controller->update_kelengkapan_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'del':
			$out_content = $kelengkapan_2_alkes_controller->delete_kelengkapan_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_2_alkes_controller->import_kelengkapan_2_alkes_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $kelengkapan_2_alkes_controller->import_kelengkapan_2_alkes();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $kelengkapan_2_alkes_controller->print_kelengkapan_2_alkes();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $kelengkapan_2_alkes_controller->list_kelengkapan_2_alkes();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Kelengkapan Perubahan Data Izin Produksi Administration';
