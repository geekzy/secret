...
class cek_pemutih_alkes_controller extends depkes2_manager {
	...
	function cek_pemutih_alkes_controller() {
		$this->cek_pemutih_alkes_label = array (
			...
			'nama_pendaftar_pemutih' => 'Nama pendaftar_pemutih',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_pemutih_alkes.no_tt as svalue,
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_pemutih_alkes ON(cek_pemutih_alkes.no_tt=tt_pemutih_alkes.no_tt)
			LEFT OUTER JOIN pendaftar_pemutih ON(pendaftar_pemutih.kode_pendaftar_pemutih = tt_pemutih_alkes.kode_pendaftar_pemutih )
			WHERE cek_pemutih_alkes.no_tt IS NULL
			ORDER BY
				tt_pemutih_alkes.date_insert
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_pemutih']){
				$sqld = "SELECT id_cek_pemutih,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_pemutih_alkes WHERE id_cek_pemutih='".$_GET['id_cek_pemutih']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Produksi";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_pemutih_alkes.id_kelengkapan)
			WHERE
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM cek_pemutih_alkes WHERE id_cek_pemutih = '".$_POST['oldpkvalue']."'");
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Produksi');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_pemutih_alkes where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Produksi'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_pemutih.nama_pabrik,
		tt_pemutih_alkes.no_tt as status,
		tt_pemutih_alkes.no_tt as status_pending,
		tt_pemutih_alkes.no_tt as status_tolak,
		tt_pemutih_alkes.no_tt as status_selesai,
...
2--
		...
		*/
		...
		$USql = "
			...
			detail_cek_pemutih_alkes.alasan_penilai,
			CASE WHEN detail_cek_pemutih_alkes.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_pemutih_alkes.alasan_kaseks,
			CASE WHEN detail_cek_pemutih_alkes.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_pemutih_alkes.alasan_kasubdit,
			CASE WHEN detail_cek_pemutih_alkes.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_pemutih_alkes.alasan_direktur
		...
		LEFT OUTER JOIN cek_pemutih_alkes ON(cek_pemutih_alkes.id_cek_pemutih = surat_tambahan_data_pemutih.kepada_surat)
		WHERE
		cek_pemutih_alkes.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_pemutih_alkes WHERE no_tt=\'$f\'");
