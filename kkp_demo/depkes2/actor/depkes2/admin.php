...
if (class_exists('admin_controller')) {
	// do nothing
} else if (defined('CLASS_ADMIN_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_ADMIN_CONTROLLER', TRUE);

include_once 'class.admin.inc.php';
class admin_controller extends depkes2_manager {
	var $admin_label;
	var $optional_arr;
	function admin_controller() {
		$this->admin_label = array (
			'userid' => 'Userid',
			'passwd' => 'Passwd',
			'name' => 'Name',
			'insert_by' => 'Insert By',
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM admin WHERE userid='{$record['userid']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Admin";
	...
	function update_admin_form() {
		return $this->add_admin_form();
	}

	// handle event add Admin
	function add_admin() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM admin WHERE userid = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM admin WHERE userid = NULL");
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from admin where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'Admin <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Admin Status'));
	...
	// handle list Admin
	function list_admin($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_admin\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_admin\')'.
			'win.focus()):' . 
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Admin'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	switch ($action) {
		case 'add':
			...
			$out_content = $admin_controller->add_admin_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $admin_controller->add_admin();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $admin_controller->update_admin_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $admin_controller->update_admin();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'del':
			$out_content = $admin_controller->delete_admin();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $admin_controller->import_admin_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $admin_controller->import_admin();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $admin_controller->print_admin();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $admin_controller->list_admin();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Admin Administration';
