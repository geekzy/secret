...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Inbox Pemutihan Izin Produksi Seksi";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$_block = new block();
		$_block->set_config('title', 'Inbox Pemutihan Izin Produksi Seksi Status');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Inbox Pemutihan Izin Produksi Seksi Status'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		RIGHT OUTER JOIN tt_pemutih_alkes ON(tt_pemutih_alkes.no_tt = inbox_pemutih_produksi_subdit.no_tt)
  		LEFT OUTER JOIN pendaftar_pemutih ON(pendaftar_pemutih.kode_pendaftar_pemutih = tt_pemutih_alkes.kode_pendaftar_pemutih)
		LEFT OUTER JOIN cek_pemutih_alkes ON(cek_pemutih_alkes.no_tt = tt_pemutih_alkes.no_tt)
		LEFT JOIN surat_keputusan_pemutih ON(surat_keputusan_pemutih.id_cek_pemutih = cek_pemutih_alkes.id_cek_pemutih)
		WHERE
		(inbox_pemutih_produksi_subdit.userid='$userlog') AND surat_keputusan_pemutih.no_surat_keputusan_pemutih IS NULL
		";
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Inbox Pemutihan Izin Produksi Seksi'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Inbox Pemutihan Izin Produksi Seksi Administration';
