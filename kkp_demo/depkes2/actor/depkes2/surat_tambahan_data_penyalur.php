...
if (class_exists('surat_tambahan_data_penyalur_controller')) {
	// do nothing
} else if (defined('CLASS_surat_tambahan_data_penyalur_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_tambahan_data_penyalur_CONTROLLER', TRUE);

include_once 'class.surat_tambahan_data_penyalur.inc.php';
class surat_tambahan_data_penyalur_controller extends depkes2_manager {
	var $surat_tambahan_data_penyalur_label;
	var $optional_arr;
	function surat_tambahan_data_penyalur_controller() {
		$this->surat_tambahan_data_penyalur_label = array (
			'id_surat_tambahan_data_penyalur' => 'id_surat_tambahan_data_penyalur',
			'nomor_menteri' => 'NOMOR',
		...
		$this->optional_arr = array (
			'id_surat_tambahan_data_penyalur' => FALSE,
			'passwd' => FALSE,
			'name' => FALSE,
			'insert_by' => TRUE,
	...
	// create add surat_tambahan_data_penyalur form
	function add_surat_tambahan_data_penyalur_form() {
		include_once 'class.xform.inc.php';

//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_tambahan_data_penyalur_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM surat_tambahan_data_penyalur WHERE id_surat_tambahan_data_penyalur='{$record['id_surat_tambahan_data_penyalur']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_tambahan_data_penyalur'] = 'protect';
			$mode = 'edit';
		...
		$optional_arr['kepada_surat'] = 'user_defined';
			...
			FROM
				cek_1_penyalur
			LEFT OUTER JOIN surat_tambahan_data_penyalur ON(surat_tambahan_data_penyalur.kepada_surat = cek_1_penyalur.id_cek_1)
			...
			WHERE
			cek_1_penyalur.status_kasubdit = \'0\' OR
			cek_1_penyalur.status_direktur = \'0\'
			ORDER BY
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_tambahan_data_penyalur']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Tambahan Data Izin Penyalur";
	...
	// create update surat_tambahan_data_penyalur form
	function update_surat_tambahan_data_penyalur_form() {
		return $this->add_surat_tambahan_data_penyalur_form();
	}

	
	function view_surat_tambahan_data_penyalur_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_tambahan_data_penyalur::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_tambahan_data_penyalur'] = 'protect';

		$record = array (
			'id_surat_tambahan_data_penyalur' => ${$GLOBALS['get_vars']}['id_surat_tambahan_data_penyalur']
		);
		$result = surat_tambahan_data_penyalur::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_tambahan_data_penyalur_label;
		global $adodb;
		...
		$sqlx = "
		SELECT
		surat_tambahan_data_penyalur.nomor_surat,
		surat_tambahan_data_penyalur.lampiran_surat,
		surat_tambahan_data_penyalur.date_surat,
		pendaftar_penyalur.nama_pabrik,
		pendaftar_penyalur.alamat_pabrik,
		surat_tambahan_data_penyalur.alat,
		surat_tambahan_data_penyalur.isi_1,
		surat_tambahan_data_penyalur.isi_2,
		surat_tambahan_data_penyalur.isi_3,
		surat_tambahan_data_penyalur.isi_4,
		surat_tambahan_data_penyalur.isi_5,
		surat_tambahan_data_penyalur.nama,
		surat_tambahan_data_penyalur.nip,
		surat_tambahan_data_penyalur.insert_by,
		surat_tambahan_data_penyalur.date_insert
		FROM surat_tambahan_data_penyalur
		LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.id_cek_1 = surat_tambahan_data_penyalur.kepada_surat)
		LEFT OUTER JOIN tt_1_penyalur ON (tt_1_penyalur.no_tt = cek_1_penyalur.no_tt)
		LEFT OUTER JOIN pendaftar_penyalur ON( pendaftar_penyalur.kode_pendaftar_penyalur = tt_1_penyalur.kode_pendaftar_penyalur)
		WHERE
		surat_tambahan_data_penyalur.id_surat_tambahan_data_penyalur ='".$value_arr['id_surat_tambahan_data_penyalur']."'
		";
		...
		$pdf->SetFont('Arial','',12);
		$pdf->Ln(20);
		$pdf->Cell(180,7,'     Sehubungan dengan Pendaftar Penyaluran alat kesehatan/ Kosmetik/ perbekalan  kesehatan   rumah','',0,'L');
		$pdf->Ln(10);
	...
	// handle event add surat_tambahan_data_penyalur
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
	...
	// handle event update surat_tambahan_data_penyalur
	function update_surat_tambahan_data_penyalur() {
		return $this->add_surat_tambahan_data_penyalur();
	}

	// handle delete surat_tambahan_data_penyalur
	function delete_surat_tambahan_data_penyalur() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_tambahan_data_penyalur where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_tambahan_data_penyalur <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Tambahan Data Izin Penyalur'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_tambahan_data_penyalur
	function list_surat_tambahan_data_penyalur($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $adodb, ${$GLOBALS['get_vars']};
		$sql = 'SELECT
		surat_tambahan_data_penyalur.id_surat_tambahan_data_penyalur,
		surat_tambahan_data_penyalur.nomor_surat,
		surat_tambahan_data_penyalur.lampiran_surat,
		surat_tambahan_data_penyalur.date_surat,
		pendaftar_penyalur.nama_pabrik,
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_tambahan_data_penyalur' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_tambahan_data_penyalur\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_tambahan_data_penyalur\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_tambahan_data_penyalur\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_tambahan_data_penyalur\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$label_arr = $this->surat_tambahan_data_penyalur_label;
		$config = array (
			'id'		=> 'surat_tambahan_data_penyalur',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Tambahan Data Izin Penyalur'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_tambahan_data_penyalur
	function print_surat_tambahan_data_penyalur() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_tambahan_data_penyalur($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_tambahan_data_penyalur_controller = new surat_tambahan_data_penyalur_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_penyalur_controller->add_surat_tambahan_data_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_tambahan_data_penyalur_controller->add_surat_tambahan_data_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_penyalur_controller->update_surat_tambahan_data_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_tambahan_data_penyalur_controller->update_surat_tambahan_data_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_tambahan_data_penyalur_controller->view_surat_tambahan_data_penyalur_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_tambahan_data_penyalur_controller->delete_surat_tambahan_data_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_penyalur_controller->import_surat_tambahan_data_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $surat_tambahan_data_penyalur_controller->import_surat_tambahan_data_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_tambahan_data_penyalur_controller->print_surat_tambahan_data_penyalur();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_tambahan_data_penyalur_controller->list_surat_tambahan_data_penyalur();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Tambahan Data Izin Penyalur Administration';
