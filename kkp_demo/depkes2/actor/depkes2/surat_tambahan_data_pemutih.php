...
if (class_exists('surat_tambahan_data_pemutih_controller')) {
	// do nothing
} else if (defined('CLASS_surat_tambahan_data_pemutih_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_tambahan_data_pemutih_CONTROLLER', TRUE);

include_once 'class.surat_tambahan_data_pemutih.inc.php';
class surat_tambahan_data_pemutih_controller extends depkes2_manager {
	var $surat_tambahan_data_pemutih_label;
	var $optional_arr;
	function surat_tambahan_data_pemutih_controller() {
		$this->surat_tambahan_data_pemutih_label = array (
			'id_surat_tambahan_data_pemutih' => 'id_surat_tambahan_data_pemutih',
			'nomor_menteri' => 'NOMOR',
		...
		$this->optional_arr = array (
			'id_surat_tambahan_data_pemutih' => FALSE,
			'passwd' => FALSE,
			'name' => FALSE,
			'insert_by' => TRUE,
	...
	// create add surat_tambahan_data_pemutih form
	function add_surat_tambahan_data_pemutih_form() {
		include_once 'class.xform.inc.php';

//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_tambahan_data_pemutih_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM surat_tambahan_data_pemutih WHERE id_surat_tambahan_data_pemutih='{$record['id_surat_tambahan_data_pemutih']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_tambahan_data_pemutih'] = 'protect';
			$mode = 'edit';
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_tambahan_data_pemutih']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Tambahan Data Pemutihan Izin Produksi";
	...
	// create update surat_tambahan_data_pemutih form
	function update_surat_tambahan_data_pemutih_form() {
		return $this->add_surat_tambahan_data_pemutih_form();
	}

	
	function view_surat_tambahan_data_pemutih_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_tambahan_data_pemutih::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_tambahan_data_pemutih'] = 'protect';

		$record = array (
			'id_surat_tambahan_data_pemutih' => ${$GLOBALS['get_vars']}['id_surat_tambahan_data_pemutih']
		);
		$result = surat_tambahan_data_pemutih::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_tambahan_data_pemutih_label;
		global $adodb;
		...
		surat_tambahan_data_pemutih.lampiran_surat,
		surat_tambahan_data_pemutih.date_surat,
		pendaftar_pemutih.nama_pabrik,
		pendaftar_pemutih.alamat_pabrik,
		surat_tambahan_data_pemutih.alat,
		surat_tambahan_data_pemutih.isi_1,
		surat_tambahan_data_pemutih.isi_2,
		surat_tambahan_data_pemutih.isi_3,
		surat_tambahan_data_pemutih.isi_4,
		surat_tambahan_data_pemutih.isi_5,
		surat_tambahan_data_pemutih.nama,
		surat_tambahan_data_pemutih.nip,
		surat_tambahan_data_pemutih.insert_by,
		surat_tambahan_data_pemutih.date_insert
		FROM surat_tambahan_data_pemutih
		LEFT OUTER JOIN cek_pemutih_alkes ON(cek_pemutih_alkes.id_cek_pemutih = surat_tambahan_data_pemutih.kepada_surat)
		LEFT OUTER JOIN tt_pemutih_alkes ON (tt_pemutih_alkes.no_tt = cek_pemutih_alkes.no_tt)
		LEFT OUTER JOIN pendaftar_pemutih ON( pendaftar_pemutih.kode_pendaftar_pemutih = tt_pemutih_alkes.kode_pendaftar_pemutih)
		WHERE
		surat_tambahan_data_pemutih.id_surat_tambahan_data_pemutih ='".$value_arr['id_surat_tambahan_data_pemutih']."'
		";
	...
	// handle event add surat_tambahan_data_pemutih
	function add_surat_tambahan_data_pemutih() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;

		$rs = $adodb->Execute("SELECT * FROM surat_tambahan_data_pemutih WHERE id_surat_tambahan_data_pemutih = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM surat_tambahan_data_pemutih WHERE id_surat_tambahan_data_pemutih = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		$status = "Successfull $st '<b>{$record['id_surat_tambahan_data_pemutih']}</b>'";
		$this->log($status);
		$_block = new block();
		$_block->set_config('title', 'Status Surat Tambahan Data Pemutihan Izin Produksi');
	...
	// handle event update surat_tambahan_data_pemutih
	function update_surat_tambahan_data_pemutih() {
		return $this->add_surat_tambahan_data_pemutih();
	}

	// handle delete surat_tambahan_data_pemutih
	function delete_surat_tambahan_data_pemutih() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_tambahan_data_pemutih where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_tambahan_data_pemutih <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Tambahan Data Pemutihan Izin Produksi'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_tambahan_data_pemutih
	function list_surat_tambahan_data_pemutih($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $adodb, ${$GLOBALS['get_vars']};
		$sql = 'SELECT
		surat_tambahan_data_pemutih.id_surat_tambahan_data_pemutih,
		surat_tambahan_data_pemutih.nomor_surat,
		surat_tambahan_data_pemutih.lampiran_surat,
		surat_tambahan_data_pemutih.date_surat,
		pendaftar_pemutih.nama_pabrik,
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_tambahan_data_pemutih' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_tambahan_data_pemutih\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_tambahan_data_pemutih\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_tambahan_data_pemutih\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_tambahan_data_pemutih\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$label_arr = $this->surat_tambahan_data_pemutih_label;
		$config = array (
			'id'		=> 'surat_tambahan_data_pemutih',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Tambahan Data Pemutihan Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_tambahan_data_pemutih
	function print_surat_tambahan_data_pemutih() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_tambahan_data_pemutih($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_tambahan_data_pemutih_controller = new surat_tambahan_data_pemutih_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_pemutih_controller->add_surat_tambahan_data_pemutih_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_tambahan_data_pemutih_controller->add_surat_tambahan_data_pemutih();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_pemutih_controller->update_surat_tambahan_data_pemutih_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_tambahan_data_pemutih_controller->update_surat_tambahan_data_pemutih();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_tambahan_data_pemutih_controller->view_surat_tambahan_data_pemutih_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_tambahan_data_pemutih_controller->delete_surat_tambahan_data_pemutih();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_tambahan_data_pemutih_controller->import_surat_tambahan_data_pemutih_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $surat_tambahan_data_pemutih_controller->import_surat_tambahan_data_pemutih();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_tambahan_data_pemutih_controller->print_surat_tambahan_data_pemutih();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_tambahan_data_pemutih_controller->list_surat_tambahan_data_pemutih();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Tambahan Data Pemutihan Izin Produksi Administration';
