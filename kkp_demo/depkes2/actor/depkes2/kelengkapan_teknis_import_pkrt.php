...
if (class_exists('kelengkapan_teknis_import_pkrt_controller')) {
	// do nothing
} else if (defined('CLASS_kelengkapan_teknis_import_pkrt_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_kelengkapan_teknis_import_pkrt_CONTROLLER', TRUE);

include_once 'class.kelengkapan_teknis_import_pkrt.inc.php';
class kelengkapan_teknis_import_pkrt_controller extends depkes2_manager {
	var $kelengkapan_teknis_import_pkrt_label;
	var $optional_arr;
	function kelengkapan_teknis_import_pkrt_controller() {
		$this->kelengkapan_teknis_import_pkrt_label = array (
	...
	function add_kelengkapan_teknis_import_pkrt_form() {
		include_once 'class.xform.inc.php';
	
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->kelengkapan_teknis_import_pkrt_label;
		...
		$rs = $adodb->Execute("SELECT * FROM kelengkapan_teknis_import_pkrt WHERE id_kelengkapan='{$record['id_kelengkapan']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Kelengkapan Teknis PKRT";
	...
	function update_kelengkapan_teknis_import_pkrt_form() {
		return $this->add_kelengkapan_teknis_import_pkrt_form();
	}

	// handle event add Kelengkapan Perubahan Data Alkes
	function add_kelengkapan_teknis_import_pkrt() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM kelengkapan_teknis_import_pkrt WHERE id_kelengkapan = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM kelengkapan_teknis_import_pkrt WHERE id_kelengkapan = NULL");
	...
	function update_kelengkapan_teknis_import_pkrt() {
		return $this->add_kelengkapan_teknis_import_pkrt();
	}

	// handle delete Kelengkapan Perubahan Data Alkes
	function delete_kelengkapan_teknis_import_pkrt() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from kelengkapan_teknis_import_pkrt where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Kelengkapan Teknis PKRT Status'));
	...
	function list_kelengkapan_teknis_import_pkrt($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		date_insert,
		jenis
		FROM kelengkapan_teknis_import_pkrt';
		$optional_arr = $this->optional_arr;
...
//		if ($this->get_permission('fill_this')) {
			...
			'?action=add' . 
			'\', 600, 400, null, null, \'add_kelengkapan_teknis_import_pkrt\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_kelengkapan_teknis_import_pkrt\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_kelengkapan_teknis_import_pkrt\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_kelengkapan_teknis_import_pkrt\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$label_arr = $this->kelengkapan_teknis_import_pkrt_label;
		$config = array (
			'id'		=> 'kelengkapan_teknis_import_pkrt',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Kelengkapan Teknis PKRT '.' - '.date($GLOBALS['date_format']));
	...
	function print_kelengkapan_teknis_import_pkrt() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_kelengkapan_teknis_import_pkrt($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$kelengkapan_teknis_import_pkrt_controller = new kelengkapan_teknis_import_pkrt_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_teknis_import_pkrt_controller->add_kelengkapan_teknis_import_pkrt_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $kelengkapan_teknis_import_pkrt_controller->add_kelengkapan_teknis_import_pkrt();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_teknis_import_pkrt_controller->update_kelengkapan_teknis_import_pkrt_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $kelengkapan_teknis_import_pkrt_controller->update_kelengkapan_teknis_import_pkrt();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'del':
			$out_content = $kelengkapan_teknis_import_pkrt_controller->delete_kelengkapan_teknis_import_pkrt();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import_pkrt':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $kelengkapan_teknis_import_pkrt_controller->import_pkrt_kelengkapan_teknis_import_pkrt_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport_pkrt':
			$out_content = $kelengkapan_teknis_import_pkrt_controller->import_pkrt_kelengkapan_teknis_import_pkrt();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $kelengkapan_teknis_import_pkrt_controller->print_kelengkapan_teknis_import_pkrt();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $kelengkapan_teknis_import_pkrt_controller->list_kelengkapan_teknis_import_pkrt();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Kelengkapan Teknis PKRT';
