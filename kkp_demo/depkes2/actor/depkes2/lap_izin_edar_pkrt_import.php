...
class cek_kelengkapan_data_edar_import_controller extends depkes2_manager {
	...
	function cek_kelengkapan_data_edar_import_controller() {
		$this->cek_kelengkapan_data_edar_import_label = array (
			...
			'nama_pendaftar_edar_pkrt_import' => 'Nama Pendaftar',
	...
	function no_tt_form(&$config) {
		...
		include_once 'class.pendaftar_edar_pkrt_import.inc.php';
		if($_GET['id_cek_1']){
		
		$fk_sql = ''.
		'SELECT
				tt_1_edar_pkrt_import.no_tt as skey,
				tt_1_edar_pkrt_import.no_tt as svalue,
			...
			LEFT OUTER JOIN cek_kelengkapan_data_edar_import ON(cek_kelengkapan_data_edar_import.no_tt=tt_1_edar_pkrt_import.no_tt)
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_kelengkapan_data_edar_import ON(cek_kelengkapan_data_edar_import.no_tt=tt_1_edar_pkrt_import.no_tt)
			LEFT OUTER JOIN pendaftar_edar_pkrt_import ON(pendaftar_edar_pkrt_import.kode_pendaftar_edar_pkrt_import = tt_1_edar_pkrt_import.kode_pendaftar_edar_pkrt_import )
			WHERE cek_kelengkapan_data_edar_import.no_tt IS NULL
			ORDER BY
				tt_1_edar_pkrt_import.date_insert
		...
		$result = pendaftar_edar_pkrt_import::select($fk_sql);
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_edar_pkrt_import.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_edar_pkrt_import LEFT OUTER JOIN tt_1_edar_pkrt_import ON(tt_1_edar_pkrt_import.kode_pendaftar_edar_pkrt_import = pendaftar_edar_pkrt_import.kode_pendaftar_edar_pkrt_import) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_pkrt_import.kode_subdit)  WHERE tt_1_edar_pkrt_import.no_tt =\''+ b +'\'';

		jumpto1('frame_tt_1_edar_pkrt_import.php?sql='+sql+'&a='+b)

		";

	$GLOBALS['out_before_footer'] = '
 	<iframe marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=auto name=\'iframe_entry1\' id=\'iframe_entry1\' style="width:0;height:0" src="frame_tt_1_edar_pkrt_import.php"></iframe>
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_edar_import WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Edar PKRT  Import";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			FROM
			pendaftar_edar_pkrt_import
			LEFT OUTER JOIN tt_1_edar_pkrt_import ON(tt_1_edar_pkrt_import.kode_pendaftar_edar_pkrt_import = pendaftar_edar_pkrt_import.kode_pendaftar_edar_pkrt_import)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_edar_pkrt_import.kode_subdit)
			...
			LEFT OUTER JOIN kelengkapan_pkrt ON(kelengkapan_pkrt.id_kelengkapan = detail_cek_kelengkapan_data_edar_import.id_kelengkapan)
			WHERE
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_edar_import WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		$adodb->Execute("DELETE FROM cek_kelengkapan_data_edar_import WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		...
		include "class.detail_cek_kelengkapan_data_edar_import.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_kelengkapan_data_edar_import::add($record);
		}
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Edar PKRT  Import');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_kelengkapan_data_edar_import where ".$query);
		}
                if ($success){
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Edar PKRT  Import'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		subdit.subdit,
		tt_1_edar_pkrt_import.no_tt,
		pendaftar_edar_pkrt_import.nama_pendaftar_edar_pkrt_import,
		pendaftar_edar_pkrt_import.nama_pabrik,
		tt_1_edar_pkrt_import.no_tt as status,
		tt_1_edar_pkrt_import.no_tt as status_pending,
		tt_1_edar_pkrt_import.no_tt as status_tolak,
		tt_1_edar_pkrt_import.no_tt as status_selesai,
		...
		$USql = "
			...
			detail_cek_kelengkapan_data_edar_import.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_edar_import.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_kelengkapan_data_edar_import.alasan_kaseks,
			CASE WHEN detail_cek_kelengkapan_data_edar_import.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_kelengkapan_data_edar_import.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_edar_import.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_kelengkapan_data_edar_import.alasan_direktur
		...
		FROM
		surat_keputusan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_import ON(cek_kelengkapan_data_edar_import.id_cek_1 = surat_keputusan.id_cek_1)
		WHERE
		cek_kelengkapan_data_edar_import.no_tt =\'$f\'";
		...
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_kelengkapan_data_edar_import ON(cek_kelengkapan_data_edar_import.id_cek_1 = surat_penolakan.id_cek_1)
		WHERE
		cek_kelengkapan_data_edar_import.no_tt =\'$f\'";
		...
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_kelengkapan_data_edar_import ON(cek_kelengkapan_data_edar_import.id_cek_1 = surat_tambahan_data.kepada_surat)
		WHERE
		cek_kelengkapan_data_edar_import.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_edar_pkrt_import WHERE no_tt=\'$f\'");
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Edar PKRT  Import'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Izin Edar PKRT  Import';
