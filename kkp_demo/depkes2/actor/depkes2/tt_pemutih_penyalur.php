...
if (class_exists('tt_pemutih_penyalur_controller')) {
	// do nothing
} else if (defined('CLASS_tt_pemutih_penyalur_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_tt_pemutih_penyalur_CONTROLLER', TRUE);

include_once 'class.tt_pemutih_penyalur.inc.php';
class tt_pemutih_penyalur_controller extends depkes2_manager {
	var $tt_pemutih_penyalur_label;
	var $optional_arr;
	function tt_pemutih_penyalur_controller() {
		$this->tt_pemutih_penyalur_label = array (
			'urut_no_tt' => 'No Tanda Terima',
			'subdit' => 'Subdit',
			'kode_subdit' => 'Kode Subdit',
			'nama_pabrik' => 'Nama Pabrik',
 			'kode_pendaftar_pemutih_penyalur' => 'Nama pabrik / Nama Pemohon',
	...
	function subdit_form(&$config) {
		...
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->tt_pemutih_penyalur_label['kode_subdit']
			)
		);
...
function pendaftar_pemutih_penyalur_form(&$config) {
                eval($this->load_config);
		//print_r($value_arr);
                $selected = $value_arr['kode_pendaftar_pemutih_penyalur'];

		include_once 'class.pendaftar_pemutih_penyalur.inc.php';
		$fk_sql = ''.
			'SELECT
				kode_pendaftar_pemutih_penyalur as skey,
				nama_pabrik as svalue,
				nama_pendaftar_pemutih_penyalur as svalue2
			FROM
		...
		$opener_sql = htmlentities($fk_sql);
		$opener_sql = str_replace("\n", " ", $opener_sql);
		$opener_var = htmlentities('kode_pendaftar_pemutih_penyalur');
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->tt_pemutih_penyalur_label['kode_pendaftar_pemutih_penyalur']
			)
		);
  ...
  $jas ="
		b = document.theform.kode_pendaftar_pemutih_penyalur.value;
		sql = 'SELECT alamat_pabrik,nama_propinsi_2 FROM pendaftar_pemutih_penyalur  WHERE kode_pendaftar_pemutih_penyalur=\''+ b +'\'';
	...
	'.$GLOBALS['out_before_footer']
		...
		$result = array_merge($default_value, $result);
		$optional_arr['kode_pendaftar_pemutih_penyalur'] = 'user_defined';
		$value_arr['kode_pendaftar_pemutih_penyalur'] = $this->select_form('kode_pendaftar_pemutih_penyalur', $result, $selected,$multiple=FALSE,$jas);
		$optional_arr['kode_pendaftar_pemutih_penyalur_rule'] = "\n".
		"       if(theform.kode_pendaftar_pemutih_penyalur.value == '')\n".
		"       {\n".
		"               alert('Field ".$label_arr['kode_pendaftar_pemutih_penyalur']." ".__('empty').".');\n".
		"               theform.kode_pendaftar_pemutih_penyalur.focus();\n".
		"               form_submitted=false;\n".
		"               return false;\n".
		"       }\n";
	...
	// create add tt_pemutih_penyalur form
	function add_tt_pemutih_penyalur_form() {
		include_once 'class.xform.inc.php';
		//if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->tt_pemutih_penyalur_label;
		...
		$rs = $adodb->Execute("SELECT * FROM tt_pemutih_penyalur WHERE no_tt='{$record['no_tt']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$optional_arr['nama_tt_pemutih_penyalur_rule'] = '';
		$optional_arr['insert_by_rule'] = '';
		$optional_arr['date_insert_rule'] = '';
		
		global $adodb;
		$rsx = $adodb->Execute("SELECT no_tt FROM tt_pemutih_penyalur ORDER BY no_tt DESC LIMIT 1");
		...
		$this->pendaftar_pemutih_penyalur_form($config);
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Tanda Terima Pemutihan Izin Penyalur";
	...
	// create update tt_pemutih_penyalur form
	function update_tt_pemutih_penyalur_form() {
		return $this->add_tt_pemutih_penyalur_form();
	}


	function view_tt_pemutih_penyalur_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = tt_pemutih_penyalur::get_field_set();
		...
		$result = tt_pemutih_penyalur::get($record);
		$value_arr = $result[0];
		$label_arr = $this->tt_pemutih_penyalur_label;
		global $adodb;
		...
		$pdf->Ln(5);
		$pdf->Cell(180,7,'Tanda Terima Permohonan Pemutihan Ijin Produksi penyalur/PKRT','',0,'L');
		$pdf->Ln(5);
		...
		pendaftar_pemutih_penyalur.tpwd,
		tt_pemutih_penyalur.insert_by
		FROM tt_pemutih_penyalur
		...
		WHERE
		tt_pemutih_penyalur.no_tt ='".$_GET['no_tt']."'
		GROUP BY
		tt_pemutih_penyalur.no_tt,
		tt_pemutih_penyalur.urut_no_tt,
		subdit.subdit,
		...
		$nama_pendaftar = $rs->fields['nama_pendaftar_pemutih_penyalur'];
		$alamat_pendaftar = $rs->fields['alamat_pendaftar_pemutih_penyalur'];
	...
	// handle event add tt_pemutih_penyalur
	function add_tt_pemutih_penyalur() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;
		foreach ($record as $k => $v) $record[$k] = trim($v);

		$rsx = $adodb->Execute("SELECT * FROM tt_pemutih_penyalur ORDER by no_tt DESC LIMIT 1");
		...
		$rs = $adodb->Execute("SELECT * FROM tt_pemutih_penyalur WHERE no_tt = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM tt_pemutih_penyalur WHERE no_tt = NULL");
	...
	// handle event update tt_pemutih_penyalur
	function update_tt_pemutih_penyalur() {
		return $this->add_tt_pemutih_penyalur();
	}

	// handle delete tt_pemutih_penyalur
	function delete_tt_pemutih_penyalur() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from tt_pemutih_penyalur where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'tt_pemutih_penyalur <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Tanda Terima Pemutihan Izin Penyalur Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list tt_pemutih_penyalur
	function list_tt_pemutih_penyalur($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		subdit.subdit,
		pendaftar_pemutih_penyalur.nama_pabrik,
		tt_pemutih_penyalur.kode_pendaftar_pemutih_penyalur ,
		pendaftar_pemutih_penyalur.alamat_pabrik,
		pendaftar_pemutih_penyalur.nama_propinsi_2,
		tt_pemutih_penyalur.insert_by,
		tt_pemutih_penyalur.date_insert
...
//		if ($this->get_permission('fill_this')) {
			...
			'?action=add' .
			'\', 600, 400, null, null, \'add_tt_pemutih_penyalur\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_tt_pemutih_penyalur\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_tt_pemutih_penyalur\')'.
			'win.focus()):' . 
...
//		}
		...
		$view_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=view%s\', 640, 480, null, null, \'view_tt_pemutih_penyalur\');'.
			'win.focus()',
			"label"=>__('View'),
		...
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_tt_pemutih_penyalur\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Tanda Terima Pemutihan Izin Penyalur'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print tt_pemutih_penyalur
	function print_tt_pemutih_penyalur() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_tt_pemutih_penyalur($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$tt_pemutih_penyalur_controller = new tt_pemutih_penyalur_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $tt_pemutih_penyalur_controller->add_tt_pemutih_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $tt_pemutih_penyalur_controller->add_tt_pemutih_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangekode_pendaftar_pemutih_penyalur()"';
			$out_content = $tt_pemutih_penyalur_controller->update_tt_pemutih_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $tt_pemutih_penyalur_controller->update_tt_pemutih_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'del':
			$out_content = $tt_pemutih_penyalur_controller->delete_tt_pemutih_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $tt_pemutih_penyalur_controller->import_tt_pemutih_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $tt_pemutih_penyalur_controller->view_tt_pemutih_penyalur_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'postimport':
			$out_content = $tt_pemutih_penyalur_controller->import_tt_pemutih_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $tt_pemutih_penyalur_controller->print_tt_pemutih_penyalur();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $tt_pemutih_penyalur_controller->list_tt_pemutih_penyalur();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Tanda Terima Pemutihan Izin Penyalur Administration';
