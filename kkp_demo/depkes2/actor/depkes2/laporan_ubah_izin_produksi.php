...
class cek_2_alkes_controller extends depkes2_manager {
	...
	function cek_2_alkes_controller() {
		$this->cek_2_alkes_label = array (
			...
			'no_sk_lama' => 'Nomor SK Lama',
			'nama_pemohon_ubah_ijin_produksi' => 'Nama pemohon_ubah_ijin_produksi',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_2_alkes.no_tt as svalue,
				pemohon_ubah_ijin_produksi.nama_pabrik as svalue2
			FROM
			tt_2_alkes
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.no_tt=tt_2_alkes.no_tt)
			LEFT OUTER JOIN pemohon_ubah_ijin_produksi ON(pemohon_ubah_ijin_produksi.kode_pemohon_ubah_ijin_produksi = tt_2_alkes.kode_pendaftar )
			WHERE cek_2_alkes.no_tt IS NULL
			ORDER BY
				tt_2_alkes.date_insert
		...
		$result = pemohon_ubah_ijin_produksi::select($fk_sql);
  ...
  $jas ="
		...
		sql = 'SELECT pemohon_ubah_ijin_produksi.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pemohon_ubah_ijin_produksi LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.kode_pendaftar = pemohon_ubah_ijin_produksi.kode_pemohon_ubah_ijin_produksi) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_2_alkes.kode_subdit)  WHERE tt_2_alkes.no_tt =\''+ b +'\'';

		jumpto1('frame_tt_2_alkes.php?sql='+sql+'&a='+b)
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Perubahan Izin Produksi";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			$sqlB = "
			SELECT
			pemohon_ubah_ijin_produksi.alamat_pabrik,
			pemohon_ubah_ijin_produksi.nama_pabrik
			FROM
			...
			FROM
			detail_cek_2_alkes
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_2_alkes.id_kelengkapan)
			WHERE
			id_cek_2='".$id_cek_2."' AND status_penilai IS NOT NULL
			";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM cek_2_alkes WHERE id_cek_2 = '".$_POST['oldpkvalue']."'");
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Perubahan Izin Produksi');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_2_alkes where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Perubahan Izin Produksi'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$sql = "
			...
			surat_keputusan.no_surat_keputusan as no_sk_lama,
			tt_2_alkes.no_tt as status,
			tt_2_alkes.no_tt as status_pending,
			tt_2_alkes.no_tt as status_tolak,
			tt_2_alkes.no_tt as status_selesai,
			cek_2_alkes.id_cek_2 as no_sk,
...
2--
		...
		*/
		...
		$USql = "
			...
			detail_cek_2_alkes.alasan_penilai,
			CASE WHEN detail_cek_2_alkes.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_2_alkes.alasan_kaseks,
			CASE WHEN detail_cek_2_alkes.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_2_alkes.alasan_kasubdit,
			CASE WHEN detail_cek_2_alkes.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_2_alkes.alasan_direktur
		...
		//PAKE NO_TT BUKAN PAKE id_cek_2 JADI SENEN BENERIN YA OK
		$eval_arr['status'] = '
		global $adodb;
		$f = "%s";
		...
		$Aql = "
		SELECT
		surat_keputusan.id_cek_2,
		cek_2_alkes.no_tt
		FROM
		surat_keputusan
		LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.id_cek_2 = surat_keputusan.id_cek_2)
		WHERE
		cek_2_alkes.no_tt =\'$f\'";
		...
		$Bql = "
		SELECT
		surat_penolakan.id_cek_2,
		cek_2_alkes.no_tt
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.id_cek_2 = surat_penolakan.id_cek_2)
		WHERE
		cek_2_alkes.no_tt =\'$f\'";
		...
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.id_cek_2 = surat_tambahan_data.kepada_surat)
		WHERE
		cek_2_alkes.no_tt =\'$f\'";
		...
		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_2_alkes WHERE no_tt=\'$f\'");
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Perubahan Izin Produksi'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Perubahan Izin Produksi';
