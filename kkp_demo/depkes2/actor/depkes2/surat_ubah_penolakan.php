...
if (class_exists('surat_ubah_penolakan_controller')) {
	// do nothing
} else if (defined('CLASS_surat_ubah_penolakan_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_ubah_penolakan_CONTROLLER', TRUE);

include_once 'class.surat_ubah_penolakan.inc.php';
class surat_ubah_penolakan_controller extends depkes2_manager {
	var $surat_ubah_penolakan_label;
	var $optional_arr;
	function surat_ubah_penolakan_controller() {
		$this->surat_ubah_penolakan_label = array (
			'id_surat_ubah_penolakan' => 'id_surat_ubah_penolakan',
			'nomor_menteri' => 'NOMOR',
	...
	// create add surat_ubah_penolakan form
	function add_surat_ubah_penolakan_form() {
		include_once 'class.xform.inc.php';
	
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_ubah_penolakan_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$field_arr[] = xform::xf('id_cek_2','N','8');
		...
		$rs = $adodb->Execute("SELECT * FROM surat_ubah_penolakan WHERE id_surat_ubah_penolakan='{$record['id_surat_ubah_penolakan']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_ubah_penolakan'] = 'protect';
			$mode = 'edit';
		...
		$optional_arr['id_cek_2'] = 'user_defined';
                $arr = array();
                $arr['name'] = 'id_cek_2';
                $arr['selected'] = $value_arr['id_cek_2'];
		...
		if($mode=='add'){
			...
			WHERE
			(surat_ubah_penolakan.id_cek_2 IS NULL) OR (surat_ubah_keputusan.id_cek_2 IS NULL)
			ORDER BY
			...
			LEFT OUTER JOIN surat_ubah_keputusan ON(surat_ubah_keputusan.id_cek_2 = cek_2_alkes.id_cek_2)
			WHERE
			surat_ubah_keputusan.id_cek_2 IS NULL
			ORDER BY
				cek_2_alkes.date_insert';
		}
		
                $value_arr['id_cek_2'] = xform::dbs($arr);
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_ubah_penolakan']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Penolakan Perubahan Izin Produksi";
	...
	// create update surat_ubah_penolakan form
	function update_surat_ubah_penolakan_form() {
		return $this->add_surat_ubah_penolakan_form();
	}

	
	function view_surat_ubah_penolakan_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_ubah_penolakan::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_ubah_penolakan'] = 'protect';

		$record = array (
			'id_surat_ubah_penolakan' => ${$GLOBALS['get_vars']}['id_surat_ubah_penolakan']
		);
		$result = surat_ubah_penolakan::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_ubah_penolakan_label;
		global $adodb;
		...
		$sqlx = "
		SELECT
		surat_ubah_penolakan.id_surat_ubah_penolakan,
		surat_ubah_penolakan.nomor_surat,
		surat_ubah_penolakan.lampiran_surat,
		surat_ubah_penolakan.date_surat,
		surat_ubah_tambahan_data.nomor_surat as nomor_surat_tambahan,
		surat_ubah_tambahan_data.lampiran_surat as lampiran_surat_tambahan,
		surat_ubah_tambahan_data.date_surat as date_surat_tambahan,
		surat_ubah_tambahan_data.alat,
		pendaftar.nama_pabrik,
		pendaftar.alamat_pabrik,
		surat_ubah_penolakan.nama,
		surat_ubah_penolakan.nip,
		surat_ubah_penolakan.insert_by,
		surat_ubah_penolakan.date_insert
		FROM surat_ubah_penolakan
		LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.id_cek_2 = surat_ubah_penolakan.id_cek_2)
		LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
		LEFT OUTER JOIN pemohon_ubah_ijin_produksi ON(pemohon_ubah_ijin_produksi.kode_pemohon_ubah_ijin_produksi = tt_2_alkes.kode_pendaftar)
		LEFT OUTER JOIN surat_keputusan ON(surat_keputusan.no_surat_keputusan = pemohon_ubah_ijin_produksi.no_surat_keputusan )
		LEFT OUTER JOIN cek_1_alkes ON(cek_1_alkes.id_cek_1 = surat_keputusan.id_cek_1)
		...
		LEFT OUTER JOIN surat_ubah_tambahan_data ON(surat_ubah_tambahan_data.kepada_surat = cek_2_alkes.id_cek_2)
		WHERE
		surat_ubah_penolakan.id_surat_ubah_penolakan ='".$value_arr['id_surat_ubah_penolakan']."'
		";
		...
		$pdf->Ln(10);
		$pdf->Cell(180,7,'tanggal '.date('d M Y',$date_surat_tambahan).', maka sesuai dengan ketentuan yang berlaku, pendaftar tersebut kami tolak.',0,'L');
		$pdf->Ln(10);
	...
	// handle event add surat_ubah_penolakan
	function add_surat_ubah_penolakan() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;

		$rs = $adodb->Execute("SELECT * FROM surat_ubah_penolakan WHERE id_surat_ubah_penolakan = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM surat_ubah_penolakan WHERE id_surat_ubah_penolakan = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		$status = "Successfull $st '<b>{$record['id_surat_ubah_penolakan']}</b>'";
		$this->log($status);
		$_block = new block();
		$_block->set_config('title', 'Status Surat Penolakan Perubahan Izin Produksi');
	...
	// handle event update surat_ubah_penolakan
	function update_surat_ubah_penolakan() {
		return $this->add_surat_ubah_penolakan();
	}

	// handle delete surat_ubah_penolakan
	function delete_surat_ubah_penolakan() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_ubah_penolakan where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_ubah_penolakan <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Penolakan Perubahan Izin Produksi'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_ubah_penolakan
	function list_surat_ubah_penolakan($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_ubah_penolakan' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_ubah_penolakan\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_ubah_penolakan\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_ubah_penolakan\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_ubah_penolakan\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Penolakan Perubahan Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_ubah_penolakan
	function print_surat_ubah_penolakan() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_ubah_penolakan($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_ubah_penolakan_controller = new surat_ubah_penolakan_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_penolakan_controller->add_surat_ubah_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_ubah_penolakan_controller->add_surat_ubah_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_penolakan_controller->update_surat_ubah_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_ubah_penolakan_controller->update_surat_ubah_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_ubah_penolakan_controller->view_surat_ubah_penolakan_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_ubah_penolakan_controller->delete_surat_ubah_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_penolakan_controller->import_surat_ubah_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $surat_ubah_penolakan_controller->import_surat_ubah_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_ubah_penolakan_controller->print_surat_ubah_penolakan();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_ubah_penolakan_controller->list_surat_ubah_penolakan();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Penolakan Perubahan Izin Produksi Administration';
