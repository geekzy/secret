...
class cek_pemutih_penyalur_controller extends depkes2_manager {
	...
	function cek_pemutih_penyalur_controller() {
		$this->cek_pemutih_penyalur_label = array (
			...
			'nama_produk' => 'Jenis penyalur Yang Diproduksi',
	...
	function no_tt_form(&$config) {
		...
		if($_GET['id_cek_pemutih']){
			if($bag=='kaseksi'){
				...
				LEFT OUTER JOIN inbox_pemutih_penyalur_subdit ON(inbox_pemutih_penyalur_subdit.no_tt = tt_pemutih_penyalur.no_tt )
				RIGHT OUTER JOIN inbox_pemutih_penyalur_seksi ON(inbox_pemutih_penyalur_seksi.id_inbox_pemutih_penyalur_subdit = inbox_SELECT
					tt_pemutih_penyalur.no_tt as skey,
					tt_pemutih_penyalur.urut_no_tt as svalue,
					pendaftar_pemutih_penyalur.nama_pabrik as svalue2
				FROM
				...
				LEFT OUTER JOIN pendaftar_pemutih_penyalur ON(pendaftar_pemutih_penyalur.kode_pendaftar_pemutih_penyalur = tt_pemutih_penyalur.kode_pendaftar_pemutih_penyalur )
				LEFT OUTER JOIN inbox_pemutih_penyalur_subdit ON(inbox_pemutih_penyalur_subdit.no_tt = tt_pemutih_penyalur.no_tt )
				LEFT OUTER JOIN inbox_pemutih_penyalur_seksi ON(inbox_pemutih_penyalur_seksi.id_inbox_pemutih_penyalur_subdit = inbox_penyalur_subdit.id_inbox_pemutih_penyalur_subdit)pemutih_penyalur_subdit.id_inbox_pemutih_penyalur_subdit)
				WHERE inbox_pemutih_penyalur_seksi.insert_by='$userlog'
				ORDER BY
				...
				LEFT OUTER JOIN pendaftar_pemutih_penyalur ON(pendaftar_pemutih_penyalur.kode_pendaftar_pemutih_penyalur = tt_pemutih_penyalur.kode_pendaftar_pemutih_penyalur )
				LEFT OUTER JOIN inbox_pemutih_penyalur_subdit ON(inbox_pemutih_penyalur_subdit.no_tt = tt_pemutih_penyalur.no_tt )
				LEFT OUTER JOIN inbox_pemutih_penyalur_seksi ON(inbox_pemutih_penyalur_seksi.id_inbox_pemutih_penyalur_subdit = inbox_pemutih_penyalur_subdit.id_inbox_pemutih_penyalur_subdit)
				ORDER BY
				tt_pemutih_penyalur.date_insert
				";
			}
			...
			LEFT OUTER JOIN inbox_pemutih_penyalur_subdit ON(inbox_pemutih_penyalur_subdit.no_tt = tt_pemutih_penyalur.no_tt )
			LEFT OUTER JOIN inbox_pemutih_penyalur_seksi ON(inbox_pemutih_penyalur_seksi.id_inbox_pemutih_penyalur_subdit = inbox_pemutih_penyalur_subdit.id_inbox_pemutih_penyalur_subdit)
			WHERE ((tt_pemutih_penyalur.no_tt Is NOT NULL) OR (inbox_pemutih_penyalur_seksi.insert_by='$userlog')) AND cek_pemutih_penyalur.no_tt IS NULL
			ORDER BY
			tt_pemutih_penyalur.date_insert
			";
		}
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			$sqlx = "SELECT id_kelengkapan_pemutih,nama_kelengkapan_pemutih FROM kelengkapan_pemutih_penyalur ORDER BY id_kelengkapan_pemutih";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){
				if($_GET['id_cek_pemutih']){
				$sqld = "SELECT id_cek_pemutih,id_kelengkapan_pemutih,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_pemutih_penyalur WHERE id_cek_pemutih='".$_GET['id_cek_pemutih']."' AND id_kelengkapan_pemutih='".$rsx->fields['id_kelengkapan_pemutih']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek Kelengkapan Pemutihan Izin Penyalur";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$pdf->Cell(180,7,'KELENGKAPAN PEMUTIHAN IZIN penyalur','',0,'C');
		$pdf->Ln(5);
...
//print_r($sqlA);
		...
		while(! $hasil->EOF){
			...
			$sqlD = "
			SELECT
			kelengkapan_pemutih_penyalur.nama_kelengkapan_pemutih,
			CASE WHEN detail_cek_pemutih_penyalur.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_pemutih_penyalur.alasan_penilai,
			CASE WHEN detail_cek_pemutih_penyalur.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_pemutih_penyalur.alasan_kaseksi,
			CASE WHEN detail_cek_pemutih_penyalur.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_pemutih_penyalur.alasan_kasubdit,
			CASE WHEN detail_cek_pemutih_penyalur.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_pemutih_penyalur.alasan_direktur
			FROM
			detail_cek_pemutih_penyalur
			LEFT OUTER JOIN kelengkapan_pemutih_penyalur ON(kelengkapan_pemutih_penyalur.id_kelengkapan_pemutih = detail_cek_pemutih_penyalur.id_kelengkapan_pemutih)
			WHERE
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
	...
	function update_cek_pemutih_penyalur() {
		...
		$_block = new block();
		$_block->set_config('title', ('Update Cek Kelengkapan Pemutihan Izin Penyalur Status'));
		$_block->set_config('width', 595);
		if ($result = cek_pemutih_penyalur::get($record)) {
			$record = $result[0];
			...
			eval($this->save_config);
			if (cek_pemutih_penyalur::update($record)) {
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
			...
			$success = $adodb->Execute("delete from detail_cek_pemutih_penyalur where ".$query);
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek Kelengkapan Pemutihan Izin Penyalur Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
		//$sqlk = "SELECT id_Cek_pemutih FROM cek_pemutih_penyalur WHERE nama_penilai
			$sql = "SELECT
			cek_pemutih_penyalur.id_cek_pemutih,
			cek_pemutih_penyalur.no_tt,
			cek_pemutih_penyalur.no_tt as urut_no_tt,
			cek_pemutih_penyalur.no_tt as alamat,
			cek_pemutih_penyalur.no_tt as subdit,
			...
			CASE WHEN cek_pemutih_penyalur.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_pemutih_penyalur.nama_penilai,
			cek_pemutih_penyalur.date_1,
			CASE WHEN cek_pemutih_penyalur.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_pemutih_penyalur.nama_kaseksi,
			cek_pemutih_penyalur.date_2,
			CASE WHEN cek_pemutih_penyalur.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_pemutih_penyalur.nama_kasubdit,
			cek_pemutih_penyalur.date_3,
			CASE WHEN cek_pemutih_penyalur.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_pemutih_penyalur.nama_direktur,
			cek_pemutih_penyalur.date_4,
			cek_pemutih_penyalur.keterangan,
			cek_pemutih_penyalur.insert_by,
			cek_pemutih_penyalur.date_insert,
			cek_pemutih_penyalur.id_cek_pemutih as detail
			FROM cek_pemutih_penyalur
			LEFT OUTER JOIN tt_pemutih_penyalur ON(tt_pemutih_penyalur.no_tt = cek_pemutih_penyalur.no_tt)
			WHERE
			cek_pemutih_penyalur.nama_penilai ='' OR cek_pemutih_penyalur.nama_penilai = '$user'
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_pemutih_penyalur.insert_by,
			cek_pemutih_penyalur.date_insert,
			CASE WHEN cek_pemutih_penyalur.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_pemutih_penyalur.read_kaseksi='0' THEN 'Unread'
			else 'Read' END AS read_kaseksi,
			cek_pemutih_penyalur.id_cek_pemutih as detail
			FROM cek_pemutih_penyalur
			LEFT OUTER JOIN tt_pemutih_penyalur ON(tt_pemutih_penyalur.no_tt = cek_pemutih_penyalur.no_tt)
			WHERE
			cek_pemutih_penyalur.nama_kaseksi ='' OR cek_pemutih_penyalur.nama_kaseksi = '$user'
			ORDER BY cek_pemutih_penyalur.read_kaseksi
			";
		}
		...
		if($bag =='kasubdit'){
			...
			cek_pemutih_penyalur.insert_by,
			cek_pemutih_penyalur.date_insert,
			CASE WHEN cek_pemutih_penyalur.indi_kasubdit ='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_pemutih_penyalur.read_kasubdit='0' THEN 'Unread'
			else 'Read' END AS read_kasubdit,
			cek_pemutih_penyalur.id_cek_pemutih as detail
			FROM cek_pemutih_penyalur
			LEFT OUTER JOIN tt_pemutih_penyalur ON(tt_pemutih_penyalur.no_tt = cek_pemutih_penyalur.no_tt)
			WHERE
			cek_pemutih_penyalur.nama_kasubdit ='' OR cek_pemutih_penyalur.nama_kasubdit = '$user'
			ORDER BY cek_pemutih_penyalur.read_kasubdit
			";
		}
		...
		if($bag =='direktur'){
			...
			cek_pemutih_penyalur.insert_by,
			cek_pemutih_penyalur.date_insert,
			CASE WHEN cek_pemutih_penyalur.indi_direktur ='0' THEN 'Return'
			else 'OK' END AS indi_direktur,
			Case WHEN cek_pemutih_penyalur.read_direktur='0' THEN 'Unread'
			else 'Read' END AS read_direktur,
			cek_pemutih_penyalur.id_cek_pemutih as detail
			FROM cek_pemutih_penyalur
			LEFT OUTER JOIN tt_pemutih_penyalur ON(tt_pemutih_penyalur.no_tt = cek_pemutih_penyalur.no_tt)
			WHERE
			cek_pemutih_penyalur.nama_direktur ='' OR cek_pemutih_penyalur.nama_direktur = '$user'
			ORDER BY cek_pemutih_penyalur.read_direktur
			";
		}
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_pemutih_penyalur WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_pemutih_penyalur WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_pemutih_penyalur WHERE no_tt=\'$f\'");
		...
		$USql = "
			...
			";
			global $adodb;
   			$str .="<table cellspacing=1 cellpadding=1><tr align=center class=title_table><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Kelengkapan Pemutih</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Status Penilai</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Alasan Penilai</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Status Kaseksi</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Alasan Kaseksi</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Status Kasubdit</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Alasan Kasubdit</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Status Direktur</b></td><td style=\'border-width:1px;border-style:solid;border-color:#000000\'><b></font>Alasan Direktur</b></td></tr>";
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek Kelengkapan Pemutihan Izin Penyalur'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Cek Kelengkapan Pemutihan Izin Penyalur Administration';
