...
if (class_exists('cek_1_penyalur_controller')) {
	// do nothing
} else if (defined('CLASS_cek_1_penyalur_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_cek_1_penyalur_CONTROLLER', TRUE);

include_once 'class.cek_1_penyalur.inc.php';
class cek_1_penyalur_controller extends depkes2_manager {
	var $cek_1_penyalur_label;
	var $optional_arr;
	function cek_1_penyalur_controller() {
		$this->cek_1_penyalur_label = array (
			'id_cek_1' => 'Id Check penyalur',
			'lisensi' => 'Lisensi Dari',
			'no_daftar' => 'No Daftar',
			'no_tt' => 'Nama Pabrik ',
			...
			'nama_produk' => 'Jenis Produksi yang Disalurkan',
	...
	function no_tt_form(&$config) {
		...
		if($_GET['id_cek_1']){
			if($bag=='kaseksi'){
				...
				LEFT OUTER JOIN cek_1_penyalur ON(cek_1_penyalur.no_tt=tt_1_penyalur.no_tt)
				LEFT OUTER JOIN pendaftar_penyalur ON(pendaftar_penyalur.kode_pendaftar_penyalur = tt_1_penyalur.kode_pendaftar_penyalur )
				LEFT OUTER JOIN inbox_penyalur_subdit ON(inbox_penyalur_subdit.no_tt = tt_1_penyalur.no_tt )
				LEFT OUTER JOIN inbox_penyalur_seksi ON(inbox_penyalur_seksi.id_inbox_penyalur_subdit = inbox_penyalur_subdit.id_inbox_penyalur_subdit)
				WHERE inbox_penyalur_seksi.insert_by='$userlog'
				ORDER BY
				...
				LEFT OUTER JOIN pendaftar_penyalur ON(pendaftar_penyalur.kode_pendaftar_penyalur = tt_1_penyalur.kode_pendaftar_penyalur )
				LEFT OUTER JOIN inbox_penyalur_subdit ON(inbox_penyalur_subdit.no_tt = tt_1_penyalur.no_tt )
				RIGHT OUTER JOIN inbox_penyalur_seksi ON(inbox_penyalur_seksi.id_inbox_penyalur_subdit = inbox_penyalur_subdit.id_inbox_penyalur_subdit)
				ORDER BY
				tt_1_penyalur.date_insert
				";
			}
			...
			LEFT OUTER JOIN inbox_penyalur_subdit ON(inbox_penyalur_subdit.no_tt = tt_1_penyalur.no_tt )
			RIGHT OUTER JOIN inbox_penyalur_seksi ON(inbox_penyalur_seksi.id_inbox_penyalur_subdit = inbox_penyalur_subdit.id_inbox_penyalur_subdit)
			WHERE ((tt_1_penyalur.no_tt IS NULL) OR (inbox_penyalur_seksi.insert_by='$userlog')) AND cek_1_penyalur.no_tt IS NULL
			ORDER BY
			tt_1_penyalur.date_insert
			";
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_penyalur.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_penyalur LEFT OUTER JOIN tt_1_penyalur ON(tt_1_penyalur.kode_pendaftar_penyalur = pendaftar_penyalur.kode_pendaftar_penyalur) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_penyalur.kode_subdit)  WHERE tt_1_penyalur.no_tt =\''+ b +'\'';
	...
	// create add cek_1_penyalur form
	function add_cek_1_penyalur_form() {
		include_once 'class.xform.inc.php';
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		//print_r($_GET);exit();
		$record = $_GET;
		$label_arr = $this->cek_1_penyalur_label;
		...
		$field_arr[] = xform::xf('lisensi','N','8');
		...
		$rs = $adodb->Execute("SELECT * FROM cek_1_penyalur WHERE id_cek_1='{$record['id_cek_1']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$value_arr['lisensi'] = '<textarea name="lisensi" class="text" rows="3" cols="50">'.$value_arr['lisensi'].'</textarea>';
		...
		global $adodb;
			$sqlx = "SELECT id_kelengkapan_penyalur,nama_kelengkapan_penyalur FROM kelengkapan_penyalur ORDER BY id_kelengkapan_penyalur";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){

				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan_penyalur,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_1_penyalur WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan_penyalur='".$rsx->fields['id_kelengkapan_penyalur']."'";
				//print $sqld;
				
				$rsd = $adodb->Execute($sqld);
				$id_cek_1x = $rsd->fields['id_cek_1'];
				$id_kelengkapanx = $rsd->fields['id_kelengkapan_penyalur'];
				...
				$alasan_direkturx = $rsd->fields['alasan_direktur'];
					if($id_kelengkapanx == $rsx->fields['id_kelengkapan_penyalur']){
				...
				$id_kelengkapan = $rsx->fields['id_kelengkapan_penyalur'];
				$nama_kelengkapan = $rsx->fields['nama_kelengkapan_penyalur'];
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek Kelengkapan Izin Penyalur";
	...
	// create update cek_1_penyalur form
	function update_cek_1_penyalur_form() {
		return $this->add_cek_1_penyalur_form();
	}

	function view_cek_1_penyalur_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = cek_1_penyalur::get_field_set();
		...
		$result = cek_1_penyalur::get($record);
		$value_arr = $result[0];
		$label_arr = $this->cek_1_penyalur_label;
		global $adodb;
		...
		while(! $hasil->EOF){
			...
			FROM
			pendaftar_penyalur
			LEFT OUTER JOIN tt_1_penyalur ON(tt_1_penyalur.kode_pendaftar_penyalur = pendaftar_penyalur.kode_pendaftar_penyalur)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_penyalur.kode_subdit)
			WHERE tt_1_penyalur.no_tt ='".$no_tt."'
			";
			...
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Jenis Produksi penyalur','',0,'L');$pdf->Cell(50,7,': '.$nama_produk.'','',0,'L');
			$pdf->Ln(5);
			...
			$sqlD = "
			SELECT
			kelengkapan_penyalur.nama_kelengkapan_penyalur,
			CASE WHEN detail_cek_1_penyalur.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_1_penyalur.alasan_penilai,
			CASE WHEN detail_cek_1_penyalur.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_1_penyalur.alasan_kaseksi,
			CASE WHEN detail_cek_1_penyalur.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_1_penyalur.alasan_kasubdit,
			CASE WHEN detail_cek_1_penyalur.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_1_penyalur.alasan_direktur
			FROM
			detail_cek_1_penyalur
			LEFT OUTER JOIN kelengkapan_penyalur ON(kelengkapan_penyalur.id_kelengkapan_penyalur = detail_cek_1_penyalur.id_kelengkapan_penyalur)
			WHERE
			...
			while(! $hasilD->EOF){
				...
				$pdf->Cell(35,3,'','TRL',0,'L');
				$pdf->Ln();
				$nama_kelengkapan_penyalur = $hasilD->fields['nama_kelengkapan_penyalur'];
				...
				$pdf->SetFont('Arial','',8);
					...
					$jml_nama_kelengkapan_penyalur = strlen($nama_kelengkapan_penyalur);
					...
					$max = max($jml_nama_kelengkapan_penyalur,$jml_alasan_penilaix,$jml_alasan_kaseksix,$jml_alasan_kasubditx,$jml_alasan_direkturx);
					...
					for($a=0;$a<=$max;$a=$a+1){
						...
						if(($g == 0)&&($a != 0)){
							...
							$nama_kelengkapan_penyalurY .= $nama_kelengkapan_penyalur[$a];
							...
							$pdf->Cell(35,3,''.$nama_kelengkapan_penyalurY.'','LR',0,'L');
	...
	// handle event add cek_1_penyalur
	function add_cek_1_penyalur() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		//foreach ($record as $k => $v) $record[$k] = trim($v);
		$jml_kelengkapan = count($_POST['id_kelengkapan_penyalur']);
		...
		if($read_direktur == 'on'){ $xread_direktur = '1';}else{ $xread_direktur ='0';}
			...
			cek_1_penyalur::add($record);

		$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_1_penyalur ORDER BY id_cek_1 DESC LIMIT 1");
		$id_cek_1 = $rsx->fields['id_cek_1'];

		include "class.detail_cek_1_penyalur.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			$id_kelengkapan_penyalurx = $_POST['id_kelengkapan_penyalur'][$a];
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				$record = array (
					'id_cek_1' => $id_cek_1,
					'id_kelengkapan_penyalur' => $id_kelengkapan_penyalurx,
				...
				detail_cek_1_penyalur::add($record);
		}
	...
	// handle event update cek_1_penyalur
	function update_cek_1_penyalur() {
		//return $this->add_cek_1_penyalur();
		...
		$_block = new block();
		$_block->set_config('title', ('Update Cek Kelengkapan Izin Penyalur Status'));
		$_block->set_config('width', 595);
		if ($result = cek_1_penyalur::get($record)) {
			$record = $result[0];
			...
			eval($this->save_config);
			if (cek_1_penyalur::update($record)) {
				$adodb->Execute("DELETE FROM detail_cek_1_penyalur WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
				...
				for($a=1;$a<=$jml_kelengkapan;$a++){
					...
					$alasan_direkturx = $_POST['alasan_direkturx'][$a];
						...
						$record = array (
							'id_cek_1' => $id_cek_1,
							'id_kelengkapan_penyalur' => $id_kelengkapanx,
							'status_penilai' => $status_penilaix,
							'status_kaseksi' => $status_kaseksix,
							'status_kasubdit' => $status_kasubditx,
							'status_direktur' => $status_direkturx,
	...
	// handle delete cek_1_penyalur
	function delete_cek_1_penyalur() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
					...
					$optional_arr['date_4'] = TRUE;
					$optional_arr['status_direktur'] = TRUE;
                        $success = $adodb->Execute("delete from cek_1_penyalur where ".$query);
			$success = $adodb->Execute("delete from detail_cek_1_penyalur where ".$query);
		}
                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'cek_1_penyalur <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek Kelengkapan Izin Penyalur Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list cek_1_penyalur
	function list_cek_1_penyalur($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
			$sql = "SELECT
			cek_1_penyalur.id_cek_1,
			cek_1_penyalur.no_tt,
			cek_1_penyalur.no_tt as urut_no_tt,
			
			

			cek_1_penyalur.no_tt as alamat,
			cek_1_penyalur.no_tt as subdit,
			cek_1_penyalur.no_rekomendasi,
			cek_1_penyalur.date_rekomendasi,
			cek_1_penyalur.no_bap,
			cek_1_penyalur.date_bap,
			cek_1_penyalur.nama_produk,
			cek_1_penyalur.lisensi,
			CASE WHEN cek_1_penyalur.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_1_penyalur.nama_penilai,
			cek_1_penyalur.date_1,
			CASE WHEN cek_1_penyalur.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_1_penyalur.nama_kaseksi,
			cek_1_penyalur.date_2,
			CASE WHEN cek_1_penyalur.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_1_penyalur.nama_kasubdit,
			cek_1_penyalur.date_3,
			CASE WHEN cek_1_penyalur.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_1_penyalur.nama_direktur,
			cek_1_penyalur.date_4,
			cek_1_penyalur.keterangan,
			cek_1_penyalur.insert_by,
			cek_1_penyalur.date_insert,
			cek_1_penyalur.id_cek_1 as detail
			FROM cek_1_penyalur

			WHERE
			cek_1_penyalur.nama_penilai ='' OR cek_1_penyalur.nama_penilai = '$user'
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_1_penyalur.insert_by,
			cek_1_penyalur.date_insert,
			CASE WHEN cek_1_penyalur.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_1_penyalur.read_kaseksi='0' THEN 'Unread'
			else 'Read' END AS read_kaseksi,
			cek_1_penyalur.id_cek_1 as detail
			FROM cek_1_penyalur

			WHERE
			cek_1_penyalur.nama_kaseksi ='' OR cek_1_penyalur.nama_kaseksi = '$user'
			ORDER BY cek_1_penyalur.read_kaseksi
			";
		}
		...
		if($bag =='kasubdit'){
			...
			cek_1_penyalur.insert_by,
			cek_1_penyalur.date_insert,
			CASE WHEN cek_1_penyalur.indi_kasubdit='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_1_penyalur.read_kasubdit='0' THEN 'Unread'
			else 'Read' END AS read_kasubdit,
			cek_1_penyalur.id_cek_1 as detail
			FROM cek_1_penyalur

			WHERE
			cek_1_penyalur.nama_kasubdit ='' OR cek_1_penyalur.nama_kasubdit='$user'
			ORDER BY cek_1_penyalur.read_kasubdit
			";
		}
		...
		if($bag =='direktur'){
			...
			cek_1_penyalur.insert_by,
			cek_1_penyalur.date_insert,
			CASE WHEN cek_1_penyalur.indi_direktur='0' THEN 'Return'
			...
			WHERE
			cek_1_penyalur.nama_direktur ='' OR cek_1_penyalur.nama_direktur ='$user'
			ORDER BY cek_1_penyalur.read_direktur
			";
		}
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_1_penyalur WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_1_penyalur WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_1_penyalur WHERE no_tt=\'$f\'");
		...
		$USql = "
			SELECT
			kelengkapan_penyalur.nama_kelengkapan_penyalur,
			CASE WHEN detail_cek_1_penyalur.status_penilai =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_penilai,
			detail_cek_1_penyalur.alasan_penilai,
			CASE WHEN detail_cek_1_penyalur.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_1_penyalur.alasan_kaseksi,
			CASE WHEN detail_cek_1_penyalur.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_1_penyalur.alasan_kasubdit,
			CASE WHEN detail_cek_1_penyalur.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_1_penyalur.alasan_direktur
			FROM
			detail_cek_1_penyalur
			LEFT OUTER JOIN kelengkapan_penyalur ON(kelengkapan_penyalur.id_kelengkapan_penyalur = detail_cek_1_penyalur.id_kelengkapan_penyalur)
			WHERE
			id_cek_1=\'%s\' AND detail_cek_1_penyalur.alasan_kaseksi IS NOT NULL
			";
			global $adodb;
			...
			while(! $S->EOF){
			$c= $c+1;
			$FA = $S->fields[\'nama_kelengkapan_penyalur\'];
...
//		if ($this->get_permission('fill_this')) {
		if($bag=='kaseksi'){
			...
			'?action=add' .
			'\', 800, 600, null, null, \'add_cek_1_penyalur\');'.
			'win.focus()',
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 800, 600, null, null, \'edit_cek_1_penyalur\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_cek_1_penyalur\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_cek_1_penyalur\');'.
			'win.focus()',
			"label"=>__('Print'),
			"type"=>"button",
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek Kelengkapan Izin Penyalur'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print cek_1_penyalur
	function print_cek_1_penyalur() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_cek_1_penyalur($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$cek_1_penyalur_controller = new cek_1_penyalur_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_1_penyalur_controller->add_cek_1_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $cek_1_penyalur_controller->add_cek_1_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangeno_tt()"';
			$out_content = $cek_1_penyalur_controller->update_cek_1_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $cek_1_penyalur_controller->update_cek_1_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $cek_1_penyalur_controller->view_cek_1_penyalur_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $cek_1_penyalur_controller->delete_cek_1_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_1_penyalur_controller->import_cek_1_penyalur_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $cek_1_penyalur_controller->import_cek_1_penyalur();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $cek_1_penyalur_controller->print_cek_1_penyalur();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $cek_1_penyalur_controller->list_cek_1_penyalur();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Cek Kelengkapan Izin Penyalur Administration';
