...
if (class_exists('surat_penolakan_controller')) {
	// do nothing
} else if (defined('CLASS_surat_penolakan_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_penolakan_CONTROLLER', TRUE);

include_once 'class.surat_penolakan.inc.php';
class surat_penolakan_controller extends depkes2_manager {
	var $surat_penolakan_label;
	var $optional_arr;
	function surat_penolakan_controller() {
		$this->surat_penolakan_label = array (
			'id_surat_penolakan' => 'id_surat_penolakan',
			'nomor_menteri' => 'NOMOR',
	...
	// create add surat_penolakan form
	function add_surat_penolakan_form() {
		include_once 'class.xform.inc.php';
	
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_penolakan_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM surat_penolakan WHERE id_surat_penolakan='{$record['id_surat_penolakan']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_penolakan'] = 'protect';
			$mode = 'edit';
		...
		if($mode=='add'){
			...
			WHERE
			surat_keputusan.id_cek_1 IS NULL AND surat_penolakan.id_cek_1 IS NULL
			ORDER BY
			...
			LEFT OUTER JOIN pendaftar ON(pendaftar.kode_pendaftar = tt_1_alkes.kode_pendaftar)
			WHERE
			surat_keputusan.id_cek_1 IS NULL
			ORDER BY
				cek_1_alkes.date_insert';
		}
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_penolakan']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Penolakan Registrasi Izin Produksi";
	...
	// create update surat_penolakan form
	function update_surat_penolakan_form() {
		return $this->add_surat_penolakan_form();
	}

	
	function view_surat_penolakan_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_penolakan::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_penolakan'] = 'protect';

		$record = array (
			'id_surat_penolakan' => ${$GLOBALS['get_vars']}['id_surat_penolakan']
		);
		$result = surat_penolakan::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_penolakan_label;
		global $adodb;
		...
		$sqlx = "
		SELECT
		surat_penolakan.id_surat_penolakan,
		surat_penolakan.nomor_surat,
		surat_penolakan.lampiran_surat,
		surat_penolakan.date_surat,
		...
		surat_penolakan.nama,
		surat_penolakan.nip,
		surat_penolakan.insert_by,
		surat_penolakan.date_insert
		FROM surat_penolakan
		LEFT OUTER JOIN cek_1_alkes ON(cek_1_alkes.id_cek_1 = surat_penolakan.id_cek_1)
		LEFT OUTER JOIN tt_1_alkes ON (tt_1_alkes.no_tt = cek_1_alkes.no_tt)
		LEFT OUTER JOIN pendaftar ON( pendaftar.kode_pendaftar = tt_1_alkes.kode_pendaftar)
		LEFT OUTER JOIN surat_tambahan_data ON(surat_tambahan_data.kepada_surat = surat_penolakan.id_cek_1)
		WHERE
		surat_penolakan.id_surat_penolakan ='".$value_arr['id_surat_penolakan']."'
		";
		//print $sqlx;
	...
	// handle event add surat_penolakan
	function add_surat_penolakan() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;

		$rs = $adodb->Execute("SELECT * FROM surat_penolakan WHERE id_surat_penolakan = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM surat_penolakan WHERE id_surat_penolakan = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		$status = "Successfull $st '<b>{$record['id_surat_penolakan']}</b>'";
		$this->log($status);
		$_block = new block();
		$_block->set_config('title', 'Status Surat Penolakan Registrasi Izin Produksi');
	...
	// handle event update surat_penolakan
	function update_surat_penolakan() {
		return $this->add_surat_penolakan();
	}

	// handle delete surat_penolakan
	function delete_surat_penolakan() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_penolakan where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_penolakan <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Penolakan Registrasi Izin Produksi'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_penolakan
	function list_surat_penolakan($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_penolakan' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_penolakan\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_penolakan\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_penolakan\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_penolakan\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button", 
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Penolakan Registrasi Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_penolakan
	function print_surat_penolakan() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_penolakan($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_penolakan_controller = new surat_penolakan_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_penolakan_controller->add_surat_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_penolakan_controller->add_surat_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_penolakan_controller->update_surat_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_penolakan_controller->update_surat_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_penolakan_controller->view_surat_penolakan_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_penolakan_controller->delete_surat_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_penolakan_controller->import_surat_penolakan_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $surat_penolakan_controller->import_surat_penolakan();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_penolakan_controller->print_surat_penolakan();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_penolakan_controller->list_surat_penolakan();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Penolakan Registrasi Izin Produksi Administration';
