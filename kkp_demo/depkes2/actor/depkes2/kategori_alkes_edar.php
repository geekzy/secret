...
class kategori_edar_controller extends depkes2_manager {
	...
	function kategori_edar_controller() {
		$this->kategori_edar_label = array (
			'id_kategori_edar' => 'Id Kategori Edar',
			'nama_kategori_edar' => 'Nama Kategori Edar',
			'insert_by' => 'Insert By',
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Kategori Izin Edar Alkes";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Kategori Izin Edar Alkes Status'));
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Kategori Izin Edar Alkes'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Kategori Izin Edar Alkes Administration';
