...
class cek_kelengkapan_data_pem_alkes_edar_import_controller extends depkes2_manager {
	...
	function cek_kelengkapan_data_pem_alkes_edar_import_controller() {
		$this->cek_kelengkapan_data_pem_alkes_edar_import_label = array (
			...
			'nama_pendaftar_pem_edar_alkes_import' => 'Nama Pendaftar',
	...
	function no_tt_form(&$config) {
		...
		'SELECT
				...
				tt_1_pem_edar_alkes_import.no_tt as svalue,
		...
		$fk_sql = ''.
			...
			LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.no_tt=tt_1_pem_edar_alkes_import.no_tt)
			LEFT OUTER JOIN pendaftar_pem_edar_alkes_import ON(pendaftar_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import = tt_1_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import )
			WHERE cek_kelengkapan_data_pem_alkes_edar_import.no_tt IS NULL
			ORDER BY
				tt_1_pem_edar_alkes_import.date_insert
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_pem_edar_alkes_import.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_pem_edar_alkes_import LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import = pendaftar_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)  WHERE tt_1_pem_edar_alkes_import.no_tt =\''+ b +'\'';

		jumpto1('frame_tt_1_pem_edar_alkes_import.php?sql='+sql+'&a='+b)

		";

	$GLOBALS['out_before_footer'] = '
 	<iframe marginwidth=0 marginheight=0 hspace=0 vspace=0 frameborder=0 scrolling=auto name=\'iframe_entry1\' id=\'iframe_entry1\' style="width:0;height:0" src="frame_tt_1_pem_edar_alkes_import.php"></iframe>
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		global $adodb;
			...
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_pem_alkes_edar_import WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Laporan Izin Pemutihan Edar Alkes Import";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		while(! $hasil->EOF){
			...
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import = pendaftar_pem_edar_alkes_import.kode_pendaftar_pem_edar_alkes_import)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
			...
			WHERE
			id_cek_1='".$id_cek_1."' AND status_penilai IS NOT NULL
			";
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_pem_alkes_edar_import WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		$adodb->Execute("DELETE FROM cek_kelengkapan_data_pem_alkes_edar_import WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		...
		include "class.detail_cek_kelengkapan_data_pem_alkes_edar_import.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_kelengkapan_data_pem_alkes_edar_import::add($record);
		}
		...
		$_block = new block();
		$_block->set_config('title', 'Laporan Izin Pemutihan Edar Alkes Import');
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){
			...
			$success = $adodb->Execute("delete from detail_cek_kelengkapan_data_pem_alkes_edar_import where ".$query);
		}
                if ($success){
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Laporan Izin Pemutihan Edar Alkes Import'));
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		pendaftar_pem_edar_alkes_import.nama_pabrik,
		tt_1_pem_edar_alkes_import.no_tt as status,
		tt_1_pem_edar_alkes_import.no_tt as status_pending,
		tt_1_pem_edar_alkes_import.no_tt as status_tolak,
		tt_1_pem_edar_alkes_import.no_tt as status_selesai,
		...
		$skA = $skA->fields[\'no_surat_keputusan\'];
		if($skA !=""){
		$str .= $skA;
		}else{
		$str .= "&nbsp;";
		...
		$USql = "
			...
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_kaseks,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_direktur
		...
		//PAKE NO_TT BUKAN PAKE ID_CEK_1 JADI SENEN BENERIN YA OK
		$eval_arr['status'] = '
		global $adodb;
		$f = "%s";
		...
		$Aql = "
		SELECT
		surat_keputusan.id_cek_1,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt
		FROM
		surat_keputusan
		LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 = surat_keputusan.id_cek_1)
		WHERE
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt =\'$f\'";
		$rsA = $adodb->Execute($Aql);
		$A = $rsA->fields[no_tt];

		$Bql = "
		SELECT
		surat_penolakan.id_cek_1,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 = surat_penolakan.id_cek_1)
		WHERE
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt =\'$f\'";
		...
		$Cql = "
		SELECT
		surat_tambahan_data.kepada_surat,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 = surat_tambahan_data.kepada_surat)
		WHERE
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt =\'$f\'";
		...
		no_tt
		FROM
		cek_kelengkapan_data_pem_alkes_edar_import
		WHERE no_tt =\'$f\'";
		$rsD = $adodb->Execute($Dql);
		$D = $rsD->fields[no_tt];

		if((!$A) && (!$B) &&(!$C) &&(!$D)){
		 $rsU = $adodb->Execute("select urut_no_tt FROM tt_1_pem_edar_alkes_import WHERE no_tt=\'$f\'");
		...
		SELECT
		surat_tambahan_data.kepada_surat,
		surat_tambahan_data.nomor_surat,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt
		FROM
		surat_tambahan_data
		LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 = surat_tambahan_data.kepada_surat)
		...
		SELECT
		surat_penolakan.id_cek_1,
		surat_penolakan.nomor_surat as nomor_surat_penolakan,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt
		FROM
		surat_penolakan
		LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 = surat_penolakan.id_cek_1)
		...
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt =\'$f\'";
		$rsB = $adodb->Execute($Bql);
		$nomor_surat_penolakan = $rsB->fields[nomor_surat_penolakan];
		$B = $rsB->fields[no_tt];
		...
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt =\'$f\'";
		$rsA = $adodb->Execute($Aql);
		$no_surat_keputusan = $rsA->fields[no_surat_keputusan];
		$A = $rsA->fields[no_tt];
...
//		}
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Laporan Izin Pemutihan Edar Alkes Import'.' - '.date($GLOBALS['date_format']));
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$_title = 'Laporan Izin Pemutihan Edar Alkes Import';
