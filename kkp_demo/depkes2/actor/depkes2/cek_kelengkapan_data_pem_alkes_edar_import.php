...
if (class_exists('cek_kelengkapan_data_pem_alkes_edar_import_controller')) {
	// do nothing
} else if (defined('CLASS_cek_kelengkapan_data_pem_alkes_edar_import_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_cek_kelengkapan_data_pem_alkes_edar_import_CONTROLLER', TRUE);

include_once 'class.cek_kelengkapan_data_pem_alkes_edar_import.inc.php';
class cek_kelengkapan_data_pem_alkes_edar_import_controller extends depkes2_manager {
	var $cek_kelengkapan_data_pem_alkes_edar_import_label;
	var $optional_arr;
	function cek_kelengkapan_data_pem_alkes_edar_import_controller() {
		$this->cek_kelengkapan_data_pem_alkes_edar_import_label = array (
			'id_cek_1' => 'Id Check Alkes',
			'id_golongan' => 'Kelas',
			'no_daftar' => 'No Daftar',
			'id_kategori_pem_alkes_edar' => 'Kategori',
			'id_sub_kategori_pem_alkes_edar' => 'Sub Kategori',
			'no_tt' => 'Nama Pabrik ',
	...
	function no_tt_form(&$config) {
		...
		include_once 'class.pendaftar_pem_alkes_edar_alkes_import.inc.php';
		if($_GET['id_cek_1']){
			if($bag=='kaseksi'){
				$fk_sql =
				"SELECT
					tt_1_pem_edar_alkes_import.no_tt as skey,
					tt_1_pem_edar_alkes_import.urut_no_tt as svalue,
					pendaftar_pem_alkes_edar_alkes_import.nama_pabrik as svalue2
				FROM
				tt_1_pem_edar_alkes_import
				LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
				LEFT OUTER JOIN cek_kelengkapan_data_pem_alkes_edar_import ON(cek_kelengkapan_data_pem_alkes_edar_import.no_tt=tt_1_pem_edar_alkes_import.no_tt)
				LEFT OUTER JOIN pendaftar_pem_alkes_edar_alkes_import ON(pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import )
				LEFT OUTER JOIN inbox_izin_alkes_import_subdit ON(inbox_izin_alkes_import_subdit.no_tt = tt_1_pem_edar_alkes_import.no_tt )
				LEFT OUTER JOIN inbox_izin_alkes_import_seksi ON(inbox_izin_alkes_import_seksi.id_inbox_izin_alkes_import_subdit = inbox_izin_alkes_import_subdit.id_inbox_izin_alkes_import_subdit)
				WHERE inbox_izin_alkes_import_seksi.insert_by='$userlog'
				ORDER BY
				...
				LEFT OUTER JOIN pendaftar_pem_alkes_edar_alkes_import ON(pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import )
				LEFT OUTER JOIN inbox_izin_alkes_import_subdit ON(inbox_izin_alkes_import_subdit.no_tt = tt_1_pem_edar_alkes_import.no_tt )
				RIGHT OUTER JOIN inbox_izin_alkes_import_seksi ON(inbox_izin_alkes_import_seksi.id_inbox_izin_alkes_import_subdit = inbox_izin_alkes_import_subdit.id_inbox_izin_alkes_import_subdit)
				ORDER BY
				tt_1_pem_edar_alkes_import.date_insert
				";
			}
			...
			LEFT OUTER JOIN inbox_izin_alkes_import_subdit ON(inbox_izin_alkes_import_subdit.no_tt = tt_1_pem_edar_alkes_import.no_tt )
			RIGHT OUTER JOIN inbox_izin_alkes_import_seksi ON(inbox_izin_alkes_import_seksi.id_inbox_izin_alkes_import_subdit = inbox_izin_alkes_import_subdit.id_inbox_izin_alkes_import_subdit)
			WHERE ((tt_1_pem_edar_alkes_import.no_tt IS NULL) OR (inbox_izin_alkes_import_seksi.insert_by='$userlog')) AND cek_kelengkapan_data_pem_alkes_edar_import.no_tt IS NULL
			ORDER BY
			tt_1_pem_edar_alkes_import.date_insert
			";
		...
		$result = pendaftar_pem_alkes_edar_alkes_import::select($fk_sql);
  ...
  $jas ="
		...
		sql = 'SELECT pendaftar_pem_alkes_edar_alkes_import.alamat_pabrik,subdit.subdit as nama_propinsi_2 FROM pendaftar_pem_alkes_edar_alkes_import LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import) LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)  WHERE tt_1_pem_edar_alkes_import.no_tt =\''+ b +'\'';
	...
	function id_kategori_pem_alkes_edar_form(&$config) {
                eval($this->load_config);
                $selected = $value_arr['id_kategori_pem_alkes_edar'];

		include_once 'class.kategori_pem_alkes_edar.inc.php';
		$fk_sql = ''.
			'SELECT
				id_kategori_pem_alkes_edar as skey,
				nama_kategori_pem_alkes_edar as svalue2
			FROM
				kategori_pem_alkes_edar
			ORDER BY
				id_kategori_pem_alkes_edar
			';
		$result = kategori_pem_alkes_edar::select($fk_sql);
		$opener_sql = htmlentities($fk_sql);
		$opener_sql = str_replace("\n", " ", $opener_sql);
		$opener_var = htmlentities('id_kategori_pem_alkes_edar');
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->sub_kategori_pem_alkes_edar_label['id_kategori_pem_alkes_edar']
			)
		);
		$result = array_merge($default_value, $result);
		$optional_arr['id_kategori_pem_alkes_edar'] = 'user_defined';
		$value_arr['id_kategori_pem_alkes_edar'] = $this->select_form('id_kategori_pem_alkes_edar', $result, $selected);
		$optional_arr['id_kategori_pem_alkes_edar_rule'] = "\n".
		"       if(theform.id_kategori_pem_alkes_edar.value == '')\n".
		"       {\n".
		"               alert('Field ".$label_arr['id_kategori_pem_alkes_edar']." ".__('empty').".');\n".
		"               theform.id_kategori_pem_alkes_edar.focus();\n".
		"               form_submitted=false;\n".
		"               return false;\n".
		"       }\n";
	...
	function id_sub_kategori_pem_alkes_edar_form(&$config) {
                eval($this->load_config);
                $selected = $value_arr['id_sub_kategori_pem_alkes_edar'];

		include_once 'class.kategori_pem_alkes_edar.inc.php';
		$fk_sql = ''.
			'SELECT
				id_sub_kategori_pem_alkes_edar as skey,
				nama_sub_kategori_pem_alkes_edar as svalue2
			FROM
				sub_kategori_pem_alkes_edar
			ORDER BY
				id_sub_kategori_pem_alkes_edar
			';
		$result = kategori_pem_alkes_edar::select($fk_sql);
		$opener_sql = htmlentities($fk_sql);
		$opener_sql = str_replace("\n", " ", $opener_sql);
		$opener_var = htmlentities('id_sub_kategori_pem_alkes_edar');
		$default_value = array(
			array (
				'skey' => '',
				'svalue' => __('Choose').' '.$this->sub_kategori_pem_alkes_edar_label['id_sub_kategori_pem_alkes_edar']
			)
		);
		$result = array_merge($default_value, $result);
		$optional_arr['id_sub_kategori_pem_alkes_edar'] = 'user_defined';
		$value_arr['id_sub_kategori_pem_alkes_edar'] = $this->select_form('id_sub_kategori_pem_alkes_edar', $result, $selected);
		$optional_arr['id_sub_kategori_pem_alkes_edar_rule'] = "\n".
		"       if(theform.id_sub_kategori_pem_alkes_edar.value == '')\n".
		"       {\n".
		"               alert('Field ".$label_arr['id_sub_kategori_pem_alkes_edar']." ".__('empty').".');\n".
		"               theform.id_sub_kategori_pem_alkes_edar.focus();\n".
		"               form_submitted=false;\n".
		"               return false;\n".
		"       }\n";
	...
	// create add cek_kelengkapan_data_pem_alkes_edar_import form
	function add_cek_kelengkapan_data_pem_alkes_edar_import_form() {
		include_once 'class.xform.inc.php';
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		//print_r($_GET);exit();
		$record = $_GET;
		$label_arr = $this->cek_kelengkapan_data_pem_alkes_edar_import_label;
		...
		$field_arr[] = xform::xf('id_kategori_pem_alkes_edar','C','32');
		$field_arr[] = xform::xf('id_sub_kategori_pem_alkes_edar','N','8');
		...
		$rs = $adodb->Execute("SELECT * FROM cek_kelengkapan_data_pem_alkes_edar_import WHERE id_cek_1='{$record['id_cek_1']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
		...
		$optional_arr['dapendaftar_pem_alkes_edar_alkes_importte_1_rule'] = '';
		...
		$this->id_kategori_pem_alkes_edar_form($config);
		$this->id_sub_kategori_pem_alkes_edar_form($config);
		...
		global $adodb;
			$sqlx = "SELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_admin_import ORDER BY id_kelengkapan";
			$rsx = $adodb->Execute($sqlx);
			$a=1;
			while(! $rsx->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_admin WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx->fields['id_kelengkapan']."'";
		...
		global $adodb;
			$sqlx2 = "SELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_teknis_import ORDER BY id_kelengkapan";
			$rsx2 = $adodb->Execute($sqlx2);
			$a=1;
			while(! $rsx2->EOF){
				if($_GET['id_cek_1']){
				$sqld = "SELECT id_cek_1,id_kelengkapan,status_penilai,status_kaseksi,status_kasubdit,status_direktur,alasan_penilai,alasan_kaseksi,alasan_kasubdit,alasan_direktur FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis WHERE id_cek_1='".$_GET['id_cek_1']."' AND id_kelengkapan='".$rsx2->fields['id_kelengkapan']."'";
				...
				$has2 .= "<td><input type=checkbox name=status_penilaix2[".$a."] value=1 class=tex2t ".$check1."></td></td><td><textarea name=alasan_penilaix2[".$a."] class='text'>".$alasan_penilaix2."</textarea></td>
					...
					$has2 .= "<td><input type=checkbox name=status_kaseksix2[".$a."] value=1 class=tex2t ".$check2."></td><td><textarea name=alasan_kaseksix2[".$a."] class='text'>".$alasan_kaseksix2."</textarea>
						...
						<input type=hidden name=alasan_kasubditx2[".$a."] value='".$alasan_kasubditx2."'>
						<input type=hidden name=status_direkturx2[".$a."] value='".$status_direkturx2."'>
						<input type=hidden name=alasan_direktuSELECT id_kelengkapan,nama_kelengkapan FROM kelengkapan_teknis_import ORDER BY id_kelengkapanrx2[".$a."] value='".$alasan_direkturx2."'>
					</td>";
					}else{
						if($ses->action == 'kasubdit'){
						if($check1 == "checked"){$has2 .= "<td><input type=hidden name=status_penilaix2[".$a."] value='".$status_penilaix2."'>".$tex2t1.",<br><input type=hidden name=alasan_penilaix2[".$a."] class='text' value='".$alasan_penilaix2."'>".$alasan_penilaix2."</td>";}else{ $has2 .= "<td><input type=hidden name=alasan_penilaix2[".$a."] class='text' value='".$alasan_penilaix2."'><nobr>&nbsp;".$tex2t1.",</nobr><br><nobr>".$alasan_penilaix2."</nobr></td>";}
		...
		/*$optional_arr['id_golongan'] = 'user_defined';
                ...
                $arr['sql'] = 'SELECT id_golongan as val, golongan as txt FROM gol_pem_alkes_edar';
		...
		$label_arr['form_title'] = "Form ".ucwords($mode)." Cek Kelengkapan Izin Pemutihan Alkes Edar Import";
	...
	// create update cek_kelengkapan_data_pem_alkes_edar_import form
	function update_cek_kelengkapan_data_pem_alkes_edar_import_form() {
		return $this->add_cek_kelengkapan_data_pem_alkes_edar_import_form();
	}

	function view_cek_kelengkapan_data_pem_alkes_edar_import_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = cek_kelengkapan_data_pem_alkes_edar_import::get_field_set();
		...
		$result = cek_kelengkapan_data_pem_alkes_edar_import::get($record);
		$value_arr = $result[0];
		$label_arr = $this->cek_kelengkapan_data_pem_alkes_edar_import_label;
		global $adodb;
		...
		$pdf->Ln(5);
		$sqlA = "SELECT
		cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1,
		cek_kelengkapan_data_pem_alkes_edar_import.no_tt,
		cek_kelengkapan_data_pem_alkes_edar_import.kemasan,
		cek_kelengkapan_data_pem_alkes_edar_import.id_golongan,
		cek_kelengkapan_data_pem_alkes_edar_import.ukuran,
		kategori_pem_alkes_edar.nama_kategori_pem_alkes_edar,
		sub_kategori_pem_alkes_edar.nama_sub_kategori_pem_alkes_edar,
		cek_kelengkapan_data_pem_alkes_edar_import.nama,
		gol_pem_alkes_edar.golongan as kelas,
		cek_kelengkapan_data_pem_alkes_edar_import.jenis,
		cek_kelengkapan_data_pem_alkes_edar_import.no_pemohon,
		cek_kelengkapan_data_pem_alkes_edar_import.nama_produk,
		cek_kelengkapan_data_pem_alkes_edar_import.id_golongan as golongan,
		CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_penilai ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_penilai,
		cek_kelengkapan_data_pem_alkes_edar_import.nama_penilai,
		cek_kelengkapan_data_pem_alkes_edar_import.date_1,
		CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_kaseksi ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_kaseksi,
		cek_kelengkapan_data_pem_alkes_edar_import.nama_kaseksi,
		cek_kelengkapan_data_pem_alkes_edar_import.date_2,
		CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_kasubdit ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_kasubdit,
		cek_kelengkapan_data_pem_alkes_edar_import.nama_kasubdit,
		cek_kelengkapan_data_pem_alkes_edar_import.date_3,
		CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_direktur ='0' THEN 'Sudah Lengkap'
				else 'Belum Lengkap'
		END AS status_direktur,
		cek_kelengkapan_data_pem_alkes_edar_import.nama_direktur,
		cek_kelengkapan_data_pem_alkes_edar_import.date_4,
		cek_kelengkapan_data_pem_alkes_edar_import.keterangan,
		cek_kelengkapan_data_pem_alkes_edar_import.insert_by,
		cek_kelengkapan_data_pem_alkes_edar_import.date_insert
		FROM cek_kelengkapan_data_pem_alkes_edar_import
		LEFT OUTER JOIN kategori_pem_alkes_edar ON(kategori_pem_alkes_edar.id_kategori_pem_alkes_edar = cek_kelengkapan_data_pem_alkes_edar_import.id_kategori_pem_alkes_edar)
		LEFT OUTER JOIN sub_kategori_pem_alkes_edar ON(sub_kategori_pem_alkes_edar.id_sub_kategori_pem_alkes_edar = cek_kelengkapan_data_pem_alkes_edar_import.id_sub_kategori_pem_alkes_edar)
		LEFT OUTER JOIN gol_pem_alkes_edar ON(gol_pem_alkes_edar.id_golongan = cek_kelengkapan_data_pem_alkes_edar_import.id_golongan)
		WHERE
		cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 ='".$value_arr['id_cek_1']."'
		";
		...
		while(! $hasil->EOF){
			...
			$kategori_pem_alkes_edar = $hasil->fields['nama_kategori_pem_alkes_edar'];
			$sub_kategori_pem_alkes_edar = $hasil->fields['nama_sub_kategori_pem_alkes_edar'];
			...
			pendaftar_pem_alkes_edar_alkes_import.alamat_pabrik,
			pendaftar_pem_alkes_edar_alkes_import.nama_pabrik
			FROM
			pendaftar_pem_alkes_edar_alkes_import
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import)
			LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
			WHERE tt_1_pem_edar_alkes_import.no_tt ='".$no_tt."'
			";
			...
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Kategori','',0,'L');$pdf->Cell(50,7,': '.$kategori_pem_alkes_edar.'','',0,'L');
			$pdf->Ln(5);
			$pdf->Cell(50,7,'Sub Kategori','',0,'L');$pdf->Cell(50,7,': '.$sub_kategori_pem_alkes_edar.'','',0,'L');
			$pdf->Ln(5);
			...
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.id_kelengkapan)
			WHERE
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.id_cek_1='".$id_cek_1."'
			";
			...
			SELECT
			kelengkapan_alkes.nama_kelengkapan,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_penilai ='1' THEN 'Ada' else 'Tidak' END AS status_penilai,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_kaseksi ='1' THEN 'Ada' else 'Tidak' END AS status_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_kasubdit ='1' THEN 'Ada' else 'Tidak' END AS status_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_direktur ='1' THEN 'Ada' else 'Tidak' END AS status_direktur,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.id_kelengkapan)
			WHERE
			detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.id_cek_1='".$id_cek_1."'
			GROUP BY kelengkapan_alkes.nama_kelengkapan,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_penilai,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_penilai,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.status_direktur,
			detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.alasan_direktur
			";
	...
	// handle event add cek_kelengkapan_data_pem_alkes_edar_import
	function add_cek_kelengkapan_data_pem_alkes_edar_import() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($read_direktur == 'on'){ $xread_direktur = '1';}else{ $xread_direktur ='0';}
			...
			$record = array (
				...
				'id_kategori_pem_alkes_edar' => $_POST['id_kategori_pem_alkes_edar'],
				'id_sub_kategori_pem_alkes_edar' => $_POST['id_sub_kategori_pem_alkes_edar'],
				'nama' => $_POST['nama'],
				'jenis' => $_POST['jenis'],
				'no_tt' => $_POST['no_tt'],
			...
			cek_kelengkapan_data_pem_alkes_edar_import::add($record);

		$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_kelengkapan_data_pem_alkes_edar_import ORDER BY id_cek_1 DESC LIMIT 1");
		$id_cek_1 = $rsx->fields['id_cek_1'];

		include "class.detail_cek_kelengkapan_data_pem_alkes_edar_import_admin.inc.php";
		for($a=1;$a<=$jml_kelengkapan;$a++){
			...
			$alasan_direkturx = $_POST['alasan_direkturx'][$a];
				...
				detail_cek_kelengkapan_data_pem_alkes_edar_import_admin::add($record);
		}
		
		
		

		include "class.detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis.inc.php";
		for($a=1;$a<=$jml_kelengkapan2;$a++){
			...
			$alasan_direkturx2 = $_POST['alasan_direkturx2'][$a];
				...
				detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis::add($record);
		}
	...
	// handle event update cek_kelengkapan_data_pem_alkes_edar_import
	function update_cek_kelengkapan_data_pem_alkes_edar_import() {
		//return $this->add_cek_kelengkapan_data_pem_alkes_edar_import();
		...
		$_block = new block();
		$_block->set_config('title', ('Update Cek Kelengkapan Izin Pemutihan Alkes Edar Import Status'));
		$_block->set_config('width', 595);
		if ($result = cek_kelengkapan_data_pem_alkes_edar_import::get($record)) {
			$record = $result[0];
			...
			if (${$GLOBALS['post_vars']}['id_kategori_pem_alkes_edar']!='') $record['id_kategori_pem_alkes_edar'] = $_POST['id_kategori_pem_alkes_edar'];
			if (${$GLOBALS['post_vars']}['id_sub_kategori_pem_alkes_edar']!='') $record['id_sub_kategori_pem_alkes_edar'] = $_POST['id_sub_kategori_pem_alkes_edar'];
			...
			eval($this->save_config);
			if (cek_kelengkapan_data_pem_alkes_edar_import::update($record)) {
				$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_admin WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
				$adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
				//$rsx = $adodb->Execute("SELECT id_cek_1 FROM cek_kelengkapan_data_pem_alkes_edar_import ORDER BY id_cek_1 DESC LIMIT 1");
	...
	// handle delete cek_kelengkapan_data_pem_alkes_edar_import
	function delete_cek_kelengkapan_data_pem_alkes_edar_import() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query){$optional_arr['nama_penilai'] = TRUE;
					...
					$optional_arr['date_4'] = TRUE;
					$optional_arr['status_direktur'] = TRUE;
            $success = $adodb->Execute("delete from cek_kelengkapan_data_pem_alkes_edar_import where ".$query);
            $success .= $adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_admin WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
            $success .= $adodb->Execute("DELETE FROM detail_cek_kelengkapan_data_pem_alkes_edar_import_teknis WHERE id_cek_1 = '".$_POST['oldpkvalue']."'");
		}
                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'cek_kelengkapan_data_pem_alkes_edar_import <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Cek Kelengkapan Izin Pemutihan Alkes Edar Import Status'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list cek_kelengkapan_data_pem_alkes_edar_import
	function list_cek_kelengkapan_data_pem_alkes_edar_import($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if($bag =='penilai'){
			...
			cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1,
			cek_kelengkapan_data_pem_alkes_edar_import.no_tt,
			cek_kelengkapan_data_pem_alkes_edar_import.no_tt as urut_no_tt,
			cek_kelengkapan_data_pem_alkes_edar_import.no_tt as alamat,
			cek_kelengkapan_data_pem_alkes_edar_import.no_tt as subdit,
			cek_kelengkapan_data_pem_alkes_edar_import.no_pemohon,
			cek_kelengkapan_data_pem_alkes_edar_import.kemasan,
			cek_kelengkapan_data_pem_alkes_edar_import.ukuran,
			cek_kelengkapan_data_pem_alkes_edar_import.nama_produk,
			cek_kelengkapan_data_pem_alkes_edar_import.id_golongan as golongan,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_penilai ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_penilai,
			cek_kelengkapan_data_pem_alkes_edar_import.nama_penilai,
			cek_kelengkapan_data_pem_alkes_edar_import.date_1,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_kaseksi ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kaseksi,
			cek_kelengkapan_data_pem_alkes_edar_import.nama_kaseksi,
			cek_kelengkapan_data_pem_alkes_edar_import.date_2,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_kasubdit ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_kasubdit,
			cek_kelengkapan_data_pem_alkes_edar_import.nama_kasubdit,
			cek_kelengkapan_data_pem_alkes_edar_import.date_3,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.status_direktur ='0' THEN 'Lengkap'
					else 'Belum Lengkap'
			END AS status_direktur,
			cek_kelengkapan_data_pem_alkes_edar_import.nama_direktur,
			cek_kelengkapan_data_pem_alkes_edar_import.date_4,
			...
			cek_kelengkapan_data_pem_alkes_edar_import.insert_by,
			cek_kelengkapan_data_pem_alkes_edar_import.date_insert,
			Case WHEN cek_kelengkapan_data_pem_alkes_edar_import.read_penilai='0' THEN 'Unread'
			else 'Read' END AS read_penilai,
			cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 as detail
			FROM cek_kelengkapan_data_pem_alkes_edar_import
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.no_tt = cek_kelengkapan_data_pem_alkes_edar_import.no_tt)
			WHERE
			cek_kelengkapan_data_pem_alkes_edar_import.nama_penilai ='' OR cek_kelengkapan_data_pem_alkes_edar_import.nama_penilai = '$user'
			";
		}
		...
		if($bag =='kaseksi'){
			...
			cek_kelengkapan_data_pem_alkes_edar_import.insert_by,
			cek_kelengkapan_data_pem_alkes_edar_import.date_insert,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.indi_kaseksi='0' THEN 'Return'
			else 'OK' END AS indi_kaseksi,
			Case WHEN cek_kelengkapan_data_pem_alkes_edar_import.read_kaseksi='0' THEN 'Unread'
			...
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.no_tt = cek_kelengkapan_data_pem_alkes_edar_import.no_tt)
			WHERE
			cek_kelengkapan_data_pem_alkes_edar_import.nama_kaseksi ='' OR cek_kelengkapan_data_pem_alkes_edar_import.nama_kaseksi = '$user'
			ORDER BY cek_kelengkapan_data_pem_alkes_edar_import.read_kaseksi
			";
		}
		...
		if($bag =='kasubdit'){
			...
			cek_kelengkapan_data_pem_alkes_edar_import.insert_by,
			cek_kelengkapan_data_pem_alkes_edar_import.date_insert,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.indi_kasubdit='0' THEN 'Return'
			else 'OK' END AS indi_kasubdit,
			Case WHEN cek_kelengkapan_data_pem_alkes_edar_import.read_kasubdit='0' THEN 'Unread'
			else 'Read' END AS read_kasubdit,
			cek_kelengkapan_data_pem_alkes_edar_import.id_cek_1 as detail
			FROM cek_kelengkapan_data_pem_alkes_edar_import
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.no_tt = cek_kelengkapan_data_pem_alkes_edar_import.no_tt)
			WHERE
			cek_kelengkapan_data_pem_alkes_edar_import.nama_kasubdit ='' OR cek_kelengkapan_data_pem_alkes_edar_import.nama_kasubdit='$user'
			ORDER BY cek_kelengkapan_data_pem_alkes_edar_import.read_kasubdit
			";
		}
		...
		if($bag =='direktur'){
			...
			cek_kelengkapan_data_pem_alkes_edar_import.insert_by,
			cek_kelengkapan_data_pem_alkes_edar_import.date_insert,
			CASE WHEN cek_kelengkapan_data_pem_alkes_edar_import.indi_direktur='0' THEN 'Return'
			...
			LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.no_tt = cek_kelengkapan_data_pem_alkes_edar_import.no_tt)
			WHERE
			cek_kelengkapan_data_pem_alkes_edar_import.nama_direktur ='' OR cek_kelengkapan_data_pem_alkes_edar_import.nama_direktur ='$user'
			ORDER BY cek_kelengkapan_data_pem_alkes_edar_import.read_direktur
			";
		}
		...
		LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
		WHERE tt_1_pem_edar_alkes_import.no_tt =\'$f\'";
		...
		if($bag =='kaseksi'){
			...
			$rsh=$adodb->Execute("SELECT indi_kaseksi,read_kaseksi FROM cek_kelengkapan_data_pem_alkes_edar_import WHERE no_tt=\'$f\'");
		...
		if($bag =='kasubdit'){
			...
			$rsh=$adodb->Execute("SELECT indi_kasubdit,read_kasubdit FROM cek_kelengkapan_data_pem_alkes_edar_import WHERE no_tt=\'$f\'");
		...
		if($bag =='direktur'){
			...
			$rsh=$adodb->Execute("SELECT indi_direktur,read_direktur FROM cek_kelengkapan_data_pem_alkes_edar_import WHERE no_tt=\'$f\'");
		...
		LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
		WHERE tt_1_pem_edar_alkes_import.no_tt =\'$b\'";
		...
		LEFT OUTER JOIN tt_1_pem_edar_alkes_import ON(tt_1_pem_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import = pendaftar_pem_alkes_edar_alkes_import.kode_pendaftar_pem_alkes_edar_alkes_import)
		LEFT OUTER JOIN subdit ON(subdit.id_subdit=tt_1_pem_edar_alkes_import.kode_subdit)
		WHERE tt_1_pem_edar_alkes_import.no_tt =\'$c\'";
		...
		$USql = "
			SELECT
			kelengkapan_alkes.nama_kelengkapan,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_penilai =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_penilai,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_penilai,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_kaseksi =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kaseksi,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_kaseksi,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_kasubdit =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_kasubdit,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_kasubdit,
			CASE WHEN detail_cek_kelengkapan_data_pem_alkes_edar_import.status_direktur =\'1\' THEN \'Lengkap\' else \'Belum Lengkap\' END AS status_direktur,
			detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_direktur
			FROM
			detail_cek_kelengkapan_data_pem_alkes_edar_import
			LEFT OUTER JOIN kelengkapan_alkes ON(kelengkapan_alkes.id_kelengkapan = detail_cek_kelengkapan_data_pem_alkes_edar_import.id_kelengkapan)
			WHERE
			id_cek_1=\'%s\' AND detail_cek_kelengkapan_data_pem_alkes_edar_import.alasan_kaseksi IS NOT NULL
			";
			global $adodb;
...
//		if ($this->get_permission('fill_this')) {
  		if($bag=='kaseksi'){
			...
			'?action=add' .
			'\', 800, 600, null, null, \'add_cek_kelengkapan_data_pem_alkes_edar_import\');'.
			'win.focus()',
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 800, 600, null, null, \'edit_cek_kelengkapan_data_pem_alkes_edar_import\');'.
			'win.focus()',
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' .
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_cek_kelengkapan_data_pem_alkes_edar_import\')'.
			'win.focus()):' .
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_cek_kelengkapan_data_pem_alkes_edar_import\');'.
			'win.focus()',
			"label"=>__('Print'),
			"type"=>"button",
		...
		$label_arr = $this->cek_kelengkapan_data_pem_alkes_edar_import_label;
		$config = array (
			'id'		=> 'cek_kelengkapan_data_pem_alkes_edar_import',
			'db'		=> &$GLOBALS['adodb'],
			'optional_arr'	=> $optional_arr,
			'label_arr'	=> $label_arr,
			'vsel_arr'	=> $vsel_arr,
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Cek Kelengkapan Izin Pemutihan Alkes Edar Import'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print cek_kelengkapan_data_pem_alkes_edar_import
	function print_cek_kelengkapan_data_pem_alkes_edar_import() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_cek_kelengkapan_data_pem_alkes_edar_import($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$cek_kelengkapan_data_pem_alkes_edar_import_controller = new cek_kelengkapan_data_pem_alkes_edar_import_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->add_cek_kelengkapan_data_pem_alkes_edar_import_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->add_cek_kelengkapan_data_pem_alkes_edar_import();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad();OnChangeno_tt()"';
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->update_cek_kelengkapan_data_pem_alkes_edar_import_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->update_cek_kelengkapan_data_pem_alkes_edar_import();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->view_cek_kelengkapan_data_pem_alkes_edar_import_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->delete_cek_kelengkapan_data_pem_alkes_edar_import();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->import_cek_kelengkapan_data_pem_alkes_edar_import_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->import_cek_kelengkapan_data_pem_alkes_edar_import();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->print_cek_kelengkapan_data_pem_alkes_edar_import();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $cek_kelengkapan_data_pem_alkes_edar_import_controller->list_cek_kelengkapan_data_pem_alkes_edar_import();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Cek Kelengkapan Izin Pemutihan Alkes Edar Import Administration';
