...
if (class_exists('surat_ubah_tambahan_data_controller')) {
	// do nothing
} else if (defined('CLASS_surat_ubah_tambahan_data_CONTROLLER')) {
	// do nothing
} else {
	define('CLASS_surat_ubah_tambahan_data_CONTROLLER', TRUE);

include_once 'class.surat_ubah_tambahan_data.inc.php';
class surat_ubah_tambahan_data_controller extends depkes2_manager {
	var $surat_ubah_tambahan_data_label;
	var $optional_arr;
	function surat_ubah_tambahan_data_controller() {
		$this->surat_ubah_tambahan_data_label = array (
			'id_surat_ubah_tambahan_data' => 'id_surat_ubah_tambahan_data',
			'nomor_menteri' => 'NOMOR',
		...
		$this->optional_arr = array (
			'id_surat_ubah_tambahan_data' => FALSE,
			'passwd' => FALSE,
			'name' => FALSE,
			'insert_by' => TRUE,
	...
	// create add surat_ubah_tambahan_data form
	function add_surat_ubah_tambahan_data_form() {
		include_once 'class.xform.inc.php';

//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_SESSION, $_GET, $adodb, $ses;
		$record = $_GET;
		$label_arr = $this->surat_ubah_tambahan_data_label;
...
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		$rs = $adodb->Execute("SELECT * FROM surat_ubah_tambahan_data WHERE id_surat_ubah_tambahan_data='{$record['id_surat_ubah_tambahan_data']}'");
		if ($rs && ! $rs->EOF) {
			$value_arr = $rs->fields;
			$optional_arr['id_surat_ubah_tambahan_data'] = 'protect';
			$mode = 'edit';
		...
		$optional_arr['kepada_surat'] = 'user_defined';
                ...
                $arr['sql'] = 'SELECT
				cek_2_alkes.id_cek_2 as val,
    				pendaftar.nama_pabrik as txt
			FROM
				cek_2_alkes
			LEFT OUTER JOIN surat_ubah_tambahan_data ON(surat_ubah_tambahan_data.kepada_surat = cek_2_alkes.id_cek_2)
		...
		$label_arr['form_extra'] .= "<input type=hidden name=oldpkvalue value='{$record['id_surat_ubah_tambahan_data']}'>";
		$label_arr['form_title'] = "Form ".ucwords($mode)." Surat Tambahan Data Perubahan Izin Produksi";
	...
	// create update surat_ubah_tambahan_data form
	function update_surat_ubah_tambahan_data_form() {
		return $this->add_surat_ubah_tambahan_data_form();
	}

	
	function view_surat_ubah_tambahan_data_form() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global ${$GLOBALS['session_vars']}, ${$GLOBALS['get_vars']}, $adodb;
		$field_arr = surat_ubah_tambahan_data::get_field_set();

		$optional_arr = $this->optional_arr;
		$optional_arr['id_surat_ubah_tambahan_data'] = 'protect';

		$record = array (
			'id_surat_ubah_tambahan_data' => ${$GLOBALS['get_vars']}['id_surat_ubah_tambahan_data']
		);
		$result = surat_ubah_tambahan_data::get($record);
		$value_arr = $result[0];
		$label_arr = $this->surat_ubah_tambahan_data_label;
		global $adodb;
		...
		$sqlx = "
		SELECT
		surat_ubah_tambahan_data.nomor_surat,
		surat_ubah_tambahan_data.lampiran_surat,
		surat_ubah_tambahan_data.date_surat,
		pendaftar.nama_pabrik,
		pendaftar.alamat_pabrik,
		surat_ubah_tambahan_data.alat,
		surat_ubah_tambahan_data.isi_1,
		surat_ubah_tambahan_data.isi_2,
		surat_ubah_tambahan_data.isi_3,
		surat_ubah_tambahan_data.isi_4,
		surat_ubah_tambahan_data.isi_5,
		surat_ubah_tambahan_data.nama,
		surat_ubah_tambahan_data.nip,
		surat_ubah_tambahan_data.insert_by,
		surat_ubah_tambahan_data.date_insert
		FROM surat_ubah_tambahan_data
		LEFT OUTER JOIN cek_2_alkes ON(cek_2_alkes.id_cek_2 = surat_ubah_tambahan_data.kepada_surat)
		LEFT OUTER JOIN tt_2_alkes ON(tt_2_alkes.no_tt = cek_2_alkes.no_tt)
		LEFT OUTER JOIN pemohon_ubah_ijin_produksi ON(pemohon_ubah_ijin_produksi.kode_pemohon_ubah_ijin_produksi = tt_2_alkes.kode_pendaftar)
		LEFT OUTER JOIN surat_keputusan ON(surat_keputusan.no_surat_keputusan = pemohon_ubah_ijin_produksi.no_surat_keputusan )
		LEFT OUTER JOIN cek_1_alkes ON(cek_1_alkes.id_cek_1 = surat_keputusan.id_cek_1)
		...
		LEFT OUTER JOIN pendaftar ON(pendaftar.kode_pendaftar = tt_1_alkes.kode_pendaftar)
		WHERE
		surat_ubah_tambahan_data.id_surat_ubah_tambahan_data ='".$value_arr['id_surat_ubah_tambahan_data']."'
		";
	...
	// handle event add surat_ubah_tambahan_data
	function add_surat_ubah_tambahan_data() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $_POST, $adodb, $ses;
		$record = $_POST;

		$rs = $adodb->Execute("SELECT * FROM surat_ubah_tambahan_data WHERE id_surat_ubah_tambahan_data = '{$record['oldpkvalue']}'");
		if ($rs && ! $rs->EOF) {
			...
			$rs = $adodb->Execute("SELECT * FROM surat_ubah_tambahan_data WHERE id_surat_ubah_tambahan_data = NULL");
			$adodb->Execute($adodb->GetInsertSQL($rs, $record));
			$st = "Added";
		}
		$status = "Successfull $st '<b>{$record['id_surat_ubah_tambahan_data']}</b>'";
		$this->log($status);
		$_block = new block();
		$_block->set_config('title', 'Status Surat Tambahan Data Perubahan Izin Produksi');
	...
	// handle event update surat_ubah_tambahan_data
	function update_surat_ubah_tambahan_data() {
		return $this->add_surat_ubah_tambahan_data();
	}

	// handle delete surat_ubah_tambahan_data
	function delete_surat_ubah_tambahan_data() {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		...
		if ($query)
                        $success = $adodb->Execute("delete from surat_ubah_tambahan_data where ".$query);

                if ($success){
                        $status = '<font color=green><b>'.__('Successfull Deleted').'</b></font> '.
                                'surat_ubah_tambahan_data <font color=red>'.$query.'</font>';
                } else {
                ...
                $_block = new block();
                $_block->set_config('title', ('Delete Surat Tambahan Data Perubahan Izin Produksi'));
                $_block->set_config('width', 595);
                $_block->parse(array("*".$status));
                return $_block->get_str();
	}

	// handle list surat_ubah_tambahan_data
	function list_surat_ubah_tambahan_data($extra_config='') {
//		if (! $this->get_permission('fill_this')) return $this->intruder();
		global $adodb, ${$GLOBALS['get_vars']};
		$sql = 'SELECT
		surat_ubah_tambahan_data.id_surat_ubah_tambahan_data,
		surat_ubah_tambahan_data.nomor_surat,
		surat_ubah_tambahan_data.lampiran_surat,
		surat_ubah_tambahan_data.date_surat,
		pendaftar.nama_pabrik,
		...
		$eval_arr = array ();
		$pk = array (
			'id_surat_ubah_tambahan_data' => TRUE
		);
//		if ($this->get_permission('fill_this')) {
			$add_anchor = pager::pager_button(array("link"=>'javascript:'.
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=add' . 
			'\', 600, 400, null, null, \'add_surat_ubah_tambahan_data\');'.
			'win.focus()', 
			"label"=>__("Add"),
...
//		if ($this->get_permission('fill_this')) {
			...
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=edit%s\', 600, 400, null, null, \'edit_surat_ubah_tambahan_data\');'.
			'win.focus()', 
			"label"=>__('Edit'),
...
//		if ($this->get_permission('fill_this')) {
			...
			__('Confirm Delete').'?\')?(' . 
			'win=openIT(\'' . $GLOBALS['PHP_SELF'] .
			'?action=del%s\', 600, 400, null, null, \'del_surat_ubah_tambahan_data\')'.
			'win.focus()):' . 
...
//		}
		$print_anchor = pager::pager_button(array(
			...
			'?action=print\''.
			', 600, 400, null, null, \'print_surat_ubah_tambahan_data\');'.
			'win.focus()', 
			"label"=>__('Print'),
			"type"=>"button",
		...
		$config = array (
			...
			'pk'		=> $pk,
			'form_width'	=> 595,
			'form_title'	=> __('List').' Surat Tambahan Data Perubahan Izin Produksi'.' - '.date($GLOBALS['date_format']));
		if (is_array($extra_config)) $config = array_merge($config, $extra_config);
		$_pager = new pager($config);
		return $_pager->render();
	}

	// handle print surat_ubah_tambahan_data
	function print_surat_ubah_tambahan_data() {
		$config['header_view'] = FALSE;
		$config['add_anchor'] = FALSE;
		$config['del_anchor'] = FALSE;
		$config['edit_anchor'] = FALSE;
		return $this->list_surat_ubah_tambahan_data($config);
	}
}
...
if (! $GLOBALS['_CONTROLLED']) {
	...
	$surat_ubah_tambahan_data_controller = new surat_ubah_tambahan_data_controller();
	$action = ${$GLOBALS['post_vars']}['action'] ? ${$GLOBALS['post_vars']}['action'] : ${$GLOBALS['get_vars']}['action'];
	switch ($action) {
		case 'add':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_tambahan_data_controller->add_surat_ubah_tambahan_data_form();
			$out_content .= $back_to_menu;
			break;
		case 'postadd':
			$out_content = $surat_ubah_tambahan_data_controller->add_surat_ubah_tambahan_data();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'edit':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_tambahan_data_controller->update_surat_ubah_tambahan_data_form();
			$out_content .= $back_to_menu;
			break;
		case 'postedit':
			$out_content = $surat_ubah_tambahan_data_controller->update_surat_ubah_tambahan_data();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'view':
		    	$out_extra_body =  'onLoad="DocumentLoad()"';
		    	$out_content = $surat_ubah_tambahan_data_controller->view_surat_ubah_tambahan_data_form();
			$out_content .= $back_to_menu;
			echo $out_content;
			exit;
			break;
		case 'del':
			$out_content = $surat_ubah_tambahan_data_controller->delete_surat_ubah_tambahan_data();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'import':
			$out_extra_body =  'onLoad="DocumentLoad()"';
			$out_content = $surat_ubah_tambahan_data_controller->import_surat_ubah_tambahan_data_form();
			$out_content .= $back_to_menu;
			break;
		case 'postimport':
			$out_content = $surat_ubah_tambahan_data_controller->import_surat_ubah_tambahan_data();
			$out_content .= $self_close_js;
			$out_content .= $back_to_menu;
			break;
		case 'print':
			$out_content = $surat_ubah_tambahan_data_controller->print_surat_ubah_tambahan_data();
			$out_extra_body = 'onLoad=top.window.print()';
			break;
		case 'find':
		default:
			$out_content = $surat_ubah_tambahan_data_controller->list_surat_ubah_tambahan_data();
			include_once 'depkes2_menu.php';
			exit;
			break;
	}

	$_title = 'Surat Tambahan Data Perubahan Izin Produksi Administration';
