...
class Nusoap {
    ...
    */
	...
	if($encodingStyle) {
		$ns_string = " soap:encodingStyle=\"$encodingStyle\"$ns_string";
	}
...
class nusoap_fault extends Nusoap {
	...
	* constructor
    *
    * @param string $faultcode (soap:Client | soap:Server)
	...
	*/
	function serialize(){
		...
		$return_msg =
			'<?xml version="1.0" encoding="'.$this->soap_defencoding.'"?>'.
			'<soap:Envelope soap:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"'.$ns_string.">\n".
				'<soap:Body>'.
				'<soap:Fault>'.
			...
			if ($this->headers['content-encoding'] == 'deflate' || $this->headers['content-encoding'] == 'gzip') {
				...
				if (function_exists('gzuncompress')) {
					if ($this->headers['content-encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
						...
						$this->fault('soap:Client', 'Errors occurred when trying to decode the data');
						return;
					}
				} else {
					$this->fault('soap:Client', 'This Server does not support compressed data');
					return;
				}
			}
		}
	...
	function invoke_method() {
		...
		if ($this->wsdl) {
			if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
				...
				$this->fault('soap:Client', "Operation '" . $this->methodname . "' is not defined in the WSDL for this service");
				return;
			}
		} else {
		...
		if ($class == '') {
			if (!function_exists($this->methodname)) {
				...
				$this->fault('soap:Client',"method '$this->methodname' not defined in service");
				return;
			}
		} else {
		...
		if(! $this->verify_method($this->methodname,$this->methodparams)){
			...
			// return fault
			$this->fault('soap:Client',"Operation '$this->methodname' not defined in service.");
			return;
		}
			...
			if ($this->methodparams) {
				foreach ($this->methodparams as $param) {
					if (is_array($param) || is_object($param)) {
						$this->fault('soap:Client', 'NuSOAP does not handle complexType parameters correctly when using eval; call_user_func_array must be available');
						return;
					}
					$funcCall .= "\"$param\",";
				}
		...
		// returned value(s)
			...
			if($this->wsdl){
				...
				if($errstr = $this->wsdl->getError()){
					...
					$this->fault('soap:Server', 'unable to serialize result');
					return;
				}
			} else {
	...
	*/
    function parseRequest($headers, $data) {
		...
		if($err = $parser->getError()){
			...
			$this->fault('soap:Client',"error in msg parsing:\n".$err);
...
class wsdl extends Nusoap {
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if($use == 'encoded' && $encodingStyle) {
			$encodingStyle = ' soap:encodingStyle="' . $encodingStyle . '"';
		}
