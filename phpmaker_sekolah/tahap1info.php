...
class ctahap1 {
	...
	function ctahap1() {
		...
		// kodePokok
		$this->kodePokok = new cField('tahap1', 'tahap1', 'x_kodePokok', 'kodePokok', 'rekening2.kodePokok', 200, -1, FALSE, 'rekening2.kodePokok', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodePokok'] =& $this->kodePokok;

		// id
		$this->id = new cField('tahap1', 'tahap1', 'x_id', 'id', 'rekening2.id', 19, -1, FALSE, 'rekening2.id', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlSelect() { // Select
		return "SELECT rekening2.kodePokok, rekening2.id FROM " . $this->SqlFrom();
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tahap1view.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tahap1add.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("tahap1edit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("tahap1add.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("tahap1delete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tahap1" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
...
function Row_Inserting($rsold, &$rsnew) {
		...
		$_SESSION['kode_pokok']=$rsnew['kodePokok'];   
		
		$nilai=ew_ExecuteScalar("SELECT namaPokok FROM pokokrek WHERE " .
		 "kodePokok='" . $rsnew["kodePokok"] . "' "  );
		
		$_SESSION['nama_kode_pokok']=$nilai;
		
		  
		header("Location: tahap22add.php");
		return FALSE;                  
}
