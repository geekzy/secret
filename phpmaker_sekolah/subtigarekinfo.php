...
//
// Table class for subtigarek
//
class csubtigarek {
	var $TableVar = 'subtigarek';
	...
	//
	function csubtigarek() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// kodePokok
		$this->kodePokok = new cField('subtigarek', 'subtigarek', 'x_kodePokok', 'kodePokok', '`kodePokok`', 200, -1, FALSE, '`kodePokok`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodePokok'] =& $this->kodePokok;

		// kodeSubSatu
		$this->kodeSubSatu = new cField('subtigarek', 'subtigarek', 'x_kodeSubSatu', 'kodeSubSatu', '`kodeSubSatu`', 200, -1, FALSE, '`kodeSubSatu`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodeSubSatu'] =& $this->kodeSubSatu;

		// kodeSubDua
		$this->kodeSubDua = new cField('subtigarek', 'subtigarek', 'x_kodeSubDua', 'kodeSubDua', '`kodeSubDua`', 200, -1, FALSE, '`kodeSubDua`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodeSubDua'] =& $this->kodeSubDua;

		// kodeSubTiga
		$this->kodeSubTiga = new cField('subtigarek', 'subtigarek', 'x_kodeSubTiga', 'kodeSubTiga', '`kodeSubTiga`', 200, -1, FALSE, '`kodeSubTiga`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodeSubTiga'] =& $this->kodeSubTiga;

		// namaSubTiga
		$this->namaSubTiga = new cField('subtigarek', 'subtigarek', 'x_namaSubTiga', 'namaSubTiga', '`namaSubTiga`', 200, -1, FALSE, '`namaSubTiga`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['namaSubTiga'] =& $this->namaSubTiga;

		// id
		$this->id = new cField('subtigarek', 'subtigarek', 'x_id', 'id', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `subtigarek` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `subtigarek` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `subtigarek` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("subtigarekview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "subtigarekadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("subtigarekedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("subtigarekadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("subtigarekdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=subtigarek" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->namaSubTiga->setDbValue($rs->fields('namaSubTiga'));
   ...
   // Common render codes
		...
		// namaSubTiga
		$this->namaSubTiga->ViewValue = $this->namaSubTiga->CurrentValue;
		$this->namaSubTiga->ViewCustomAttributes = "";
		...
		// namaSubTiga
		$this->namaSubTiga->LinkCustomAttributes = "";
		$this->namaSubTiga->HrefValue = "";
		$this->namaSubTiga->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('namaSubTiga', $this->namaSubTiga->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->namaSubTiga);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->namaSubTiga);
