...
$keu_master_tanggungan = NULL;

//
// Table class for keu_master_tanggungan
//
class ckeu_master_tanggungan {
	var $TableVar = 'keu_master_tanggungan';
	...
	//
	function ckeu_master_tanggungan() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// nama_biaya
		$this->nama_biaya = new cField('keu_master_tanggungan', 'keu_master_tanggungan', 'x_nama_biaya', 'nama_biaya', '`nama_biaya`', 200, -1, FALSE, '`nama_biaya`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_biaya'] =& $this->nama_biaya;

		// apakah_disembunyikan
		$this->apakah_disembunyikan = new cField('keu_master_tanggungan', 'keu_master_tanggungan', 'x_apakah_disembunyikan', 'apakah_disembunyikan', '`apakah_disembunyikan`', 200, -1, FALSE, '`apakah_disembunyikan`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['apakah_disembunyikan'] =& $this->apakah_disembunyikan;

		// kode_otomatis
		$this->kode_otomatis = new cField('keu_master_tanggungan', 'keu_master_tanggungan', 'x_kode_otomatis', 'kode_otomatis', '`kode_otomatis`', 200, -1, FALSE, '`kode_otomatis`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_otomatis'] =& $this->kode_otomatis;

		// rek_pendapatan
		$this->rek_pendapatan = new cField('keu_master_tanggungan', 'keu_master_tanggungan', 'x_rek_pendapatan', 'rek_pendapatan', '`rek_pendapatan`', 200, -1, FALSE, '`rek_pendapatan`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['rek_pendapatan'] =& $this->rek_pendapatan;

		// rek_kas
		$this->rek_kas = new cField('keu_master_tanggungan', 'keu_master_tanggungan', 'x_rek_kas', 'rek_kas', '`rek_kas`', 200, -1, FALSE, '`rek_kas`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "keu_master_tanggungan_Highlight";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "keu_master_tanggunganlist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("keu_master_tanggunganview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "keu_master_tanggunganadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("keu_master_tanggunganedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("keu_master_tanggunganadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("keu_master_tanggungandelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=keu_master_tanggungan" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				...
				$this->rek_pendapatan->ViewValue .= ew_ValueSeparator(0,2,$this->rek_pendapatan) . $rswrk->fields('D/K');
				$rswrk->Close();
			} else {
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				...
				$this->rek_kas->ViewValue .= ew_ValueSeparator(0,2,$this->rek_kas) . $rswrk->fields('D/K');
				$rswrk->Close();
			} else {
