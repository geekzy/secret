...
//
// Table class for neraca2
//
class cneraca2 {
	var $TableVar = 'neraca2';
	...
	function cneraca2() {
		...
		// Norek
		$this->Norek = new cField('neraca2', 'neraca2', 'x_Norek', 'Norek', '`Norek`', 200, -1, FALSE, '`Norek`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Norek'] =& $this->Norek;

		// Keterangan
		$this->Keterangan = new cField('neraca2', 'neraca2', 'x_Keterangan', 'Keterangan', '`Keterangan`', 200, -1, FALSE, '`Keterangan`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Keterangan'] =& $this->Keterangan;

		// tanggal
		$this->tanggal = new cField('neraca2', 'neraca2', 'x_tanggal', 'tanggal', '`tanggal`', 200, -1, FALSE, '`tanggal`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tanggal'] =& $this->tanggal;

		// saldo_debet
		$this->saldo_debet = new cField('neraca2', 'neraca2', 'x_saldo_debet', 'saldo_debet', '`saldo_debet`', 5, -1, FALSE, '`saldo_debet`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->saldo_debet->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['saldo_debet'] =& $this->saldo_debet;

		// saldo_kredit
		$this->saldo_kredit = new cField('neraca2', 'neraca2', 'x_saldo_kredit', 'saldo_kredit', '`saldo_kredit`', 5, -1, FALSE, '`saldo_kredit`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `neraca2` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `neraca2` SET ";
		foreach ($rs as $name => $value) {
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("neraca2view.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "neraca2add.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("neraca2edit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("neraca2add.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("neraca2delete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=neraca2" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) {
			...
			$Doc->ExportAggregate($this->Norek, '');
			$Doc->ExportAggregate($this->Keterangan, '');
