...
//
// Table class for pengguna
//
class cpengguna {
	var $TableVar = 'pengguna';
	...
	function cpengguna() {
		...
		// id
		$this->id = new cField('pengguna', 'pengguna', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] =& $this->id;

		// nama_pengguna
		$this->nama_pengguna = new cField('pengguna', 'pengguna', 'x_nama_pengguna', 'nama_pengguna', '`nama_pengguna`', 200, -1, FALSE, '`nama_pengguna`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_pengguna'] =& $this->nama_pengguna;

		// password
		$this->password = new cField('pengguna', 'pengguna', 'x_password', 'password', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['password'] =& $this->password;

		// username
		$this->username = new cField('pengguna', 'pengguna', 'x_username', 'username', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['username'] =& $this->username;

		// kode_otomatis_tingkat
		$this->kode_otomatis_tingkat = new cField('pengguna', 'pengguna', 'x_kode_otomatis_tingkat', 'kode_otomatis_tingkat', '`kode_otomatis_tingkat`', 200, -1, FALSE, '`kode_otomatis_tingkat`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_otomatis_tingkat'] =& $this->kode_otomatis_tingkat;

		// user_level
		$this->user_level = new cField('pengguna', 'pengguna', 'x_user_level', 'user_level', '`user_level`', 3, -1, FALSE, '`user_level`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `pengguna` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `pengguna` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("penggunaview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "penggunaadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("penggunaedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("penggunaadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("penggunadelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=pengguna" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		// kode_otomatis_tingkat
		if (strval($this->kode_otomatis_tingkat->CurrentValue) <> "") {
			$sFilterWrk = "`kode_otomatis` = '" . ew_AdjustSql($this->kode_otomatis_tingkat->CurrentValue) . "'";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->kode_otomatis_tingkat->ViewValue = $rswrk->fields('tingkat');
				$rswrk->Close();
			} else {
				$this->kode_otomatis_tingkat->ViewValue = $this->kode_otomatis_tingkat->CurrentValue;
			}
		} else {
			$this->kode_otomatis_tingkat->ViewValue = NULL;
		}
		...
		if (strval($this->user_level->CurrentValue) <> "") {
			$sFilterWrk = "`userlevelid` = " . ew_AdjustSql($this->user_level->CurrentValue) . "";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$rswrk = $conn->Execute($sSqlWrk);
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$this->user_level->ViewValue = $rswrk->fields('userlevelname');
				$rswrk->Close();
			} else {
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('nama_pengguna', $this->nama_pengguna->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->nama_pengguna);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->nama_pengguna);
	...
	function UserIDFilter($userid) {
		$sUserIDFilter = '`username` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING);
		$sParentUserIDFilter = '`username` IN (SELECT `username` FROM ' . "`pengguna`" . ' WHERE `kode_otomatis_tingkat` = ' . ew_QuotedValue($userid, EW_DATATYPE_STRING) . ')';
	...
	function AddUserIDFilter($sFilter) {
		...
		if (!$Security->IsAdmin()) {
			...
			if ($sFilterWrk <> "")
				$sFilterWrk = '`username` IN (' . $sFilterWrk . ')';
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		global $conn;
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `pengguna` WHERE " . $this->AddUserIDFilter("");
