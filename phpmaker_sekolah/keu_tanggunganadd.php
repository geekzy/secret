...
// Create page object
$keu_tanggungan_add = new ckeu_tanggungan_add();
$Page =& $keu_tanggungan_add;

// Page init
$keu_tanggungan_add->Page_Init();

// Page main
$keu_tanggungan_add->Page_Main();
?>
...
// Create page object
var keu_tanggungan_add = new ew_Page("keu_tanggungan_add");

// page properties
keu_tanggungan_add.PageID = "add"; // page ID
keu_tanggungan_add.FormID = "fkeu_tanggunganadd"; // form ID
var EW_PAGE_ID = keu_tanggungan_add.PageID; // for backward compatibility

// extend page with ValidateForm function
keu_tanggungan_add.ValidateForm = function(fobj) {
...
keu_tanggungan_add.Form_CustomValidate =  
...
keu_tanggungan_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
keu_tanggungan_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $keu_tanggungan->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $keu_tanggungan->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $keu_tanggungan_add->ShowPageHeader(); ?>
<?php
$keu_tanggungan_add->ShowMessage();
?>
<form name="fkeu_tanggunganadd" id="fkeu_tanggunganadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return keu_tanggungan_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_kode_otomatis" id="x_kode_otomatis" size="30" maxlength="50" value="<?php echo $keu_tanggungan->kode_otomatis->EditValue ?>"<?php echo $keu_tanggungan->kode_otomatis->EditAttributes() ?>>
...
<?php
$keu_tanggungan_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$keu_tanggungan_add->Page_Terminate();
?>
<?php
...
class ckeu_tanggungan_add {
	...
	// Page object name
	var $PageObjName = 'keu_tanggungan_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$keu_tanggungan->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$keu_tanggungan->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["kode_otomatis"] != "") {
				...
				$keu_tanggungan->setKey("kode_otomatis", $keu_tanggungan->kode_otomatis->CurrentValue); // Set up key
			} else {
				$keu_tanggungan->setKey("kode_otomatis", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$keu_tanggungan->CurrentAction = "C"; // Copy record
			} else {
				$keu_tanggungan->CurrentAction = "I"; // Display blank record
		...
		switch ($keu_tanggungan->CurrentAction) {
			...
			case "A": // ' Add new record
				$keu_tanggungan->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $keu_tanggungan->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "keu_tanggunganview.php")
						$sReturnUrl = $keu_tanggungan->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$keu_tanggungan->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $keu_tanggungan;
		$keu_tanggungan->kode_otomatis->CurrentValue = NULL;
		$keu_tanggungan->kode_otomatis->OldValue = $keu_tanggungan->kode_otomatis->CurrentValue;
		$keu_tanggungan->kode_otomatis_master_tanggungan->CurrentValue = NULL;
		$keu_tanggungan->kode_otomatis_master_tanggungan->OldValue = $keu_tanggungan->kode_otomatis_master_tanggungan->CurrentValue;
		$keu_tanggungan->identitas->CurrentValue = NULL;
		$keu_tanggungan->identitas->OldValue = $keu_tanggungan->identitas->CurrentValue;
		$keu_tanggungan->diskon_sosial->CurrentValue = 0;
		$keu_tanggungan->diskon_waktu->CurrentValue = 0;
		$keu_tanggungan->diskon_prestasi->CurrentValue = 0;
		$keu_tanggungan->diskon_internal->CurrentValue = 0;
		$keu_tanggungan->diskon_lain_lain->CurrentValue = 0;
		$keu_tanggungan->nilai_tanggungan->CurrentValue = 0;
		$keu_tanggungan->tanggal_rencana_bayar->CurrentValue = "0300-01-01 00:00:00";
	}
	...
	function RenderRow() {
		...
		} elseif ($keu_tanggungan->RowType == EW_ROWTYPE_ADD) { // Add row

			// kode_otomatis
			$keu_tanggungan->kode_otomatis->EditCustomAttributes = "";
			$keu_tanggungan->kode_otomatis->EditValue = ew_HtmlEncode($keu_tanggungan->kode_otomatis->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		if ($keu_tanggungan->kode_otomatis->CurrentValue == "" && $keu_tanggungan->kode_otomatis->getSessionValue() == "") {
		...
		$sFilter = $keu_tanggungan->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $keu_tanggungan->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		if ($keu_tanggungan->kode_otomatis->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(kode_otomatis = '" . ew_AdjustSql($keu_tanggungan->kode_otomatis->CurrentValue) . "')";
			$rsChk = $keu_tanggungan->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
				$sIdxErrMsg = str_replace("%f", 'kode_otomatis', $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $keu_tanggungan->kode_otomatis->CurrentValue, $sIdxErrMsg);
		...
		// kode_otomatis
		$keu_tanggungan->kode_otomatis->SetDbValueDef($rsnew, $keu_tanggungan->kode_otomatis->CurrentValue, "", FALSE);

		// kode_otomatis_master_tanggungan
		$keu_tanggungan->kode_otomatis_master_tanggungan->SetDbValueDef($rsnew, $keu_tanggungan->kode_otomatis_master_tanggungan->CurrentValue, "", FALSE);

		// identitas
		$keu_tanggungan->identitas->SetDbValueDef($rsnew, $keu_tanggungan->identitas->CurrentValue, "", FALSE);

		// diskon_sosial
		$keu_tanggungan->diskon_sosial->SetDbValueDef($rsnew, $keu_tanggungan->diskon_sosial->CurrentValue, 0, strval($keu_tanggungan->diskon_sosial->CurrentValue) == "");

		// diskon_waktu
		$keu_tanggungan->diskon_waktu->SetDbValueDef($rsnew, $keu_tanggungan->diskon_waktu->CurrentValue, 0, strval($keu_tanggungan->diskon_waktu->CurrentValue) == "");

		// diskon_prestasi
		$keu_tanggungan->diskon_prestasi->SetDbValueDef($rsnew, $keu_tanggungan->diskon_prestasi->CurrentValue, 0, strval($keu_tanggungan->diskon_prestasi->CurrentValue) == "");

		// diskon_internal
		$keu_tanggungan->diskon_internal->SetDbValueDef($rsnew, $keu_tanggungan->diskon_internal->CurrentValue, 0, strval($keu_tanggungan->diskon_internal->CurrentValue) == "");

		// diskon_lain_lain
		$keu_tanggungan->diskon_lain_lain->SetDbValueDef($rsnew, $keu_tanggungan->diskon_lain_lain->CurrentValue, 0, strval($keu_tanggungan->diskon_lain_lain->CurrentValue) == "");

		// nilai_tanggungan
		$keu_tanggungan->nilai_tanggungan->SetDbValueDef($rsnew, $keu_tanggungan->nilai_tanggungan->CurrentValue, 0, strval($keu_tanggungan->nilai_tanggungan->CurrentValue) == "");

		// tanggal_rencana_bayar
		$keu_tanggungan->tanggal_rencana_bayar->SetDbValueDef($rsnew, ew_UnFormatDateTime($keu_tanggungan->tanggal_rencana_bayar->CurrentValue, 7), ew_CurrentDate(), strval($keu_tanggungan->tanggal_rencana_bayar->CurrentValue) == "");

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $keu_tanggungan->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($keu_tanggungan->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$keu_tanggungan->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
