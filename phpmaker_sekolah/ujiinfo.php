...
class cuji {
	...
	function cuji() {
		...
		// Norek
		$this->Norek = new cField('uji', 'uji', 'x_Norek', 'Norek', 'rekening2.Norek', 200, -1, FALSE, 'rekening2.Norek', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Norek'] =& $this->Norek;

		// Keterangan
		$this->Keterangan = new cField('uji', 'uji', 'x_Keterangan', 'Keterangan', 'rekening2.Keterangan', 200, -1, FALSE, 'rekening2.Keterangan', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Keterangan'] =& $this->Keterangan;

		// kodePokok
		$this->kodePokok = new cField('uji', 'uji', 'x_kodePokok', 'kodePokok', 'rekening2.kodePokok', 200, -1, FALSE, 'rekening2.kodePokok', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kodePokok'] =& $this->kodePokok;

		// saldo_debet
		$this->saldo_debet = new cField('uji', 'uji', 'x_saldo_debet', 'saldo_debet', 'Sum(rekeningJU.debet - rekeningJU.kredit)', 5, -1, FALSE, 'Sum(rekeningJU.debet - rekeningJU.kredit)', FALSE, FALSE, 'FORMATTED TEXT');
		$this->saldo_debet->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['saldo_debet'] =& $this->saldo_debet;

		// saldo_kredit
		$this->saldo_kredit = new cField('uji', 'uji', 'x_saldo_kredit', 'saldo_kredit', 'Sum(rekeningJU.kredit - rekeningJU.debet)', 5, -1, FALSE, 'Sum(rekeningJU.kredit - rekeningJU.debet)', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function SqlFrom() { // From
		return "rekening2, rekeningJU";
	}

	function SqlSelect() { // Select
		return "SELECT DISTINCT rekening2.Norek, rekening2.Keterangan, rekening2.kodePokok, SUM(rekeningJU.debet - rekeningJU.kredit) AS saldo_debet, SUM(rekeningJU.kredit - rekeningJU.debet) AS saldo_kredit FROM " . $this->SqlFrom();
	}

	function SqlWhere() { // Where
		$sWhere = "rekeningJU.NoRek = rekening2.Norek AND rekening2.kodePokok = '4'";
		$this->TableFilter = "";
	...
	function SqlGroupBy() { // Group By
		return "rekening2.Norek, rekening2.Keterangan, rekening2.kodePokok";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO rekening2, rekeningJU ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE rekening2, rekeningJU SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM rekening2, rekeningJU WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("ujiview.php", $this->UrlParm());
	}
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("ujiedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("ujiadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("ujidelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=uji" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
