...
$st_master_pengajar = NULL;

//
// Table class for st_master_pengajar
//
class cst_master_pengajar {
	var $TableVar = 'st_master_pengajar';
	...
	//
	function cst_master_pengajar() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// nama
		$this->nama = new cField('st_master_pengajar', 'st_master_pengajar', 'x_nama', 'nama', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama'] =& $this->nama;

		// kode_pengajar
		$this->kode_pengajar = new cField('st_master_pengajar', 'st_master_pengajar', 'x_kode_pengajar', 'kode_pengajar', '`kode_pengajar`', 200, -1, FALSE, '`kode_pengajar`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_pengajar'] =& $this->kode_pengajar;

		// password
		$this->password = new cField('st_master_pengajar', 'st_master_pengajar', 'x_password', 'password', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['password'] =& $this->password;

		// kode_otomatis_tingkat
		$this->kode_otomatis_tingkat = new cField('st_master_pengajar', 'st_master_pengajar', 'x_kode_otomatis_tingkat', 'kode_otomatis_tingkat', '`kode_otomatis_tingkat`', 200, -1, FALSE, '`kode_otomatis_tingkat`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_otomatis_tingkat'] =& $this->kode_otomatis_tingkat;

		// hp
		$this->hp = new cField('st_master_pengajar', 'st_master_pengajar', 'x_hp', 'hp', '`hp`', 200, -1, FALSE, '`hp`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['hp'] =& $this->hp;

		// kode_otomatis
		$this->kode_otomatis = new cField('st_master_pengajar', 'st_master_pengajar', 'x_kode_otomatis', 'kode_otomatis', '`kode_otomatis`', 200, -1, FALSE, '`kode_otomatis`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "st_master_pengajar_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `st_master_pengajar` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `st_master_pengajar` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `st_master_pengajar` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "st_master_pengajarlist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("st_master_pengajarview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "st_master_pengajaradd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("st_master_pengajaredit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("st_master_pengajaradd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("st_master_pengajardelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=st_master_pengajar" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					$XmlDoc->AddField('nama', $this->nama->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('kode_pengajar', $this->kode_pengajar->ExportValue($this->Export, $this->ExportOriginalValue));
					...
					$XmlDoc->AddField('hp', $this->hp->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->kode_pengajar);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->kode_pengajar);
