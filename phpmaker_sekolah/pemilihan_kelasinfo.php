...
$pemilihan_kelas = NULL;

//
// Table class for pemilihan_kelas
//
class cpemilihan_kelas {
	var $TableVar = 'pemilihan_kelas';
	...
	//
	function cpemilihan_kelas() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// nama_kelas_kelompok
		$this->nama_kelas_kelompok = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_nama_kelas_kelompok', 'nama_kelas_kelompok', 'st_master_kelas_kelompok.nama_kelas_kelompok', 200, -1, FALSE, 'st_master_kelas_kelompok.nama_kelas_kelompok', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_kelas_kelompok'] =& $this->nama_kelas_kelompok;

		// apakah_valid
		$this->apakah_valid = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_apakah_valid', 'apakah_valid', 'st_master_kelas_kelompok.apakah_valid', 200, -1, FALSE, 'st_master_kelas_kelompok.apakah_valid', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['apakah_valid'] =& $this->apakah_valid;

		// kode_otomatis_tingkat
		$this->kode_otomatis_tingkat = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_kode_otomatis_tingkat', 'kode_otomatis_tingkat', 'st_master_kelas_kelompok.kode_otomatis_tingkat', 200, -1, FALSE, 'st_master_kelas_kelompok.kode_otomatis_tingkat', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_otomatis_tingkat'] =& $this->kode_otomatis_tingkat;

		// tahun
		$this->tahun = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_tahun', 'tahun', 'st_master_kelas_kelompok.tahun', 200, -1, FALSE, 'st_master_kelas_kelompok.tahun', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tahun'] =& $this->tahun;

		// kelas
		$this->kelas = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_kelas', 'kelas', 'st_master_kelas_kelompok.kelas', 200, -1, FALSE, 'st_master_kelas_kelompok.kelas', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kelas'] =& $this->kelas;

		// kode_otomatis
		$this->kode_otomatis = new cField('pemilihan_kelas', 'pemilihan_kelas', 'x_kode_otomatis', 'kode_otomatis', 'st_master_kelas_kelompok.kode_otomatis', 200, -1, FALSE, 'st_master_kelas_kelompok.kode_otomatis', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "pemilihan_kelas_Highlight";
	}
	...
	function SqlSelect() { // Select
		return "SELECT st_master_kelas_kelompok.nama_kelas_kelompok, st_master_kelas_kelompok.apakah_valid, st_master_kelas_kelompok.kode_otomatis_tingkat, st_master_kelas_kelompok.tahun, st_master_kelas_kelompok.kelas, st_master_kelas_kelompok.kode_otomatis FROM " . $this->SqlFrom();
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('nama_kelas_kelompok') . '=' . ew_QuotedValue($rs['nama_kelas_kelompok'], $this->nama_kelas_kelompok->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "st_master_kelas_kelompok.nama_kelas_kelompok = '@nama_kelas_kelompok@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@nama_kelas_kelompok@", ew_AdjustSql($this->nama_kelas_kelompok->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "pemilihan_kelaslist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("pemilihan_kelasview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "pemilihan_kelasadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("pemilihan_kelasedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("pemilihan_kelasadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("pemilihan_kelasdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->nama_kelas_kelompok->CurrentValue)) {
			$sUrl .= "nama_kelas_kelompok=" . urlencode($this->nama_kelas_kelompok->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=pemilihan_kelas" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["nama_kelas_kelompok"]; // nama_kelas_kelompok
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->nama_kelas_kelompok->CurrentValue = $key;
   ...
   // Common render codes
		...
		if (strval($this->nama_kelas_kelompok->CurrentValue) <> "") {
			$sFilterWrk = "`kode_otomatis` = '" . ew_AdjustSql($this->nama_kelas_kelompok->CurrentValue) . "'";
	...
	function Row_Inserting($rsold, &$rsnew) {
			$_SESSION['kode_otomatis_kelompok']=$rsnew['nama_kelas_kelompok'];
			$nilai=ew_ExecuteScalar("SELECT nama_kelas_kelompok FROM " .
				" st_master_kelas_kelompok WHERE kode_otomatis='" .   $rsnew['nama_kelas_kelompok'] . "' ");
			$_SESSION["nama_kelas"]=$nilai;
			header("Location: view_peserta2list.php");
			return FALSE;        
	}
