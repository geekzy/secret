...
//
// Table class for view_peserta2
//
class cview_peserta2 {
	var $TableVar = 'view_peserta2';
	...
	//
	function cview_peserta2() {
		global $Language;
		$this->AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// A_nama_Lengkap
		$this->A_nama_Lengkap = new cField('view_peserta2', 'view_peserta2', 'x_A_nama_Lengkap', 'A_nama_Lengkap', 'master_siswa.A_nama_Lengkap', 200, -1, FALSE, 'master_siswa.A_nama_Lengkap', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['A_nama_Lengkap'] =& $this->A_nama_Lengkap;

		// A_nis_nasional
		$this->A_nis_nasional = new cField('view_peserta2', 'view_peserta2', 'x_A_nis_nasional', 'A_nis_nasional', 'master_siswa.A_nis_nasional', 200, -1, FALSE, 'master_siswa.A_nis_nasional', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "view_peserta2_Highlight";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('A_nama_Lengkap') . '=' . ew_QuotedValue($rs['A_nama_Lengkap'], $this->A_nama_Lengkap->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "master_siswa.A_nama_Lengkap = '@A_nama_Lengkap@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@A_nama_Lengkap@", ew_AdjustSql($this->A_nama_Lengkap->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("view_peserta2view.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "view_peserta2add.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("view_peserta2edit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("view_peserta2add.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("view_peserta2delete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->A_nama_Lengkap->CurrentValue)) {
			$sUrl .= "A_nama_Lengkap=" . urlencode($this->A_nama_Lengkap->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=view_peserta2" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["A_nama_Lengkap"]; // A_nama_Lengkap
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->A_nama_Lengkap->CurrentValue = $key;
