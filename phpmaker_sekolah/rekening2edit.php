...
// Create page object
$rekening2_edit = new crekening2_edit();
$Page =& $rekening2_edit;

// Page init
$rekening2_edit->Page_Init();

// Page main
$rekening2_edit->Page_Main();
?>
...
// Create page object
var rekening2_edit = new ew_Page("rekening2_edit");

// page properties
rekening2_edit.PageID = "edit"; // page ID
rekening2_edit.FormID = "frekening2edit"; // form ID
var EW_PAGE_ID = rekening2_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
rekening2_edit.ValidateForm = function(fobj) {
...
rekening2_edit.Form_CustomValidate =  
...
rekening2_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
rekening2_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $rekening2->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $rekening2->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $rekening2_edit->ShowPageHeader(); ?>
<?php
$rekening2_edit->ShowMessage();
?>
<form name="frekening2edit" id="frekening2edit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return rekening2_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="rekening2">
...
<?php if ($rekening2->id->Visible) { // id ?>
	<tr id="r_id"<?php echo $rekening2->RowAttributes() ?>>
		...
		<td<?php echo $rekening2->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $rekening2->id->ViewAttributes() ?>><?php echo $rekening2->id->EditValue ?></div>
<input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($rekening2->id->CurrentValue) ?>">
</span><?php echo $rekening2->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class crekening2_edit {
	...
	// Page object name
	var $PageObjName = 'rekening2_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$rekening2->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$rekening2->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$rekening2->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$rekening2->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($rekening2->id->CurrentValue == "")
			$this->Page_Terminate("rekening2list.php"); // Invalid key, return to list
		switch ($rekening2->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$rekening2->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$rekening2->RowType = EW_ROWTYPE_EDIT; // Render as Edit
	...
	function LoadFormValues() {
		...
		if (!$rekening2->id->FldIsDetailKey)
			$rekening2->id->setFormValue($objForm->GetValue("x_id"));
	...
	function RestoreFormValues() {
		...
		$rekening2->id->CurrentValue = $rekening2->id->FormValue;
	...
	function RenderRow() {
		...
		if ($rekening2->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($rekening2->RowType == EW_ROWTYPE_EDIT) { // Edit row
			...
			// id
			$rekening2->id->EditCustomAttributes = "";
			$rekening2->id->EditValue = $rekening2->id->CurrentValue;
			$rekening2->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		$sFilter = $rekening2->KeyFilter();
			...
			$sFilterChk = "(`Norek` = '" . ew_AdjustSql($rekening2->Norek->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$rekening2->CurrentFilter = $sFilterChk;
			$sSqlChk = $rekening2->SQL();
		...
		if ($rs->EOF) {
			...
			// kodePokok
			$rekening2->kodePokok->SetDbValueDef($rsnew, $rekening2->kodePokok->CurrentValue, "", $rekening2->kodePokok->ReadOnly);

			// kodeSubSatu
			$rekening2->kodeSubSatu->SetDbValueDef($rsnew, $rekening2->kodeSubSatu->CurrentValue, "", $rekening2->kodeSubSatu->ReadOnly);

			// kodeSubDua
			$rekening2->kodeSubDua->SetDbValueDef($rsnew, $rekening2->kodeSubDua->CurrentValue, "", $rekening2->kodeSubDua->ReadOnly);

			// kodeSubTiga
			$rekening2->kodeSubTiga->SetDbValueDef($rsnew, $rekening2->kodeSubTiga->CurrentValue, "", $rekening2->kodeSubTiga->ReadOnly);

			// Norek
			$rekening2->Norek->SetDbValueDef($rsnew, $rekening2->Norek->CurrentValue, "", $rekening2->Norek->ReadOnly);

			// Keterangan
			$rekening2->Keterangan->SetDbValueDef($rsnew, $rekening2->Keterangan->CurrentValue, "", $rekening2->Keterangan->ReadOnly);

			// D/K
			$rekening2->D2FK->SetDbValueDef($rsnew, $rekening2->D2FK->CurrentValue, "", $rekening2->D2FK->ReadOnly);

			// SaldoAwal
			$rekening2->SaldoAwal->SetDbValueDef($rsnew, $rekening2->SaldoAwal->CurrentValue, 0, $rekening2->SaldoAwal->ReadOnly);

			// Saldo
			$rekening2->Saldo->SetDbValueDef($rsnew, $rekening2->Saldo->CurrentValue, 0, $rekening2->Saldo->ReadOnly);

			// TanggalSaldo
			$rekening2->TanggalSaldo->SetDbValueDef($rsnew, ew_UnFormatDateTime($rekening2->TanggalSaldo->CurrentValue, 7), ew_CurrentDate(), $rekening2->TanggalSaldo->ReadOnly);

			// target
			$rekening2->target->SetDbValueDef($rsnew, $rekening2->target->CurrentValue, 0, $rekening2->target->ReadOnly);

			// debet_kali
			$rekening2->debet_kali->SetDbValueDef($rsnew, $rekening2->debet_kali->CurrentValue, 0, $rekening2->debet_kali->ReadOnly);

			// kredit_kali
			$rekening2->kredit_kali->SetDbValueDef($rsnew, $rekening2->kredit_kali->CurrentValue, 0, $rekening2->kredit_kali->ReadOnly);

			// Call Row Updating event
			$bUpdateRow = $rekening2->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($rekening2->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$rekening2->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
